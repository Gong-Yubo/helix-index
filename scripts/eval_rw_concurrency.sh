...
#      ⚠️ **它是读数、不是门槛**（设计 §7 + `eval-report.md` §8.15.7）：档 4 的语料
#      **比档 1 多 delta 那批**（合并只并段、不删内容）⇒ 「档4/档1」只能读作
#      **「回到 1 段的形态」**、不是同一个数字 ⇒ **本脚本对该比一律不打 ✅/❌**。
#      🔴 本行原写「第 4 档**必须**回到第 1 档量级（本脚本取「≤ 1.2×」作可判定的读数口径）」
#      —— 那与「读数不是门槛」的口径**打架**，且会让将来某次 >1.2 的打印被误读成「未达标」；
#      按 **`#67` 第 1 轮评审 P4-2** 更正（旧措辞保留在此，供对照）。
...
for arm, budget_ms in (("lat-bm25", 5.0), ("lat-hybrid", 20.0)):
    ...
    # 🔑 **窗口「早期切片」对照**（`#67` 第 1 轮评审 **P4-1**）：`merge_pending` 逐段吸收 ⇒
    #    B1 窗口内**布局在渐进改善**（17 段 → 1 段）⇒ 整窗聚合把「合并干扰」与「布局改善」
    #    混在一起。窗口前 25% 里布局基本还是起始形态 ⇒ 那才是「同布局 + 干扰」的最干净读数。
    #    ⚠️ 它是**对照列、不是判据**（切片 `n` 更小 ⇒ 分位数更不稳）。
    m2 = re.search(r"p99_b1early_over_a1=([0-9.]+) b1early_n=(\d+) b1early_p99_us=(\d+)", log)
    if m2:
        r_e, e_n, e_p99 = float(m2.group(1)), int(m2.group(2)), int(m2.group(3))
        print(f"    🔑 窗口**早期切片**（前 25%，n={e_n}）P99/A1 = {r_e:.4f}"
              f"  ⓘ **对照，非判据**（B1 整窗混入了「布局改善」；切片才是「同布局 + 干扰」口径）"
              f"   [切片 p99={e_p99}us]")
    else:
        print("    ⓘ 窗口早期切片：⚠️ 读数缺失（载体未输出 `p99_b1early_over_a1`）")
...
if "stage1" in hot and "stage4" in hot:
    ...
    # 🔴 **本行不打 `mark()`**（`#67` 第 1 轮评审 **P4-2**）：设计 §7 与 `eval-report.md` §8.15.7
    #    都写明 S8-T12 是**读数不是门槛**，且**档 4 的语料 ≠ 档 1**（多 delta 那批）⇒ 打 ✅/❌ 会
    #    把它变成**隐形门槛**，将来某次 >1.2 的打印会被误读成「未达标」。⇒ 只报数 + 标注口径。
    print(f"① S8-T12 档4/档1 的 BM25 lane P50 比 = {r:.4f}"
          f"   ⓘ **读数（非门槛）**；档 4 语料 ≠ 档 1（多 delta 那批）⇒ 读作「回到 1 段的形态」"
          f"   [档1={hot['stage1'][0]:.0f}us 档2={hot.get('stage2',(0,0))[0]:.0f}us"
          f" 档3={hot.get('stage3',(0,0))[0]:.0f}us 档4={hot['stage4'][0]:.0f}us]")
    # R52 的两项成本**分开报**（与 `eval-report.md` §8.15.7 的两条读法一一对应）：
    # 谓词成本 = 档3 − 档2（**同语料**，只差墓碑）；多段扫描成本 = 档2 − 档1。
    print(f"   （R52 读数：谓词成本 档3 − 档2 = "
          f"{hot.get('stage3',(0,0))[0] - hot.get('stage2',(0,0))[0]:+.0f}us；"
          f"多段扫描成本 档2 − 档1 = {hot.get('stage2',(0,0))[0] - hot['stage1'][0]:+.0f}us）")
