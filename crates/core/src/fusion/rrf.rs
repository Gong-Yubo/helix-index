...
mod tests {
    ...
    /// tier 1 + tier 2 两臂同时命中时 `w[0]` 仍为 `0`（幂等）、`w[1]` 不受牵连。
    ///
    /// ⚠️ 如实登记（评审 P4-2 实测）：本用例**区分不了 tier 1 是否生效**——θ=0.9 时
    /// tier 2 自己就把 `w[0]` 置 0（置 0 幂等 ⇒ 两臂写同一个值）；M1 变异（关 tier 1）
    /// 下本用例**仍绿**。当前实现没有「叠加打折」的路径（乘性/减性形态），要断言
    /// 「不重复打折」得先有那种形态。它钉住的是**w[1] 不被误伤**这一条。
    #[test]
