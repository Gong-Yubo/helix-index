...
pub enum Error {
    ...
    ///
    /// # 产生点与寿命（2026-09-19 按 `S8-03` 评审批次同步，P3-4.1 + P2-1）
    ///
    /// ⚠️ 原文写「`S8-02` 的过渡约束是唯一产生点，`S8-03` 后内核不再产生该错误」——
    /// **该说法已作废**：过渡约束（`with_main_mut`）确实已随 `S8-03` 删除，但同一批次引入了
    /// **两条新的产生点**，都在 `SearchIndex::commit()` 的**发布前对账**里
    /// （同一 `Arc<Shared>` 上可有多个写端 —— `Searcher::into_index()` 总是成功）：
    ///
    /// | 产生点 | 触发条件 | 语义 |
    /// | --- | --- | --- |
    /// | **基址对账** | 另一个写端在本写端 `add` 之后 `commit()` 过 | 段内 ID 与全局 ID 已不对应 |
    /// | **世代对账** | 另一个写端 `compact()` 过（重编号 ⇒ 旧 ID 空间作废） | 基址**等值也拦不住**（ABA），故按世代判 |
    ///
    /// 🔴 **这两条不满足「重试即可」的承诺**：本写端 builder 里**未提交的内容已被丢弃**
    /// （`commit()` 末尾无条件换新 builder）⇒ 重试**不会**恢复它，必须**重新 `add`**。
    /// 两条的错误消息里都写明了这一点（NFR-07：语义偏差不得静默）。
