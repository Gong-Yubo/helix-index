...
pub enum Error {
    ...
    /// | **世代对账**（`commit()` / `fold`） | 另一个写端 `compact()` 过（重编号 ⇒ 旧 ID 空间作废） | 基址**等值也拦不住**（ABA），故按世代判 | 同上 |
    /// | **快照前缀身份**（`fold_deltas()`，P1-2） | 另一个写端 `fold` 过（增量段列表被**替换**，长度可能相同） | 世代**不变**（`fold` 不换代）⇒ 世代对账拦不住 | **只有本次合并的计算**（内容仍在视图里） |
    ///
    /// 🔴 **前两条不满足「重试即可」的承诺**：本写端 builder 里**未提交的内容已被丢弃**
    /// （`commit()` 末尾无条件换新 builder）⇒ 重试**不会**恢复它，必须**重新执行未成功的
    /// `add` 与 `remove`**（`remove` 也要重做 —— 分支②的跨段墓碑同样在 builder 上，P3-1）。
    /// ⚠️ **第三条不同**：`fold` 是**维护性**操作，被判拒时增量段**仍在视图里、内容没丢**，
    /// 恢复方式就是**重试本次 `save()` / `compact()`** —— 三条消息各自的恢复指引都写明了差别
    /// （NFR-07：语义偏差不得静默）。
