...
/// 整体偏慢 ×1.38，归一后 sel-0.1% / sel-1% / tier-5% 分别为 46.6× / 11.2× / 2.06×，
/// 反转结论不变。）交叉点插值落在 `allowed ≈ 9700`。
///
/// 取 **8192**（= 2¹³）：低于实测交叉点、也低于首个反转档位 `9946`，留 ~15% 余量——
/// R31 指出路径 C 的 `O(N)` 访问项随规模线性增长（而高选择度下路径 B 由 `ef` 封顶），
/// 交叉点会**随规模下移** ⇒ **宁低勿高**。
///
