...
//! 2. **不覆盖「有人把生产循环改回 `IntoIterator`」** —— 改回去之后**覆盖仍然正确**
//!    （`IntoIterator` 的语义就是全量），所以 `hnsw_rs_index.rs` 的断言**抓不住它**；
//!    能抓它的只有本文件的存在 + 评审红线（同族的「不变式靠评审守」见架构 §14.6）。
//!    ⇒ 这也是本文件的主要价值：把「为什么不能用 `IntoIterator`」变成**可执行的证据**。
//! 3. **不覆盖 `get_max_level()` 与 `get_max_level_observed()` 的混用** —— 那条由生产侧
//!    `全量遍历基数等于点数且零重复` 的**层号范围断言**守（层号只写 0 会立刻少点）。
