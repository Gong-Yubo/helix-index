...
//!
//! ⚠️ **T10 由 S6-10 收尾补入**（原属 PR 5 的承诺，实现期漏做；详见该用例的文档注释与
//! `v2-step6-design.md` §7 末列状态、§11.6）。**T14** 编号下现挂的是 **PR #43（`a5c2693`）
//! 改向后的用例**（半向量判据的前提，与 E3 无关）；原 S6-T14 的 `ConfigMismatch` 探针
//! **随 S6-05 挂起**，并非由本批「补入」。
//!
...
///
/// ⚠️ **覆盖边界**（PR #43 第 2 轮评审后更新，别照旧版读）：
/// - **已进 CI**：`build()` 里那条**预检**（`reject_vectorless_snapshot`）—— 它只读快照指纹
///   （`embedder_id` 为空）、**不进 embedder 构造** ⇒ `CLI smoke` 的「半向量守卫必须拒绝」
///   一步就能覆盖它，**不需要模型**。
/// - **仍未覆盖**：`build_into_existing` 里基于 `tombstone_stats()` 的**权威判据**那条
///   **接线**（要真模型构造 embedder 才能 `load`）。它的**判据**由本测试覆盖（两个方向），
///   **接线**由本地端到端 + 变异验证负责。
#[test]
