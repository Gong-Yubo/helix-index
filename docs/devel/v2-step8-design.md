...
- **基址对账 / 世代对账**（`commit()`）：丢的是**本写端 builder 里未提交的内容 + 跨段墓碑** ⇒ 恢复 = **重做 `add` 与 `remove`**；
