...
**版本面**：需求 / 架构 / `plan-v2` **随判后记录升版本行** —— 需求 **v1.22 → v1.23**、
架构 **v1.21 → v1.22**、`plan-v2` **v0.22 → v0.23**（按仓内先例：**评审响应即使只改笔误也升
版本行**，成因见 `v1.21`）⇒ 三份文档各加一行版本记录，并同步「文档版本 / 状态」两面 +
§1.1 / §15.2 / 附录 C 三个版本表。设计文档按体例升 **v0.16** 并新增本节。

⚠️ **旧措辞更正（第 2 轮评审 P4）**：本段**原写**「本轮**不升**需求 / 架构 / `plan-v2` 的版本号
—— 它们当轮的 **v1.22 / v1.21 / v0.22** 就是「Step 8 收尾」这一件工作（PR 尚未合并）⇒ 只
**就地扩写**那三行版本描述」—— 那是**决策改变之前**的写法（先按「同一件工作不重复开版」写下，
随后按仓内先例改为**升版本行**），我却**只改了版本行、漏改了本段** ⇒ 描述与实际矛盾。
**旧句保留在此供对照**，实际改动以上一段为准（提交 `ba5ac59` 可证）。
...
#### 4.13.6 第 2 轮评审响应（v0.17，2026-09-23）

评审（`pulls/67/reviews` `5287966572`，基线 **`04a668d`**，**纯 review body、无新增行内意见**）
= **0×新意见 + 1×P4（文档小疵）**，结论「**复审通过、无阻塞项、达到可合并形态**」。
第 1 轮的 **1×P3 + 3×P4 逐条签收 ✅**；评审还**用更新后的脚本又独立复跑了一次**（`SKIP_WRITE=1`、exit 0），
新机制全部生效：②a **1.1325 / 1.0114**、早期切片对照列**正常输出**（1.1224 / 1.0352）、
②b **3.47 / 4.25 ms**、四档 **19 / 22 / 23 / 21 us**（档4/档1 = 1.1053，**无门槛标记** ✓）、
R52 分列 谓词 **+1 us** / 多段 **+3 us**、CI head `04a668d` **10/10**。

| # | 位置 | 处置 |
| --- | --- | --- |
| 🔵 **P4** | §4.13.5 的「版本面」段**与实际改动矛盾**（写「**不升**需求 / 架构 / `plan-v2` 的版本号 ⇒ 只就地扩写」，而实际升了 **v1.23 / v1.22 / v0.23**） | ✅ **已改准**（本文件 §4.13.5）：改为「**随判后记录升版本行**（`v1.22 → v1.23` / `v1.21 → v1.22` / `v0.22 → v0.23`），按仓内先例」；⚠️ **旧句保留**并标注为「旧措辞更正」 |

**根因与一般化（值得写下来）**：这段是**在决策改变之前**写下的 —— 先按「同一件收尾工作不重复开版」
写下「不升号」，随后按仓内先例（`v1.21` 就是为**两处笔误**升的）改为**升版本行**，
结果**只改了版本行、漏改了「描述这件事」的那一段**。
⇒ **一般化**：**「改了做法，就要回头扫『描述该做法的句子』」** —— 它与「改了行为要回头改正文里的
旧断言」（`#66` 的功课）是**同一类**，但**载体的位置更隐蔽**：不在「结果」里，而在「**方法 / 说明**」里。
⇒ **提交前可扫判据**：对本轮写下的每一个**否定式 / 选择式说明**（「本轮**不** X，只 Y」「改为 Z」）
**再确认一次它仍然成立**（本轮的错误句恰好就是「本轮**不**升…」这个形态）。
⇒ 同类先例：#66 的「结论已出、文档仍是将来时」是**时态**错位；本轮是**决策变更后描述未同步**错位。

**版本面（本轮）**：**只升设计文档 v0.16 → v0.17** —— 因为本轮**没有**改需求 / 架构 / `plan-v2` 的
**内容**（第 1 轮升的三行版本记录**保持原样**）。⚠️ 这正是「**升版本行 ⇔ 内容真的变了**」的对偶检查：
上一条 P4 的教训就是**描述与实际不符**，所以本轮把「**哪份文档真的改了内容**」也写清楚
（可复核：`git diff --stat ba5ac59..HEAD` 只含本文件）。

**未闭环 / 盲区**：同 §4.13.5（切片非判据 / 同深度尾部仍未决 / 未在 Linux / CI 复跑）；
另：评审本轮的复跑读数与本机**同参数不同轮次**的读数为**同量级、同结论**
（②a bm25 **1.13** vs 我上轮的 **1.09**、hybrid **1.01** vs **1.07**；四档 19/22/23/21 vs 20/24/25/22）
⇒ 仍**只报「同向 + 同结论」，不做逐位比对**（跨轮次数字不可相减）。
