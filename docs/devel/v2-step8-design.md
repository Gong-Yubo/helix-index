...
| 文件 | 变更 |
| --- | --- |
| `crates/core/src/search/view.rs`（**新**） | `Segment` / `View` / `Shared` / `Tombstones` / `IdAllocator` + `publish` / `snapshot` / `with_main_mut` / `advance` |
...
| 落点 | 用例 | 说明 |
| --- | --- | --- |
| `view.rs`（4） | `S8_02_初始视图的deltas恒空且序号为零` / `S8_02_发布只递增序号且不产生新段` / **`S8_02_有读者时写入必须失败`** / `S8_02_已取出的快照不受后续发布影响` | 骨架不变式；第 3 条在 `S8-03` 时应**删除**（不是改成永远绿） |
| `index.rs`（1） | `S8_02_commit递增视图序号` | 🔴 **必须是单测**（见下方覆盖边界 ①） |
...
| # | 注入 | 实测结果 |
| --- | --- | --- |
| **M5** | `commit()` **不 `publish`** | ⚠️ **首轮零命中**（`tests/step8_segments.rs` 8 条**全绿**）⇒ 补 `index.rs` 的单测后 **1 条红**（`S8_02_commit递增视图序号`），其余 10 条绿 |
| **M6′** | `into_searcher()` 去掉隐含 `commit()` | ⚠️ **首轮零命中**（全仓测试**全绿**）⇒ 把「旧 API vs 新 API 逐位一致」改用**带向量 lane** 的装配后 **1 条红**，其余 7 条绿 |

##### 🔴 两条覆盖边界（**如实登记，不声称已覆盖**）

① **「`commit()` 不 `publish`」在外部 API 层面不可观测**：`S8-02` 期内容与视图**同源**
