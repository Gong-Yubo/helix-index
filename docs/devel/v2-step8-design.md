...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.16（2026-09-22，`S8-09` 第 1 轮评审响应）** —— 评审 = **0×P1/P2 + 1×P3 + 3×P4**（结论「**无阻塞项**」；评审还**独立复跑了整个脚本**并逐项复现读数），**四条全部采纳**：① 🟡 **P3-1** 补齐 **`NFR-10` ③ 的判后记录**（三个载体：需求行 / 架构 **`R43`** / `plan-v2` 门槛行 ④）；② 🔵 **P4-1** 载体新增**窗口早期切片**（`B1` 前 25%）并**量化**「布局渐进改善混进聚合口径」的效应（hybrid 整窗 mean **−12.2%** vs 切片 **−0.4%**）⇒ 就地更正 §4.13.1 保留 1 的「同向」措辞（原句保留）；③ 🔵 **P4-2** 脚本去掉 `档4/档1` 的 ✅/❌（**读数是读数、不是门槛**）；④ 🔵 **P4-3** `eval-report.md` §8.15.6 / §8.15.7 各补**精确复现命令**。新增 **§4.13.5**。 **v0.15（2026-09-22，`S8-09` 收尾：标定 + 四处定义面回写）** —— Step 8 收尾 PR（**零生产代码改动**）：① **`S8-T11` 已落地**：新增 `examples/eval_rw_concurrency.rs` + `scripts/eval_rw_concurrency.sh`（A1→B→A2 三臂；**基线定义写死为「合并起始布局」**）；② 🔴 **`NFR-14` ②a / ②b 口径定稿（去「拟」）**，但 **`NFR-14` 整体保持「打开」**（含两条保留：样本数不可比 + 尾部扰动不可复现）；③ **`S8-T12` 量化 R52**（谓词成本 **+1 us**、多段扫描 **+8 us**）；④ **`S8-T15` 落地 ⇒ `NFR-11` 去「拟」定稿**（P50 0.694 s / P99 1.026 s）；⑤ **四处定义面回写**（需求 v1.22 / 架构 v1.21 / `plan-v2` / `docs/README.md`）+ **`R51`~`R54` 残余回填**（`R54` 记为**消解**：因判「不投」而条件未触发）；⑥ **修 `FR-17` 的状态漂移**（§5.3.5 的「完整版归 V2.1」→ 已由 V2.0 Step 8 交付）。**不改任何决策（`D-S8-01` ~ 12）与风险编号。** **v0.14（2026-09-22，`S8-08` 第 1 轮评审响应）** —— 评审 = **0×P1/P2 + 1×P3 + 3×P4**（结论「PR7a 范围内无阻塞项、达到可合并形态」），**逐条全部采纳**：① 🟡 **P3-1** 把公开面 rustdoc 的**将来时**全部回填为「**已判『不投』** + 当前无 >1 的推荐值」—— 载体实为 **7 处**（评审点到 2 处，另 5 处同族一并扫到）；② 🔵 **P4-1** 修 `slot_index` 注释与代码不符；③ 🔵 **P4-2** 在 `eval-report.md` §8.15.3 **钉死 `QPS(4)/QPS(1)` 的映射定义**并**并列三个读法**（2.10× / 2.04× / 1.66×，三者皆 < 2.5× ⇒ **结论对读法稳健**）；④ 🔵 **P4-3** 不加 cap、写清失败模式，并**更正评审「会 abort」的前提**（实测是**可捕获的 panic**）；⑤ ⚪ 一项「不立项」按评审归属**留 PR7b**，本轮把**载体范围**查清并登记，**同时更正我自己上一轮的登记**（「无缓存下取模型**并失败**」不实：实测**取模型成功**（96 MB / 54 s），失败只出现在**冷缓存 + 并发**）。⚠️ 本轮**无断言可变异**（全是文档/注释）⇒ 替代验证 = 正向断言 8 条 + 旧措辞扫零 + 三读法复算。新增 **§4.11.5**；不改任何决策（`D-S8-01` ~ 12）与风险编号。 **v0.13（2026-09-21，`S8-08` = PR7a：查询侧会话池 + spike S8-S2）** —— 本轮**只回填实施结果与标定结论、不改任何决策**：① **会话池落地**（`LocalEmbedder` 的 `Vec<Mutex<TextEmbedding>>` + 轮转取槽、`Config::embed_sessions` **默认 1**、`SearchIndexBuilder::embed_sessions(n)`；⚠️ **不进 `ConfigFingerprint`**，`dim` / `id()` / `is_normalized()` 一字不变 ⇒ **零行为变化**）；② 🔴 **spike S8-S2 判「不投」**（三条合取不成立）：判据① **PASS**（含一个**加强**结论 —— `intra_threads` 在本机本模型上**也**不改变数值）、判据② **2.10× < 2.5×**、判据③ **+148.4% ≫ 20%**；⇒ `NFR-10` ③ 与 `R43` **保持「打开」**，四处定义面回写归 **PR7b（`S8-09`）**；③ ⚠️ **两条供评审复核的加法**：判据③ 的阈值在**查询侧口径**下对任何 `sessions > 1` **结构性不可达**（**算术支撑**：≈ 97 MiB/槽，只加到 2 槽也约 +50%）；判据② 的机理（每槽吃满核 ⇒ 过订阅）**尚未单独证伪**（生产 `LocalEmbedder` 不给 `intra_threads`）；④ **PR7 拆为 PR7a / PR7b**（用户拍板）。新增 **§4.11.4**；读数落 `eval-report.md` **§8.15**。**不改任何决策（`D-S8-01` ~ 12）与风险编号。** **v0.12（2026-09-21，`S8-06` 第 1 轮评审响应）** —— 本轮按评审 **1×P1（阻塞）+ 1×P2 + 1×P3 + 4×P4** **七条全部采纳**（评审的「无异议」清单逐条签收 ✅）：① 🔴 **P1-1（阻塞）**：`merge_pending` 的**部分合并**会把**指向未合并段**的跨段墓碑「摘掉但没物理化」（`Index::remove` 对越界 `doc_id` 是静默 no-op）⇒ **已删文档复活**、被固化进主段并随 `save` 落盘（**删除永久丢失**）⇒ 修法 = `Tombstones::below(upper)` **按本次合并区间过滤**（与 §4.9.6 的 P1-2「前缀身份」同一思路），`tombstones_applied` 只计**真物理化**数；判据 ×2（含 `save`→`load` 臂）+ 纯函数单测钉边界 + **4 组变异**；② **P2-1**：零向量增量段原会触发向量侧**全量重建**（`O(N·logN)` + 拓扑重随机）⇒ 改走 A 的 `dump→load` **零插入**；**文本侧**「零内容段白付一次 `merge_from`（含全量 `field_index.rebuild`）」**一并修**（槽位全 0 即跳过，逐位等价），`Metrics.segments` 膨胀则**有意保留**并写明理由；③ **P3-1**：`S8_T10` 读端的**记忆序窗口** ⇒ 宽限重读 + **三臂探针**（窗口是真的 / 宽限关得掉 / 检测不缩水）；④ **P4-1~4** 全部采纳（含从 `git show` **逐字取回**被截断的判据原文）；⑤ 🔴 **顺带登记**：我自己的 markdown 检查器**漏报了 P4-1**（它要求**行尾也有分隔符**才认表格行）⇒ 已修判据 + 补两组对照样本；该判据在全仓扫出的 `thirdparty.md` 3 处经「比内容」核对 = **非本 PR 引入**、**不擅自改**。**不改任何决策（`D-S8-01` ~ 12）与风险编号。** 详见新增 **§4.9.8**。 —— **只回填实施结果、不改任何决策**：① 交付 `merge_pending()` / `merge_all()` / `MergeReport` / `VectorMergeStrategy`（原 `fold_deltas` 参数化为 `merge_n(take)`）；② 向量侧**方案 A 落地**、失败/不支持回落 B（**spike S8-S1 五条判据全 PASS**：A/B = **0.134**）；③ 🔴 **判据③ 的阈值就地更正**（原「重合率 ≥ 0.99」**结构性不可达** —— 同数据重建两次只有 0.9280）⇒ 改为「相对基线 + `Brute` 参照的 recall」；④ 新增 **§4.9.7**（实施结果 / spike 读数 / 结构边界 / 偏差）。**不改任何决策（`D-S8-01` ~ 12）与风险编号。** **v0.10（2026-09-20，PR5 第 1 轮评审响应）** —— 本轮按评审 **1×P3 + 1×P4**（**无阻塞**；评审结论 = 「PR5 范围内无阻塞项、达到可合并形态」）**两条全部采纳**：① **P3-1（已实证的覆盖盲区）**：三条跨段向量用例的夹具 `main` **恒为空** ⇒ 「向量路漏查 `main`」这一类变异**全仓不可检测**（评审注入实证 + 我独立复现 17/17 绿）⇒ 补 `main(2) + delta(2) + delta(2)` 变体 + **3 组变异自证**（新变体是该缺口的**唯一绊线**），并抽共用断言助手；② **P4-1**：CHANGELOG 的 Breaking 段补 `SearchParts.vector_segments`。**四处取形评审逐条确认 ✅、一处未改**。详见新增 **§4.6.5**。**v0.9（2026-09-20，PR5 = `S8-05` 实施结果）** —— 本轮**只回填实施结果、不改任何决策**：新增 **§4.6.4 实施结果**（落点表 + **`vector_shortfall` 的偏差**：公式未改、理由见该节 + `Mixed` 的端到端覆盖边界）；**§6 的 Q5 行**由「取形待定」改为**已落地**（新增 `Mixed` 变体，CHANGELOG 记 `Breaking`）；**§7 的 S8-T5 行**标为已落地（+2 条同批用例）。**v0.8（2026-09-20，PR #63 第 3 轮评审响应）** —— v0.7（2026-09-19，PR #63 第 2 轮评审响应）→ 本轮为 `S8-03` 实现 PR 的**第 3 轮**评审（**复审通过**：原文「PR4（`S8-03` / `S8-04` + 最小合并）范围内评审通过 —— **无阻塞项**」，且它**独立复现**了两条 P1 的变异自证、实跑 3 条新判据），唯一新意见 = **1×P4**（`commit_view` 的 epoch 拒绝消息被 `commit()` 与 `fold_deltas()` 共用 ⇒ 对 `fold` 侧三条声明全不成立）⇒ **已采纳并顺手修**：新增 **`PublishCaller`**（`Commit` / `Fold` + `loss_and_recovery()`）⇒ 消息按调用方在**同一个地方**分家；判据 `S8_03_epoch拒绝的恢复指引按调用方分家`（变异：两臂对调 ⇒ 唯一红）+ 端到端**接线判据**；落点 **3 处**（`search/view.rs` / `error.rs` 表 / **本节**，第 3 处评审没点到）。**不改任何决策（D-S8-01 ~ 12）与风险编号。** v0.6（PR #62 第 1 轮评审响应）→ 本轮为 `S8-03` 实现 PR 的**第 2 轮**评审响应，**2×P1 + 1×P3 + 1×P4**：① 🔴 **P1-2 更正了 `absorb_window` 的判据**（「长度前缀」**不够** —— 并发 `fold` 不改 `epoch` ⇒ 世代对账有盲区；判据升级为**前缀 `Arc::ptr_eq` 身份校验**，不符即拒绝发布）⇒ 新增 **§4.9.6** + **§4.3.2 的锁内拒绝说明**；② 🔴 **P1-1 更正 §4.8.3 的查重伪代码**（墓碑有**两个落点**，只查已发布那个会静默吞掉重加/替换）；③ **P3-1** 三条 `Busy` 消息的**恢复指引按语义分家**（`commit()` 丢内容 ⇒ 重做 `add`/`remove`；`fold` 只丢计算 ⇒ 重试 `save`/`compact`）；④ **P4-1** 墓碑-only 空段的次生成本登记给 `S8-06`。**v0.6（2026-09-18，PR #62 第 1 轮评审响应）** —— v0.5（`S8-02` / `T7-18` 实施结果）→ 本轮为**实现 PR 的评审响应**：① 新增 **§4.3.5 第 1 轮评审响应**（**6 条全部采纳**，含 **P2 阻塞级**的 `commit()` 原子化：**两套实测**量化 + `M7`/`M8` 变异 + 2 条覆盖边界）；② 🔴 **P2 修复改变了实现取形**：`Shared::publish` / `advance` **已删除**，发布逻辑内联进新增的 `Shared::commit_view()`（持 `ids` 锁完成四步）⇒ **§4.3.2 / §4.3.4 的落点表已同步**；③ `tombstone_stats()` 改为**真·跨段求和**（抽出 `View::sums()` 纯函数）。**v0.5（2026-09-18，`S8-02` / `T7-18` 实施结果 = PR3）** —— v0.4（PR #61 第 1 轮评审响应）→ 本轮**只回填实施结果、不改任何决策**：① 新增 **§4.3.4 实施结果**（落点 / **5 条与正文的偏差** / 13 条测试 / **变异验证 M5·M6′** / **2 条覆盖边界**）；② ⚠️ **`into_index()` 不再可能失败**（行为变更，§4.12 已指定取形、但后果需确认）；③ **过渡约束**：`with_main_mut` 要求「无并发读者」、**`I8-2` 在 PR3 尚未成立**（由 S8-03 解除）；④ 可见性语义**刻意保留**「未 commit 亦可见」。**v0.4（2026-09-18，PR #61 第 1 轮评审响应）** —— v0.3（S8-01 / T7-25 实施结果）→ 本轮按评审 **4×P4**（**无阻塞**）+ 行内 4 条逐条处置：① **P4-1 数值勘误**（`L ≈ 2.4` → **`E[L] ≈ 2.9`**，口径一并改正为**最大次序统计量**）；② **P4-2 删除冗余断言**（`assert!(l0 < n)` **永不报红**）；③ **P4-3 不立项**、改交**行为级**替代物（**新增空图断言**）；④ **P4-4** PR 正文 run id 笔误；⑤ 🔴 **覆盖边界②就地更正**（原写「改回 `IntoIterator` 时任何 CI 断言都不会红」**已不准确** —— 空图断言能抓住；**M4 变异实测**：该用例红、其余 10 条全绿）。逐条处置见 **§4.2.1 末节**。**v0.3（2026-09-18，S8-01 / T7-25 实施结果）** —— v0.2（2026-09-17，第 1 轮评审响应）→ 本轮**只回填实现结果、不改任何决策**：① **T7-25 已实现**（`search_exact_filtered` 的全量遍历改为**逐层**，与 §4.2 的写法逐字一致）；② **S8-T1 的实际形态**（std-only 复刻体 + **受闸写者**握手 + 两条臂）与**平台前提实测**（macOS + Linux 各 5/5）；③ 🔴 **实现期新发现两条「覆盖边界」**（见 §4.2.1）：覆盖类断言**必须经由生产函数**（在用例内复刻循环 ⇒ 变异下仍绿，已实测）、把生产循环**改回 `IntoIterator`** 时**任何 CI 断言都不会红**（⚠️ **该表述已被 v0.4 就地收窄**：空图断言能抓住，见 §4.2.1）；④ **变异表**（3 组注入 + 命中）；⑤ **行号漂移实测 = 0**（活文档 13 处引用全部 ≤ 改动区间）。**v0.2（2026-09-17，第 1 轮评审响应）** —— v0.1（2026-09-16，首版）→ 该轮按评审 **5×P3 + 7×P4**（**无阻塞**）逐条处置：**12 条意见全部采纳**；其中 **P3-1（跨段墓碑在合并时的处置）/ P3-4（双写端 `commit()` 的原子性）** 是本轮**最实质的两条**（补写 §4.4.3 / §4.7.3 / §4.9.2 / §4.9.5 的取形与用例）；**P3-5（`Box::leak` 累积）** 结论采纳、**量级前提经独立复算后更正**（不是「按图规模增长」，实测 ≈ **200B + 路径串 / 每次合并**）；**P4-1（「100+ 处调用点」）** 经独立复算改为 **41 处**。⚠️ **本轮不改任何决策取值（D-S8-01 ~ 12）与风险编号**；逐条处置见 **§10**。 |
| 日期 | 2026-09-16 |
| 状态 | **`S8-09` 第 1 轮评审已响应（v0.16，2026-09-22）**：**1×P3 + 3×P4 全部采纳**（结论「无阻塞项」）—— P3-1 补 `NFR-10` ③ 的判后记录（需求 / 架构 `R43` / `plan-v2` 三处）；P4-1 新增「窗口早期切片」对照并**量化**布局改善的混合效应（hybrid 整窗 mean −12.2% vs 切片 −0.4%）⇒ 就地更正「均值同向」的措辞（⚠️ **主判据结论不变**）；P4-2 脚本去掉隐形的 ✅/❌；P4-3 补精确复现命令。详见 **§4.13.5**。 **`S8-09` 收尾已完成、PR7b 已开（v0.15，2026-09-22）**：**零生产代码改动**；`NFR-14` ②a·②b **定稿**（②a 三次运行 ≤ 1.2，基线写死为「合并起始布局」）但**条目保持「打开」**；`NFR-11` **去「拟」定稿**（P50 0.694 s / P99 1.026 s）；`R51`~`R54` 残余已按实测回填（`R54` **消解**）；读数落 `eval-report.md` **§8.15.6 ~ §8.15.8**。详见 **§4.13.4**。 **`S8-08` 第 1 轮评审已响应（v0.14，2026-09-22）**：**1×P3 + 3×P4 全部采纳**（评审结论 = 「PR7a 范围内无阻塞项、达到可合并形态」）—— P3-1 把公开面 rustdoc 的**将来时**全部改成「已判『不投』」（**7 处载体**，评审点到 2 处）；P4-2 在 `eval-report.md` §8.15.3 **钉死映射定义 + 并列三读法**（2.10× / 2.04× / 1.66×，皆 < 2.5× ⇒ 结论对读法稳健）；P4-3 不加 cap 但**写清失败模式**并更正评审「会 abort」的前提（实测 = **可捕获的 panic**）。本轮**无断言可变异**（全是文档/注释）。详见 **§4.11.5**。**`S8-08`（查询侧会话池）已实现且**标定结论 = 「不投」**（v0.13，2026-09-21，PR7a）**：池化落地（默认 1 ⇒ 零行为变化）+ spike S8-S2 的三条合取判据 **① PASS / ② 2.10× / ③ +148.4%** ⇒ **判「不投」**，`NFR-10` ③ 与 `R43` 保持「打开」（四处回写归 PR7b）。详见 **§4.11.4** + `eval-report.md` **§8.15**。**`S8-06` 第 1 轮评审已响应（v0.12，2026-09-21）**：**1×P1（阻塞）+ 1×P2 + 1×P3 + 4×P4，七条全部采纳** —— P1-1（部分合并丢弃跨段墓碑 ⇒ 已删文档复活）**先确定性复现、再修**，判据 + 变异齐备；P2-1 把「零向量增量 ⇒ 全量重建」改为 A 的**零插入**并把文本侧同类一并修；P3-1 补宽限重读（**三臂探针**证明检测能力不缩水）。详见 **§4.9.8**。**`S8-06`（分段合并器）已实现（v0.11，2026-09-20）**：`merge_pending()` / `merge_all()` / `MergeReport` + 向量侧**方案 A**（spike S8-S1 标定，A/B = 0.134）；门测试 = `S8-T10`（写/合并/读并发 + 双写端）/ `S8-T13`（字段索引等价）/ `S8-T14`（向量侧不丢点，**判据口径已按 spike 更正**）。⚠️ **`S8-T10` 的「三线程」不可达**（`!Sync`）⇒ 实为两线程。 **PR5 第 1 轮评审已响应（v0.10，2026-09-20）**：**1×P3 + 1×P4，全部采纳**（P3 = 已实证的覆盖盲区「`main` 恒空 ⇒ 向量路漏查 `main` 全仓不可检测」⇒ 补变体 + 3 组变异；P4 = Breaking 段补 `SearchParts.vector_segments`）；评审结论 = 「PR5 范围内无阻塞项、达到可合并形态」，四处取形逐条确认 ✅。详见 **§4.6.5**。**PR5（`S8-05` 跨段向量）已实现（v0.9，2026-09-20）**：逐段检索 + 全局归并 + 逐段谓词下推 + `VectorRoute::Mixed`；门测试 = **`S8-T5`**（`Brute` 下跨段 == 单段逐位一致）。⚠️ **#63 已合并（`b3e1ab6`，2026-09-20）** ⇒ 本 PR 的 base 已由 `feat/v2-step8-delta-segments` **改为 `main`**（同日同步）。**PR #63 第 3 轮评审已响应（v0.8，2026-09-20）**：**复审通过（无阻塞项）** + 唯一新意见 **1×P4 已采纳并顺手修**（`PublishCaller` 让 epoch 拒绝消息按调用方分家；判据 + 变异 + 端到端接线判据齐备）。详见 **§4.9.6**。**PR #63 第 2 轮评审已响应（v0.7，2026-09-19）**：评审落点 = `pulls/63/reviews` 1 条 + 行内 4 条 ⇒ **2×P1 + 1×P3 + 1×P4，全部采纳**（正文另逐条确认 §6 十二项 = **12 项全 ✅**）。**第 2 轮的 2 条 P1 都落在「多写端 / 未发布状态」的缝隙里**（查重不看未发布墓碑 / `absorb_window` 的长度前缀假设被并发 `fold` 破坏）⇒ **两条都已确定性复现 + 修复 + 变异自证**；逐条处置见 **§4.9.6**。**PR #61 第 1 轮评审已响应（v0.4，2026-09-18）**：**4×P4、无阻塞**（+ 行内 4 条）⇒ **3 条采纳 + 1 条不立项（改交行为级替代物）**；**覆盖边界②就地收窄 + 新增空图断言（M4 变异实测）**。逐条处置见 **§4.2.1 末节**。**第 1 轮评审已响应（v0.2，2026-09-17）**：评审 **5×P3 + 7×P4、无阻塞**；**4 条需拍板 + 需求面 1 条全部获同意**（其中 3 条附前置：**P4-7① → D-S8-01** / **P3-4 → D-S8-06** / **P3-5 → D-S8-09**）；**12 条意见全部采纳**（含 1 条结论采纳、量级前提更正）。逐条处置见 **§10**。**待评审**。本文回答 `plan-v2.md` §4.0 的 **H4 / H5 / H6** 三条开工前置，并把 §4 Step 8 的四个任务（**T7-25 / T7-06 / T7-26 / T7-18**）落成 `D-S8-01~12` 决策、`S8-01~09` 任务与 `S8-T1~T16` 测试计划。**风险编号 R49~R54 由本文起分配**（架构 §14 导读钦定：「V2.1 从 R49 起」）。⚠️ **本文纯设计 —— 不落任何生产代码**。 |
...
#### 4.6.4 实施结果（`S8-05`，2026-09-20，**PR5**）

> 落点 = `retriever/vector.rs`（`SegmentedVectorRetriever` / `VectorSegmentRef` / `union_route`）、
> `query/searcher.rs`（`VecLane` 两形态 + `SearchParts.vector_segments`）、
> `search/searcher.rs`（逐段载具 + `ViewPredicate::build_all`）、
> `search/view.rs`（`SegmentPredicate` + `allowed_per_segment`）、
> `query/filter.rs`（`BuiltPredicates` / `PredicateBuilder::build_all`）、`vector/mod.rs`（`Mixed`）。

| 设计条目 | 实现取形 | 偏差 |
| --- | --- | --- |
| 每段一次 + 全局归并 `(距离 asc, 全局 chunk_id asc)` | ✅ `SegmentedVectorRetriever::search_segmented`：逐段 `k = candidate_k` → 本地 id 折全局 → **复用 `to_scored`** → `truncate(k)` | 无 |
| 谓词**逐段下推**（「第 `i` 段的本地谓词」） | ✅ `SegmentPredicate`（段内本地语义 + **段内** `allowed_count`） | 无 |
| `prefers_exact` **逐段判定** | ✅ 编排层逐段 `plan`（各段自己那份谓词） | 无 |
| 过采样 `alive_ratio = allowed / len` **逐段算** | ✅ 由 `SegmentPredicate::allowed_count` 提供**段内**值 ⇒ `HnswRsIndex` 内部公式自动逐段 | 无 |
| `Metrics.vector_route` 并集分类（含 `Mixed`） | ✅ `union_route`（纯函数，公开） | 无 |
| `Metrics.vector_shortfall` 语义 | ⚠️ **公式不变**（`min(candidate_k, allowed) − len`） | ⚠️ **见下** |

⚠️ **`vector_shortfall` 的偏差（如实报评审）**：本节正文写「`vector_route` / `vector_shortfall`
从『一次检索一个值』变成**逐段判定 + 并集上报**」。实现里 **`vector_shortfall` 的公式一字未改** ——
因为它的两个输入（`candidate_k` 与 `allowed`）**本来就是全局量**（`allowed` 是跨段精确和），
而「逐段」影响的是**每段各取多少**（= 全局 `candidate_k`）与**归并后的长度**；后者已由
`len ≤ candidate_k` 与 `min(candidate_k, allowed)` 覆盖 ⇒ 逐段化**不会**改变这个量的语义。
⇒ 处置：**公式不动**，但在本节登记这条偏差（避免后人按「已逐段化」去读它）。
若评审要求逐段形态（例如报 `Σ_seg min(k, allowed_seg) − len`），请指出，我按新口径改。

🔑 **一条实现期的结构性结论（写进 `SegmentedVectorRetriever` 的 rustdoc）**：
`Brute` 下「跨段 == 单段」逐位一致**不是**巧合 —— 全局 top-`k` 的任一成员在**它自己那段**的
排名必然 `< k`（比它更优的同段候选 ≤ 全局更优候选数 `< k`）⇒ 每段取自己的前 `k` 后归并
**无损**；两侧又用同一个 `to_scored` ⇒ 逐位一致是**结构性**的（`S8-T5` 钉住）。

⚠️ **`Mixed` 的端到端覆盖边界**：造「一段 `Exact` + 一段 `Ann`」需要某段 `allowed` 跨过
`BRUTE_FALLBACK_MAX_ALLOWED`（8192）⇒ 端到端要万级语料。分类规则是纯函数 ⇒ 单测锁死；
「逐段 route 真的被收集并归并」由 `S8_T5` 的 `vector_route` 相等断言锁（`Brute` 两端都 `Exact`）。

#### 4.6.5 第 1 轮评审响应（2026-09-20，PR5 = `S8-05`）

> 落点 = `pulls/64/reviews` 1 条（id `5259462645`，SHA `63ad318`）+ 行内 1 条；
> `issues/64/comments` = 0。评审结论 = **「PR5 范围内无阻塞项，达到可合并形态」**。

| # | 级别 | 处置 | 落地 |
| --- | --- | --- | --- |
| 1 | 🟡 P3 | 三条跨段向量用例的夹具 `main` **恒为空** ⇒ 「向量路漏查 `main`」**全仓不可检测**（评审注入实证）⇒ **采纳**：补 `main(2)+delta(2)+delta(2)` 变体 + 抽共用断言助手 | `tests/step8_segments.rs` 的 `t5_main_and_two_deltas` / `S8_T5_main非空与delta并存时跨段仍逐位一致` / `assert_vector_cross_eq_single` |
| 2 | 🔵 P4 | CHANGELOG 的 Breaking 段漏了 `SearchParts.vector_segments`（同族破坏性）⇒ **采纳** | `CHANGELOG.md` 的 `#### ⚠️ Breaking` |

**独立复现（不引用评审的话）**：注入 `if i == 0 { continue; }` ⇒ `step8_segments` **17/17 全绿** ⇒ 属实。
**变异自证（3 组）**：跳过段 0 ⇒ **唯一红 = 新变体**；跳过段 1 / 段 2 ⇒ 两条 `S8_T5` 都红
⇒ 新夹具是**整类**绊线而非 `main` 专用。第 1 组的失败原文还**顺带证明新夹具的 `main` 确实非空**
（变异下跨段侧恰好丢掉 `chunk_id` 0/1，单段参照系仍 6 条）。

**与评审不同之处**：**无**。四处取形（§4.6.1 载具不共用 / §4.6.4 的 `vector_shortfall` 不逐段化 /
§4.6.4 兜底退回单索引 / `Mixed` 无端到端）评审**逐条确认 ✅**，本 PR **一处未改**。

**未闭环盲区（本节就地收窄 + 残留登记）**：

- ✅ 「漏查 `main`」已收窄（新变体 = 整类绊线）；
- ⚠️ **残留**：`S8_05_逐段谓词的语义折算` 的夹具 `main` **仍为空** —— 该用例的鉴别力来自
  「段的 `base_chunk ≠ 0`」（谓词折算 / 段对齐），与 `main` 是否为空**正交** ⇒ 未改
  （评审也只要求 `S8_T5` 补变体）；若将来加非空 `main`，可多锁「`filters[i]` 与 `segments[i]`
  含 `main` 的对齐」，属可选加固；
- ⚠️ **`Mixed` 的端到端判据仍无**（§4.6.4 已登记）：评审**接受**该处置，并留了一句非阻塞建议
  —— 若将来给 `BRUTE_FALLBACK_MAX_ALLOWED` 开 `#[cfg(test)]` 覆盖（或测试装配可注入阈值），
  端到端 `Mixed` 用例会变便宜 ⇒ **记入 `S8-06` 的顺手项**。
...
| 4 | 🔵 P4 | 墓碑-only 空段的次生成本（`metrics.segments` 膨胀 + fold 期白付 O(N)） | ✅ 登记 | `SegmentBuilder::is_empty()` 文档 + `commit()` 注释；**处置归 `S8-06`** ⇒ 🔵 **`S8-06` 已结清**（② 已修 / ① 有意保留并写明理由 / ③ 评审新指出、已修），见 **§4.9.8** |
...
### 4.9.7 `S8-06` 实施结果与 **spike S8-S1** 的结论（2026-09-20，PR6）

#### 落地清单

| 部件 | 落点 | 备注 |
| --- | --- | --- |
| `VectorMergeStrategy`（`None`/`Incremental`/`Rebuild`） | `search/index.rs` | **纯加法**；`None` 是**实现期新增的取值**（原设计只列两个）——「纯 BM25 装配没有向量侧工作」必须能如实表达，否则就是「把无说成有」 |
| `MergeReport`（7 字段） | 同上 | 字段同 §4.9.1 的取形 |
| `merge_pending()` / `merge_all()` | 同上 | 严格 FIFO；`merge_pending` 无段时 `Ok(None)` |
| `merge_n(take)`（原 `fold_deltas`） | 同上 | 把「最小文本侧合并」参数化为「并前 `take` 段」⇒ 两个原语共用一份逻辑 |
| 向量侧方案 A | `merge_vectors_incremental`（私有） | `dump→load→增量 insert`；失败/不支持 ⇒ **回落 B** 并 `tracing::warn!` |
| `save` / `compact` 前置 | 同上 | 改调 `merge_all()`（`D-S8-01` / `D-S8-12`） |

#### 🔴 spike S8-S1 的实测（`crates/core/examples/spike_s8s1.rs`，可复跑）

**环境**：12000（主）+ 1200（增量）点、384 维、100 query、`k=10`、`ef=200`、`--release`。

| 判据 | 实测 | 结论 |
| --- | --- | --- |
| ① 可行性 | load 后点数吻合、insert 后可检索、`len` = 13200 | **PASS** |
| ② 收益 | **A = 639ms**（dump 30 + load 33 + insert 576）vs **B = 4774ms** ⇒ **A/B = 0.134** | **PASS**（阈值 ≤ 0.2，7.5× 裕度） |
| ③a 相对基线 | A vs B = **0.9250** ≥ 基线（**B vs B′ = 0.9280**）− 0.01 | **PASS** |
| ③b 召回 | `recall@10`：A = **0.9170** vs B = **0.9270** | **PASS**（差 1%，与「B 自己重建一次」的抖动同量级） |
| ⑤ 累积 | 连续 10 次 A 的峰值 RSS 增量 = **0 KiB** | **PASS**（`Box::leak` 只漏 `HnswIo` ≈200B/次，淹没在 `ps` 的 4KB 粒度里） |

⇒ **取形 = A（`Incremental`）+ 回落 B**，`MergeReport.vector_strategy` 如实记录。

#### 🔴 判据③ 的阈值**就地更正**（两处，都实测出来的）

1. **原「A vs B 的 top-10 重合率 **≥ 0.99**」结构性不可达**：`hnsw_rs` 用**无种子 `OsRng`**
   分配层级 ⇒ **同一份数据重建两次**的图（B vs B′）只有 **0.9280** 的重合率。
   ⇒ 这不是「A 丢点」，而是**方法固有的拓扑抖动**。判据必须**相对基线**（`A vs B ≥ B vs B′ − 0.01`）。
2. **`A vs A′` 也不是 1.0**（实测 0.994 ~ 0.999，两轮）：A 路径的 `insert` 阶段**同样**走 `OsRng`
   分层，只是只重随机 delta 点（1200/13200）而非全部 ⇒ **不要假设「A 是确定性的」**。
   这一点与 §4.6.2 的「ANN 结果本来就允许不同」一致 —— 但原判据的 0.99 与它自相矛盾。

⚠️ **两条都已按「口径类先独立复算」处理**：先做**对照实验**（B vs B′ 基线）再下结论，
没有停在「0.925 < 0.99 ⇒ 回落 B」这个**首跑**结论上（首跑若照收，会得出与事实相反的取形）。

#### ⚠️ 一条**结构边界**（实现期实测，设计未提）

**主段为空时方案 A 不可用**：首次合并时内容**全在增量段**里 ⇒ 主段图是**空图**，
而 `hnsw_rs` 的 `file_dump` 对空图报 `向量图读写失败: unexpected error`（**实测**）。
⇒ 实现里**显式**判 `main.index.total_chunks() == 0` 走 B（比依赖 dump 报错更清楚，也省一次临时文件写）。
⚠️ 该场景下 B 只需重建增量段那一批点（本就很小）⇒ **不是性能问题**。

#### 与设计正文的**其它**偏差（如实登记）

- **`merge_pending` 的「一次一段」与「双写端」**：§9.2 的 Q1 说「宿主循环调用」，实现按此交付；
  但**「三线程」不可达**（`!Sync`），见 §7 的 `S8-T10` 行。
- **`reclaimed_terms` 口径**（`D-S8-01` 后收窄）：**未在本 PR 处理** —— 它属 `compact` 的回收统计，
  与合并器的 `MergeReport` 是两套口径；留待需要时统一（登记在案，不静默）。
- **`MergeReport` / `VectorMergeStrategy` 都未加 `#[non_exhaustive]`**（与 `VectorRoute` /
  `GraphStatus` 等既有公开枚举一致）；它们是**首次发布的新类型** ⇒ 不构成破坏性变更。

#### 4.9.8 第 1 轮评审响应（2026-09-21，**`S8-06`** = PR #65）

> 落点 = `pulls/65/reviews` **1 条**（id `5262508481`，SHA `66f87fb`）+ **行内 4 条**；
> `issues/65/comments` 无新增（唯一一条是作者自己的交付说明）。
> **1×P1（阻塞）+ 1×P2 + 1×P3 + 4×P4**；评审列出的「本轮无异议」清单（spike 方法论、
> 「三线程 → 两线程」的论证、`VectorMergeStrategy::None`、`S8-T13` battery、门测试 21/5 全绿）
> **逐条签收 ✅**。

| # | 级别 | 落点 | 结论 | 落地 |
| --- | --- | --- | --- | --- |
| P1-1 | 🔴 P1（阻塞） | `search/index.rs` 的 ② 跨段墓碑物理化 | ✅ **采纳**（先复现后修） | `Tombstones::below(upper)` 按**合并区间**过滤；`tombstones_applied` 只计真物理化数；判据 ×2（**含 `save`→`load` 臂**）+ 纯函数单测（钉边界）+ **4 组变异** |
| P2-1 | 🟠 P2 | `merge_vectors_incremental` + `merge_n` 文本侧 | ✅ **采纳 + 扩到文本侧** | 向量侧：零增量走 A 的 `dump→load` **零插入**（记 `Incremental`）；文本侧：零内容段**跳过 `merge_from`**；第 3 条**有意保留**并写明理由（见下） |
| P3-1 | 🟡 P3 | `step8_rw_concurrency.rs` 读端判据 | ✅ **采纳** | 判失败前**宽限重读** `committed`（50ms）；**三臂探针**分别证明「窗口是真的 / 宽限关得掉 / 检测不缩水」 |
| P4-1 | 🔵 P4 | 设计 §7 的 `S8-T14` 行 | ✅ **采纳** | 补回后三格 + 被截断的判据原文（**从 `git show 47f42fd:` 逐字取回**，不是凭记忆重写）；**检查器同步修** + 加两组对照样本 |
| P4-2 | 🔵 P4 | `step8_rw_concurrency.rs` 末尾 | ✅ **采纳** | import 收敛为 `Error`；删除 `let _: Result<()> = Ok(());` 占位 |
| P4-3 | 🔵 P4 | `merge_pending` 文档 | ✅ **采纳** | 补 `Err(Busy)` 的**两种成因 + 恢复动作（重试本次合并）+ 宿主循环形状**（含「`Busy` 重试要有界」的告警） |
| P4-4 | 🔵 P4 | `merge_n` 的注释 | ✅ **采纳** | 删掉**错误的不变式声明**（「FIFO 保证墓碑的目标段必已先被合入」），换成真判据与它的失效条件 |

##### P1-1：先复现、再修（复现读数）

评审给了确定性探针（纯 BM25 / `remove(B ∈ D2)` / `merge_pending()` 只并 D1）。**修前逐字复现**：

| 步骤 | 修前实测 | 应然 | 修后实测 |
| --- | --- | --- | --- |
| `remove(B)` + `commit()` 后检索 B | 0 命中（墓碑生效）✅ | 0 | 0 |
| `merge_pending()`（只并队首 D1） | **1 命中（复活）** + `tombstones_applied = 1`（**谎报**：真物理化 0 条） | 0 / 0 | **0 / 0** |
| 继续并完 D2、D3 | **1 命中**（复活被**固化**进主段） | 0 | **0** |
| `save()` → `load()` | **1 命中**（删除**永久丢失**） | 0 | **0** |

**根因**（评审给的两处叠加，复核属实）：① `SearchIndex::remove` 的**分支②**对**任何既往段**
（含**还没合并**的任意 delta）都记墓碑；② 原 `merge_n` 把**全量** `view.tombstones` 交给
`absorb_window` 摘除，而 `Index::remove` 对**越界 `doc_id` 是静默 no-op** ⇒ 出现
「**被摘掉、却没被物理化**」⇒ 检索期再无东西挡它。

**修法**（判据 = **本次合并覆盖到的 doc 槽位区间**）：`upper` = **物理化前** `merged` 的 doc
槽位上界（FIFO 下等于 `deltas[take].base_doc`；`I8-7` 保证槽位与段的对应唯一）⇒
**只有 `doc < upper`** 才物理化、才从集合里摘；区间外的墓碑**原样保留**。
⇒ 与 **§4.9.6** 的 P1-2（前缀 `Arc::ptr_eq` 身份）**同一思路：只处置本次真正覆盖的那部分**。

**配套判据**（评审明说的两条都补）：①「墓碑指向未合并段 ⇒ 部分合并后**不可见**」
（`S8_06_部分合并不得丢弃指向未合并段的墓碑`，**含 `save`→`load` 臂** —— 钉住「固化」那一层）；
②「`tombstones_applied` == **真**物理化数」（同用例内，独立断言）。另加
`Tombstones::below` 的**纯函数单测**，把边界「**严格小于**」逐值钉死
（写成 `<=` 会在「墓碑目标 = 下一个未合并段的首个 doc」这一格上放行 = 复活）。

##### P2-1：接受「更贵」的纠正，并把「至少登记」升级为「逐条处置」

评审指出设计 v0.6（`S8-03` 评审 **P4-1**）登记的**文本侧两条**本 PR 既未处理也未登记，
且实测出**第三条更贵的**（向量侧全量重建）。三条的处置：

| 次生效应 | 处置 |
| --- | --- |
| ③ 零向量增量段触发向量侧**全量重建**（`O(N·logN)` + `OsRng` 拓扑重随机） | ✅ **已修**：`delta_raw.is_empty()` 不再返回 `Ok(None)` ⇒ 走 A 的 `dump→load` **零插入**，`strategy` 如实记 `Incremental`（修前实测 `Rebuild`）。⚠️ 评审指出的类型边界如实记录：`Segment.vector_index` 是 `Box`（非 `Clone`）⇒「直接复用主图、零 I/O」拿不到所有权，`dump→load` 是当下**最小修法** |
| ② 零内容段白付一次 `append_from` + `field_index.rebuild`（各 O(N)） | ✅ **已修**：`merge_n` 对**两个槽位都为 0** 的段**跳过 `merge_from`**。**等价性**：`other` 槽位全 0 ⇒ `append_from` 无内容、`stats` / `chunk_lens` 各加 0、`content_hashes` 无条目、`base_*` 不变 ⇒ **逐位等价**。⚠️ 判据必须**两个槽位都为 0** —— 只判 chunk 会漏掉「有 doc 槽位但无分片」的段（那种段仍须 append doc 槽位以保基址） |
| ① `Metrics.segments` 被零内容段**膨胀** | 🔵 **有意保留**（**不是**漏做）：入列的**理由**（跨段墓碑只能搭一次发布的便车进 `View.tombstones`）没有替代方案 ——「不入列」就要另开一条墓碑发布路径；且膨胀**有界且自愈**（宿主按 `deltas.len() > K` 合并，合并后段数回落），而经 ② ③ 之后**消化这类段的代价已降到最低** |

##### P3-1：**三臂探针**（不只说「加了重试」）

窗口是**指令级**的 ⇒ 直接在原用例里几乎撞不上。做法：把窗口**人为放大**
（写端在 `commit()` 与 `store` 之间 `sleep(30ms)`）后三臂对跑（本机实测）：

| 臂 | 写端 | 读端判据 | 实测 |
| --- | --- | --- | --- |
| ① | `commit()` 后 `sleep(30ms)` | **旧**（当场判失败） | `rounds=9534` / **违约 3416 次** ⇒ **窗口是真的**（评审的前提成立） |
| ② | 同上 | **新**（宽限 50ms 重读） | `rounds=5793` / **违约 0 次** ⇒ 宽限把它吃干净 |
| ③ | 同上 + **永不 `store`**（真泄漏） | 新 | **违约**（5 轮内命中）⇒ **检测能力没有缩水** |

⚠️ **只测 ② 是不够的**：它只能证明「不误报」，证明不了「不漏报」——**臂 ③ 才是「不缩水」的证据**。

##### 变异自证（5 组；每组都只打红**目标那一条**用例）

| 变异 | 手法 | 命中判据 | 失败原文（行号） |
| --- | --- | --- | --- |
| `m-p1-1-a` | `physicalized` 退回**全量**墓碑（= 修前形态） | `S8_06_部分合并不得丢弃指向未合并段的墓碑` | `step8_segments.rs:1619` `left: 1 / right: 0`（B 复活）—— 其余 22 条全绿 |
| `m-p1-1-b` | 行为不动、只把**计数**改报全量墓碑数 | 同一条用例的**第二条**断言 | `:1624` `left: 1 / right: 0`（谎报）—— 其余 22 条全绿 |
| `m-p2-1-a` | 零增量**退回** `Ok(None)`（= 修前形态） | `S8_06_墓碑only段的合并不触发向量全量重建` | `:1720` `left: Rebuild / right: Incremental` |
| `m-p2-1-b` | 走 A 但**交回一张空图**（策略仍是 `Incremental`） | 同一条用例的**「保拓扑」**断言 | `:1735`（向量检索命中为空）⇒ **证明「保拓扑」那条有独立牙齿**（策略断言抓不到它） |
| `m-below` | `Tombstones::below` 的边界 `<` 改成 `<=` | `S8_06_墓碑可按合并区间切分且边界是严格小于` | `view.rs:1278` `left: 1 / right: 0` —— 其余 9 条单测全绿 |

##### 🔴 顺带：**我自己的 markdown 检查器漏报了 P4-1**

P4-1 的形态是「表格行的**行尾 `|` 与后三格一起丢**」。我的结构检查器 `mdcheck.py` 要求
**同时**以 `|` 开头**和**结尾才算 table 行 ⇒ 这种行被**整个当非表格行丢掉** ⇒
对设计文档报 **0 处**（**漏报**，正是评审这一条能溜过我的自检的原因）。
⇒ 已修判据（以 `|` 开头即算 table 行；缺尾按补齐归一再算单元格），并补**两个对照样本**：
① 「行尾 `|` 与后几格一起丢」必须**红**；② 「**整表**都省略尾 `|`」是**合法**写法、必须**静默**。
修后对设计文档精确报出 `:1318` 这一处。

⚠️ 该判据同时在全仓扫出 `docs/devel/thirdparty.md` **3 处**同类形态（一张 3 列表被紧随其后的
2 列元数据行吞掉）。经「**比内容不比行号**」核对：`HEAD` 与 `origin/main` **都是 3 处** ⇒
**非本 PR 引入**，且**本 PR 一行未动该文件** ⇒ 按纪律**如实登记、不擅自改**，建议作独立的
工程卫生项处理（同 `#59`/`#60` 对「`main` 上既有孤儿」的处置先例）。
...
#### 4.11.4 `S8-08` 实施结果 + **spike S8-S2** 的三条判据结论（2026-09-21，PR7a）

##### 落地清单

| 部件 | 落点 | 备注 |
| --- | --- | --- |
| **会话池** | `crates/core/src/embed/local.rs`：`LocalEmbedder { pool: Vec<Mutex<TextEmbedding>>, next: AtomicUsize, dim }` | `embed_query` / `embed_documents` **轮转取槽**（只锁那一个） |
| `LocalEmbedder::new_with_sessions(n)` | 同上（**新增公开构造**） | `new()` = `new_with_sessions(1)` ⇒ **签名不变** |
| `LocalEmbedder::sessions()` | 同上（**新增公开读数**） | 供装配层与测试观测「规范化真的生效了」 |
| `Config::embed_sessions: usize` | `search/config.rs`（**默认 1**） | ⚠️ **不进 `ConfigFingerprint`**（会话数不改变索引内容 ⇒ 同一快照在 1 / N 下都合法） |
| `SearchIndexBuilder::embed_sessions(n)` | 同上 | 只对**默认装配路径**生效；显式 `embedder(Some(..))` 时忽略 |
| 会话说规范化 `normalize_sessions` | `embed/mod.rs`（**不带 feature gate**） | `0` ⇒ `1`；放在无 gate 处是为了 `--no-default-features` 也能编译（装配层要用它） |
| spike 载体 | **新增** `crates/core/examples/spike_s8s2.rs` + **新增** `scripts/eval_s8s2.sh` | 见「与设计的偏差」第 1 条 |
| 读数 | **新增** `eval-report.md` **§8.15.2** | 本节只放结论摘要，数据源在那边 |

##### spike S8-S2 的结论：**判「不投」**（三条合取不成立）

运行范围：macOS aarch64 / **10 核** / 插电 / 非 CI；短 query 12 条轮转 × `queries = 300`；
冻结图 2000 篇（`Hnsw`）；`sessions ∈ {1,4}` × `threads ∈ {1,4}`，**逐档位独立进程**；
RSS 由**外置** BSD `/usr/bin/time -l` 采（**字节**，脚本统一换算 KiB）。

| # | 判据 | 阈值 | 实测 | 结论 |
| --- | --- | --- | --- | --- |
| ① | 数值一致性（`Q3′` 的前置） | 逐位相同 | 同池轮转 4 槽 ✅；`1 vs 4` 会话（query 侧 **与** 入库侧）✅ | **PASS** |
| ② | 吞吐增益（vector 路 `QPS(4)/QPS(1)`） | **≥ 2.5×** | **2.10×**（492.9 → 1034.1）；对照**未池化 = 1.23×**。⚠️ **映射定义 + 并列的三个读法（2.10× / 2.04× / 1.66×，三者皆 < 2.5× ⇒ 结论对读法稳健）见 `eval-report.md` §8.15.3** | ❌ **FAIL** |
| ③ | 峰值 RSS 增量（查询侧口径） | **≤ 20%** | **+148.4%**（201 104 → 499 600 KiB） | ❌ **FAIL** |

⇒ **判「不投」**：`NFR-10` ③ 与 `R43` **保持「打开」**（如实记未达标，**不是**「只记录不判定」）；
**四处定义面回写归 `S8-09`（PR7b）** —— 本 PR 只把结论与读数落到设计 + `eval-report.md`。

🔑 **三条附加观察（读数之外的机理，标注证据类型）**：

1. **判据① 比预期更强**：①c 的**非生产路径**对照显示 **`intra_threads` 也**不**改变数值**
   （`10` vs `2` 的 `embed` 结果逐位相同）⇒ `Q3′` 的**两个子问题都有答案**（实例数不变、
   `intra_threads` 不变）⇒ 池内**可以**按需分化 `intra_threads`（本实现仍取「槽同构」的保守取形）。
   ⚠️ **样本边界**：①c 是**单观测**（一对取值 × 12 条短文本 × 1 次），不可外推成「任意组合都一致」。
2. ⚠️ **判据③ 的阈值在本口径下对任何 `sessions > 1` **结构性不可达****（**实测支撑的算术**）：
   4 会话比 1 会话多 `298 496 KiB / 3 槽 ≈ 97 MiB / 槽`（每槽是**独立 ONNX 会话**、各持一份权重）
   ⇒ **只加到 2 个会话**也约 **+50%，已越过 20% 上限**。
   ⇒ 这不是「池化没做好」，而是**阈值取自建库口径（`D-S6-01`）与查询侧口径不匹配**：
   查询侧基线只有 201 MiB（**其中约一半就是那一个会话的权重**），而每槽成本是**固定量级**
   ⇒ **相对阈值 + 小基数 = 结构性不可达**（同族：`#60` 评审 P3-5 的「相对阈值 + 小量级 = 空转门」）。
   ⚠️ **本轮不擅自改阈值**（口径类裁定归 `S8-09` / 评审），只给算术。
3. ⚠️ **判据②「未过」的机理尚未单独证伪**（**推断，非实测**）：最可疑处 = 每槽 `intra_threads`
   沿用 fastembed 默认（`None` ⇒ **吃满所有核**）⇒ 4 槽 × 满核线程在 10 核上**过订阅**
   （Step 6 的 E2 正是按 `ceil(核数 / sessions)` 切分的）。要给这条定罪**必须让每槽可设
   `intra_threads`**，而生产 `LocalEmbedder` **当前不暴露该参数**（§4.11.1 只规定「槽同构」）
   ⇒ **本轮不做**，登记进**复审触发条件**。
   ⚠️ 因此 ② 的 FAIL **不能**读成「池化无效」：`1.23× → 2.10×` 的方向与幅度都说明**确有增益**，
   只是没到 2.5×。
...
| # | 设计原文 | 实际 | 理由 |
| --- | --- | --- | --- |
| 1 | `S8-T16` 落点 = 「`bench_embed_session.rs` **扩展** + 脚本」 | **新建** `examples/spike_s8s2.rs` + **新建** `scripts/eval_s8s2.sh` | **两者的协议与口径不同**：E1/E2/E3 测**建库侧**（batch 64 × 长文本、按波交错、多档位同进程风险）；S8-S2 测**查询侧**（batch 1 × 短 query、同进程内 1 vs N 槽**逐位**比较、四臂读**同一张冻结图**）。混进一个文件会让两套协议互相污染 ⇒ 按 `spike_s8s1.rs` 的先例**另立文件** |
| 2 | （未提）RSS 单位 | 脚本统一把 BSD `time -l` 的**字节**换算成 KiB | ⚠️ **实测踩到**：BSD 是字节、GNU `gtime -v` 是 KiB ⇒ 当同一量用会让「比值」判据照样成立、**绝对值差 1024 倍** ⇒ 报告不可读 |
| 3 | （未提）CLI 开关 | **不加** `--embed-sessions` | `Q4` 只说「库侧默认不动」；CLI 侧的**口径与推荐值**待 spike 结论 ⇒ 本轮结论是**「不投」**，不加开关更自洽。⚠️ **库侧入口已可用**（`SearchIndexBuilder::embed_sessions(n)` 与 `LocalEmbedder::new_with_sessions(n)` 两条公开路径） |
| 4 | （未提）**PR7 拆分为两个 PR** | **用户 2026-09-21 拍板**：`PR7a` = `S8-08` + spike S8-S2 + `S8-T16`（含生产代码）；`PR7b` = `S8-09` 标定收尾（新建 `scripts/eval_rw_concurrency.sh` + `S8-T11`/`T12`/`T15` + **四处定义面回写**） | 评审面更清晰；且 `S8-08` 与 PR4~6 正交（§8 原文已注），拆分不产生依赖倒置 |

##### 变异自证（4 组；每组**只**打红目标判据）

| 变异 | 手法 | 命中判据 | 结果 |
| --- | --- | --- | --- |
| `m-s8s08-a` | `slot_index` **恒返回 0**（= 池化名存实亡） | `S8_08_轮转取槽把并发摊到不同槽且单槽时恒为0` | ✅ 红（其余 11 条绿） |
| `m-s8s08-b` | `normalize_sessions` 退化为**恒等**（`0` 透传） | `S8_08_builder的会话数默认一且零归一为一` + 接线判据的 ③ | ✅ **2 条**红 |
| `m-s8s08-c` | `default_embedder(cfg.embed_sessions, ctor)` → `(1, ctor)`（**装配链路吞掉会话数**） | `S8_08_会话数必须真的到达构造器且默认为一` | ✅ 红 |
| `m-s8s08-d` | 指纹的 `dim` 里**混入** `embed_sessions` | `S8_08_会话数不进配置指纹` | ✅ 红 |

🔴 **`m-s8s08-c` 首轮是「零命中」—— 这是本轮最值钱的一条留痕**：第一版判据**直接调**
`resolve_embedder(false, recording_ctor, n)`，它只能证明「形参往下转交」，**绕过了**
`build_config → default_embedder(cfg.embed_sessions, ..)` 那一段
⇒ 会话数被写死成 `1` 时**全部单测照样绿**。
**修法** = 把**构造器**变成新入口 `build_config_with(ctor)` 的形参（生产入口 `build_config`
注入真实构造器），判据改从**装配那一层**进 ⇒ 同一条变异当场变红。

⚠️ **为什么这条特别值得记**：漏传的后果是**静默退回单槽**，而「单槽」**就是默认的合法配置**
⇒ **行为判据（「结果对不对」）永远看不见它**；只有「断言构造器**实收了什么**」才看得见。

#### 4.11.5 第 1 轮评审响应（2026-09-22）

**评审结论**：**0×P1/P2 + 1×P3 + 3×P4 ⇒ PR7a 范围内无阻塞项、达到可合并形态**
（`pulls/66/reviews` `5273480265`，`COMMENTED`，基线 `45d2566`；另有**行内 3 条**）。
评审的「独立复核清单」逐条签收 ✅（CI 10/10、无模型单测 5/5、判据① 真模型独立复跑、
`m-s8s08-a` 变异抽查、非破坏性变更核实、判据② / ③ 的量级复核、脚本工程质量）。

| # | 评审意见（摘） | 处置 | 落地 |
| --- | --- | --- | --- |
| 🟡 P3-1 | 公开面 rustdoc 仍是「由 spike 决定」的**将来时** ⇒ 决策面与代码面脱钩 | ✅ **本轮改**（不留到 PR7b）：「由…决定」⇒ **已判「不投」** + 「**当前没有 >1 的推荐值**」+ 指向本节与 `eval-report.md` §8.15 | `search/config.rs` **4 处** + `embed/local.rs` **3 处**（见下「载体范围」①） |
| 🔵 P4-1 | `slot_index` 注释与代码不符（把「先加后取模」写反、且说「回绕到**负**」） | ✅ 注释改为「**无界递增、靠 `usize` 回绕**；收敛的是**返回值**不是计数器本身；回绕点最多一次调度「不轮转」；`usize` 无负值」 | `embed/local.rs` 的 `slot_index` |
| 🔵 P4-2 | `QPS(4)/QPS(1)` 在双因子下有**三个**合理读法，报告只报 2.10× | ✅ **钉死映射定义**（= `NFR-10` ② 字面口径）+ **并列三个读数**（2.10× / 2.04× / 1.66×）+「**三者皆 < 2.5× ⇒ 结论对读法稳健**」 | `eval-report.md` §8.15.3（+ 本节上表 ② 加指针） |
| 🔵 P4-3 | `new_with_sessions` 无上界守卫 | ✅ **不加 cap**（`batch_size` 同类：信任调用方）⇒ 改为**写清失败模式**；⚠️ **并更正评审的一处前提**（见下「与评审不同」②） | `embed/local.rs` 的 `new_with_sessions` rustdoc |
| ⚪ 不立项 | 既有 `默认配置零配置可用` 仍走 `build_config()` 联网 | ✅ **同意归 PR7b**；本轮把**载体范围**查清并登记（见下「载体范围」②）+ **更正我自己上一轮的一句登记**（见下「与评审不同」③） | 本节 + `CHANGELOG` + PR 正文 |

##### 载体范围（一类缺陷全扫的结果）

① 🔑 **P3-1 的载体 = 7 处，不是评审点到的 2 处**（`config.rs` **4** + `local.rs` **3**）。
多出来的 5 处：`Default` 的注释、`Config::embed_sessions` 字段文档、`required_local_embedder`
的 CLI 注释、类型文档的「内存门槛由 S8-S2 实测」与「`intra_threads` 是否影响数值由 S8-S2 实测」
—— 后两处**写在结论之前是将来时，结论出了就成了错话**（已一并回填读数）。

② 🔑 **「联网」的载体范围比评审的措辞更窄、也更危险**：全仓 `build_config()` 调用点 **12 处**
（生产 **3**：`SearchIndexBuilder::build()` / `::load()` / `SearchIndex::load()`；测试 **9**），
其中**会触默认构造器（因而读模型 / 联网）的只有 3 处**，全在 `search/config.rs` 的
**2 个用例**里（`默认配置零配置可用`、`并行建图默认开且可显式关闭`）；
其余 6 处测试调用点**都显式给了 `embedder(None)` 或 `embedder(Some(..))`** ⇒ 不触构造器。
⚠️ 而且它的危害**不是「慢」而是「冷缓存下并发时会失败」**（实测见「与评审不同」③）。

##### 与评审不同 / 需要更正的四处

① **「判据③ 的门是坏的」我同意，但处置归 PR7b**（评审也这么定）—— 本节只给算术，不擅改阈值。

② 🔴 **更正评审 P4-3 的一处前提**：评审写「`embed_sessions(usize::MAX)` 会 `Vec::with_capacity`
**直接 abort**」。**实测（`rustc` 直编复刻、同类型宽度）**：`Vec::<u8>::with_capacity(usize::MAX)`
**是 panic（`capacity overflow`）不是 abort** —— `catch_unwind` **能**捕获（实测 `is_err() == true`）；
真正的阈值是 `N × size_of::<Mutex<TextEmbedding>>() > isize::MAX`（**本机构建**实测该尺寸 = **1232 字节**
⇒ `N > 7.49e15`），**远在实用范围之外**；**先撑爆的是内存**（≈ 97 MiB / 槽）。
⇒ 建议本身**采纳**（文档注一句），但**失败模式必须写准**：`abort` 会被读成「不可恢复的运行时中止」，
实际是**可捕获的 panic** 且更早的失败其实是**内存耗尽**。

③ 🔴 **更正我自己上一轮的登记**（`CHANGELOG` 与 PR 正文都写过）：「无缓存环境下花 **~85 s**
**尝试取模型并失败**」—— **两处都不准**：

- **实测（全新 `HOME`、串行）= 取模型成功**：下载 **96 MB / 54 s**、**无失败告警**（`--nocapture` 零告警行）；
- **失败只出现在「冷缓存 + 并发」**：同机同二进制、**默认线程数** + 全新 `HOME` 复跑 ⇒
  `[警告] 本地 embedder 初始化失败（原因: 嵌入模型错误: Failed to retrieve model file 'onnx/model.onnx'）`
  —— 这**正好解释** CI 首轮那次红灯（同一用例两次构造「一失败一成功」）：
  CI **每次都从冷缓存开始**，而 runner 默认并行跑测试。
- ⇒ 正确措辞是「冷缓存下取模型**很慢且非确定**」，**不是**「取不到模型」；`~85 s` 是**多用例合计**
  的模糊数（单用例冷缓存实测 **54 s**）。
- ⚠️ 附带一条我自己差点采信的**假读数**：`/tmp/nohome` 会被上一次跑**写满** ⇒ 二次测量得到的是
  「有缓存」读数（**0.87 s**），差点据此写下「不慢」——**每次必须 `rm -rf` 新建 HOME**
  （同族纪律：验证夹具必须每次重建）。
- ⚠️ **同源风险登记**：`#[ignore]` 的真模型判据（`S8_T16_*`）在**冷缓存 + 默认线程数**下同样可能
  撞上这条 ⇒ 冷机首次跑建议 `--test-threads=1`（或先预热缓存）。

④ ✅ **一处比评审更靠前**：P3-1 评审说「若归 PR7b，至少本轮把『由…决定』改成『已判不投』」
—— 我把**全部 7 处载体**一次改完（含他没点到的 5 处），**不留任何将来时**。

##### 本轮**无断言可变异**（明说「不适用」）

P3-1 / P4-1 / P4-2 / P4-3 **全部是文档与注释** ⇒ **没有可被变异的断言**。
替代验证：① **事实复核** —— 逐处回读改后文本（**正向断言 8 条**）+ 反向扫旧措辞
（「取模前先把计数器收敛」「是否给推荐值由 spike S8-S2 的三条判据决定」等 **4 条须 0 次**）；
② **P4-2 的三个读数是复算出来的** —— 从 §8.15.2 的八臂原表按三种分母各算一次
（`2.098 / 2.038 / 1.662` ⇒ 2.10 / 2.04 / 1.66）。

（同族：`S8-03` 的「按调用方分家只写『两条文案不同』的判据 ⇒ 抓不到接线错」。）

⚠️ **覆盖边界（本 PR 不声称守住的）**：
- **判据②的量级依赖核数**（10 核上的过订阅程度与其它核数不同）⇒ 换机需重测；
- **未测 Linux / CI**；**未测建库侧口径**（本节 RSS 刻意是查询侧）；
- **①c 是单观测**（见上）；
- **`LocalEmbedder` 不给 `intra_threads`** ⇒ 判据②的机理方向（过订阅）**本轮无法单独证伪**。
...
| **② 读延迟** | **②a 相对判据（主判据）**：同机、同 query 集、**同一已提交内容**下，「**合并进行中**」的 `took` P99 ≤ 「**静止期**」的 `took` P99 × **1.2 ✅ 定稿**（2026-09-22，`S8-09`）。🔴 **基线定义必须写死**：「静止期」= **A1 = 合并「起始布局」的静止期**（与 B 的起始布局逐位相同）；⚠️ **不得**改用「合并**完成后**的静止期」（A2）—— 那个布局已被合并改变（17 段 → 1 段、墓碑清零）⇒ 会得到 1.07 / 1.26 / **1.41**（三次运行）的**假 FAIL**（实测见 `eval-report.md` §8.15.6）。**②b 绝对判据（护栏）**：合并期 `took` P99 **仍在 NFR-02 该模式的预算内** —— ✅ **定稿为「数量级护栏」**（⚠️ 本档语料规模与 NFR-02 原协议不同 ⇒ 只作量级比对） | S8-T11 **已落地**：`scripts/eval_rw_concurrency.sh` + `examples/eval_rw_concurrency.rs`；读数见 `eval-report.md` **§8.15.6** |
...
🔴 **两条保留（`S8-09` 标定后如实登记，`NFR-14` 因此整体保持「打开」）**：

1. **样本数不可比**：A 臂固定 400 条（⇒ 其「P99」≈ 第 4 大样本）、B 臂 3.7k ~ 12.7k 条（P99 落在尾部更深处）⇒ **比值对 `n` 敏感**。**均值口径**（对 `n` 不敏感）一并记为佐证：bm25 **+8.9%** / hybrid **−10.8%**（`eval-report.md` §8.15.6）—— 与 P99 口径同向。
   🔴 **`#67` 第 1 轮评审 P4-1 后更正（2026-09-22）**：该「同向」在 **hybrid** 上**只是数值同向、机理不同向** —— 那 −10.8% **主要是 `B1` 窗口内「布局渐进改善」（17 段 → 1 段）的贡献**，而不是「合并让查询变快」；换上**窗口早期切片**（前 25%）后 hybrid 的 mean 变成 **−0.4%**（r4）⇒ 均值口径的**净「干扰」读数接近零**（bm25 侧则是真的 **+6.1%**）。⚠️ **主判据（②a 的 P99，基线 = A1）不受该效应影响**（整窗 vs 切片：bm25 **1.0604 / 1.0478**、hybrid **0.9597 / 0.9698**），但**均值的强度曾被高估**；原句保留在此供对照（`eval-report.md` §8.15.6 有完整对照表）。
2. **尾部扰动：可复现「同向」、不可复现「幅度」**：B 的 `max` 三次为 **8.5 / 40.8 / 23.5 ms**（A1 为 **15.1 / 4.1 / 3.5 ms**，其中 **r1 反向**）；同深度口径（第 4 大）一次达 **2.74×**。⇒ **本轮不把它当判据**（幅度被本机噪声支配），但**如实登记**并进**复审触发条件**。

**复审触发条件（`NFR-14` ②）**：① 出现比本机更稳的测量环境（或改用**交错执行**）时，用**等样本数** + **同深度尾部**口径重测；② 宿主真的把合并放到后台线程（`Q1` 的 CLI 侧）后复测「合并期 vs 静止期」；③ 向量侧合并路径改动（`S8-S1` 的方案 A 变动）时复测。
...
| **定稿（2026-09-22，`S8-09`）** | ✅ **去「拟」定稿**：`P50 ≤ 1.0 s / P99 ≤ 2.0 s` 保持原值。实测（`S8-T15`，`eval-report.md` §8.15.8）：**P50 = 0.694 s**（余量 **31%**）/ **P99 = 1.026 s**（余量 **49%**）。**口径边界（必须与阈值一起引用）**：`batch_size = 64`、**n = 40 批**（⚠️ `n = 40` ⇒ 「P99」实际是**最大值**，偏保守）、语料 **2560 篇**（`data/t2-corpus.jsonl`；拟值的原文口径写的是「T2Ranking **12K**」）、CPU / FP32、macOS aarch64 / 10 核、不含模型加载。**复审触发条件**：若要在 **12K 全量**口径（或 Linux / CI）上复核 ⇒ 按同协议重跑 `scripts/eval_rw_concurrency.sh`（`BATCHES` 可调） |
...
#### 4.13.4 `S8-09` 标定结果与裁定（2026-09-22，PR7b）

运行范围：macOS aarch64 / **10 核** / 插电 / **非 CI** / `--release`；
工具 = **新增** `crates/core/examples/eval_rw_concurrency.rs` + **新增** `scripts/eval_rw_concurrency.sh`；
读数落 **`eval-report.md` §8.15.6 ~ §8.15.8**。

| 门 | 判据 | 实测 | 裁定 |
| --- | --- | --- | --- |
| **`S8-T11` / NFR-14 ②a** | 合并期 ÷ 静止期 `took` P99 ≤ **1.2**（基线 = **A1**） | bm25 **1.107 / 1.049 / 1.063**；hybrid **0.992 / 0.927 / 0.993**（三次运行） | ✅ **定稿**（去「拟」），**基线定义写死** |
| **`S8-T11` / NFR-14 ②b** | 合并期 P99 仍在 NFR-02 该模式预算内 | bm25 **3.69 ms ≤ 5 ms**；hybrid **7.54 ms ≤ 20 ms** | ✅ **定稿为「数量级护栏」**（语料规模与原协议不同） |
| **`S8-T12` / R52** | **读数**（不是门槛）：第 4 档回到第 1 档量级 | 四档 lane p50 = **44 / 52 / 53 / 50 us**；档3−档2 = **+1 us**、档2−档1 = **+8 us**；档4 `segments = 1`、墓碑 0 | ✅ 已量化；⚠️ **档 4 ≠ 档 1**（档 4 的语料多 delta 那 400 篇） |
| **`S8-T15` / NFR-11** | `P50 ≤ 1.0 s / P99 ≤ 2.0 s` | **0.694 s / 1.026 s**（n = 40、2560 篇、`batch_size = 64`） | ✅ **定稿**（去「拟」）+ 写明口径边界 |

🔴 **`NFR-14` 整体保持「打开」**（三条并记，缺一会被打回）：① **②a / ②b 的判据口径已定稿**（去「拟」）；② **另立一条如实记未达标**：见 §4.13.1 的两条保留（`n` 不可比 + 尾部扰动不可复现）—— 其中**同深度尾部口径下一次测得 2.74×**；③ **整体保持「打开」+ 写明复审触发条件**（§4.13.1 末的三条）。

⚠️ **本轮「零生产代码改动」**：改动面 = 文档四处 + 本设计 + 新增 demo/脚本 + `CHANGELOG`（`git status --porcelain | grep -c '\.rs$'` = **0**；两个新增 `.rs` 在 `examples/` 下，不进 `src/`、不进 public API）。
...
#### 4.13.5 第 1 轮评审响应（v0.16，2026-09-22）

评审（`pulls/67/reviews` 1 条 + **行内 3 条**）= **0×P1/P2 + 1×P3 + 3×P4**，结论「**无阻塞项**」。
🔑 评审**独立复跑了整个 `scripts/eval_rw_concurrency.sh`**（脚本**默认**参数）并逐项复现了读数：
②a bm25 **1.027** / hybrid **0.836**、**`B1/A2` = 1.4064**、②b **3.82 / 5.92 ms**、
四档 **22/24/26/23 us**、`NFR-11` **P50 0.707 / P99 1.040 s** —— 其中「**换基线得 ~1.41 假 FAIL**」
这条从「理论上对」升级为**经验上必要**。

| # | 位置 | 处置 | 落地位置 |
| --- | --- | --- | --- |
| 🟡 **P3-1** | 「四处定义面回写」缺 **`NFR-10` ③ 的判后记录**（三个载体） | ✅ **本轮补齐**（Step 8 不会再有文档 PR）：需求 `NFR-10` 行 / 架构 **`R43`** 行 / `plan-v2` 门槛行 **④** 各补一句判后记录 | `requirements-spec.md` 的 `NFR-10` 行；`architecture-design.md` §14.4 **`R43`**；`plan-v2.md` §6 门槛行 ④ |
| 🔵 **P4-1** | `B1` 窗口内「布局渐进改善」混进统计口径 | ✅ **① 写明 + ② 加对照列**：载体新增**窗口早期切片**（前 25%）；`eval-report.md` §8.15.6 加对照表并**量化**该效应；**就地更正** §4.13.1 保留 1 的「同向」措辞（原句保留） | `examples/eval_rw_concurrency.rs`（`B1_early_quarter` 臂 + `p99_b1early_over_a1`）；`scripts/eval_rw_concurrency.sh`（合取表新行）；`eval-report.md` §8.15.6 |
| 🔵 **P4-2** | 脚本把 `S8-T12` 的 `档4/档1` 印成 ✅/❌（与「读数不是门槛」打架） | ✅ 去掉 `mark()` + 标注口径；**顺带**更正脚本头部同族的「第 4 档**必须**回到第 1 档量级（取 ≤ 1.2× 作可判定的读数口径）」（旧措辞保留供对照）；并把 R52 的两项成本**分列**（与 §8.15.7 的两条读法对齐） | `scripts/eval_rw_concurrency.sh`（头部注释 + 合取表 ①） |
| 🔵 **P4-3** | §8.15.6 / §8.15.7 缺精确复现命令 | ✅ 两节各补一行**确切命令**（`MAIN=4000 DELTA=400 DELTAS=16 Q=400 TOMB=40`）+ 标注「脚本默认是 2000/200/8/20/300 ⇒ 按默认复跑得到**另一组绝对数**（比值与门槛结论仍复现）」 | `eval-report.md` §8.15.6 / §8.15.7 |

**与评审不同 / 需补充说明的两处**：

1. **P4-1 的效应方向是「按模式分档」的，不是全局常量**（评审只在 hybrid 上观察到 −11.6%）：
   本轮实测 hybrid **整窗 mean −12.2% vs 早期切片 −0.4%**（效应真且大）；而 **bm25 侧可忽略**
   （`A2` mean 只比 `A1` 低 **1.6%**）⇒ 整窗 **+7.2%** 与切片 **+6.1%** 同向同量级，
   即 bm25 的「干扰」是真的。⇒ 结论：**该混合效应不构成对 bm25 判据的质疑，但确实污染了 hybrid 的均值佐证**。
2. **P4-2 的范围比评审点的那一行大**：同一「读数被当门槛」的表述在**脚本头部**还出现过一次
   ⇒ 按「一类缺陷全扫」一并更正（本仓纪律：一处形态 ⇒ 扫同一事实的全部载体）。

**本轮没有断言可变异**（4 条全是文档 / 注释 / 脚本展示层）。⚠️ 但新输出字段有**解析级自证**：
合取表在缺 `p99_b1early_over_a1` 时打「⚠️ 读数缺失」而**不是静默跳过** ⇒ 字段一旦漂移就当场可见。

**版本面**：本轮**不升**需求 / 架构 / `plan-v2` 的版本号 —— 它们当轮的 **v1.22 / v1.21 / v0.22**
就是「Step 8 收尾」这一件工作（PR 尚未合并）⇒ 只**就地扩写**那三行版本描述；设计文档按体例升
**v0.16** 并新增本节。

**未闭环 / 盲区**：① 早期切片**不是判据**（切片 `n` 更小 ⇒ 分位数更不稳）；② 「同深度尾部」
（第 `k` 大样本对齐）仍是**未决项**（尾部幅度不可复现，见 §4.13.1 保留 2）；③ 本轮**未**在
Linux / CI / 其它核数上复跑（评审的复跑与本轮读数**同机**）。
...
| `VectorRoute::Mixed` | **新增枚举变体** | ✅ **已落地（2026-09-20，PR5）**：Q5 已结案（`VectorRoute` **无** `#[non_exhaustive]`）⇒ **两条路都 breaking、没有「零破坏」选项** ⇒ 取形 = **新增 `Mixed` 变体**，并在 PR5 的 CHANGELOG 记 `Breaking` |
...
| **S8-T2** | **覆盖等价「每点恰一次」**：**经生产路径** `search_exact_filtered` 的 ID 集 == 入库 ID 全集（2026-09-18 第 1 轮评审后：`get_layer_iterator(0)` 的基数 `<` `get_nb_point()` 一条**已删** —— 由 `upper > 0` + 并集等式**蕴含**、**永不报红**） | CI | `crates/core/src/vector/hnsw_rs_index.rs` 单测（**扩展既有用例**） （实现落点 = `hnsw_rs_index.rs` 单测 **`全量遍历基数等于点数且零重复`**，2026-09-22 台账审计确认同名不存在但语义已覆盖） | 红 = 遍历写回单层（3.7% 静默错答） |
| **S8-T3** | **定向用例**：`level ≥ 1` 的点自查询排第一 | CI | 既有用例（确认仍绿） （实现落点 = 既有用例 **`精确扫描覆盖高层点_定向用例`**） | 红 = 高层点被漏 |
...
| **S8-T5** | **Brute 后端的 `vector` 模式跨段逐位一致** | ✅ **已落地（2026-09-20，PR5）**：`step8_segments.rs` 的 `S8_T5_Brute后端vector模式跨段逐位一致`（另两条同批：逐段谓词折算 / `union_route` 分类） | 同上 | 红 = 精确路径的全局归并写错（`(距离, chunk_id)` 全序）**或**向量路只覆盖了一部分段 |
...
| **S8-T7** | **可见性双向**：`add` 后未 `commit` ⇒ 查不到；`commit` 后 ⇒ 立即查到 | CI | 同上 （实现落点 = `tests/step8_segments.rs` 的 **`S8_03_未commit不可见_commit后立即可见`**） | 红 = 可见性边界错（NFR-14 ①） |
...
| **S8-T10** | ✅ **已落地（2026-09-20，PR6）** —— ⚠️ **取形与本文不同**：`D-S8-05` 让 `SearchIndex` 保持 **`!Sync`**，而 `merge_pending()` 也要 `&mut self` ⇒「写 + 合并」**必须共用一个写端**、「三线程同跑」在安全 Rust 下**不可达** ⇒ 实为「**写+合并**」一个线程 + 「读」一个线程（被测语义不变）。落点 = `step8_rw_concurrency.rs` 的 `S8_T10_写与合并与读的并发探针` + `S8_T10_双写端交替提交不得让段ID空间重叠`。原文：**三线程并发探针**（🔴 **2026-09-17 按评审 P3-4 加「双写端」臂**：由 `into_index()` 再造一个写端、两写端各自持续 `add`/`commit`，断言**段 ID 空间无重叠**（`base` 严格递增、全局 `chunk_id` 无重复）—— 这是 §4.4.3 那条锁的鉴别力用例）：写线程持续 `add`/`commit`、合并线程持续 `merge_pending`、读线程持续检索 ⇒ **零 `Err`、零超时、结果只反映已提交前缀**；并断言 `Error` 枚举**无**「重建索引中」类变体 | CI（规模小） | 同上 | 红 = 读端被阻塞 / 看到未提交内容 / 出现新错误码 |
| **S8-T11** | ✅ **已落地（2026-09-22，PR7b）**：判据 ②a 实测 = bm25 **1.107 / 1.049 / 1.063**、hybrid **0.992 / 0.927 / 0.993**（**四次**运行，基线 = **A1**；r1~r3 = `1.107 / 1.049 / 1.063` + **r4 = `1.0604`**）⇒ **定稿去「拟」**（基线定义写死，见 §4.13.1）；读数落 `eval-report.md` **§8.15.6**（⚠️ **r4 起**另有**窗口早期切片**对照，见 §4.13.5 的 P4-1）。原文：**合并期读延迟**（同机、同**图**、同 query 集 —— ⚠️ **2026-09-17 按评审 P4-3 更正**：原写「同 fig」）：②a 比值 ≤ 1.2（拟）与 ②b 绝对护栏| 脚本 | `scripts/eval_rw_concurrency.sh` | 红 = 写锁被用在重活上（I8-1 被违反） |
| **S8-T12** | ✅ **已落地（2026-09-22，PR7b）**：四档 lane p50 = **44 / 52 / 53 / 50 us**；**R52 的谓词成本 = +1 us（档3 − 档2；r4 = +0 us，同量级 ⇒ 该成本在分辨率边缘）**、多段扫描 = **+8 us（档2 − 档1）**；读数落 `eval-report.md` **§8.15.7**。🔴 **本项是「读数」不是「门槛」**（`#67` 第 1 轮评审 **P4-2**）：档 4 的语料 ≠ 档 1 ⇒ 脚本已**去掉**`档4/档1 ≤ 1.2` 的 ✅/❌（该比只能在「回到 1 段的形态」这个意义上读）。原文：**热路径回归对照**：无 delta / 有 delta 无墓碑 / 有跨段墓碑 / **有跨段墓碑但已 `merge_all()`** **四档**的 `bm25` lane 耗时（第 4 档由 P3-1 的「合并后回到 `None`」推出 —— 2026-09-17 新增）| 脚本 | 同上 | 量化 R52（不是门槛，是读数）；第 4 档必须回到第 1 档量级 |
| **S8-T13** | ✅ **已落地（2026-09-20，PR6）**：`S8_T13_合并后的字段索引与单段全量重建等价`（200 篇 / 分 4 段 / 8 条过滤 battery）。原文：**合并器等价的字段索引**：合并后的 `field_index` 求值 == 全量重建的求值（过滤 battery） | CI | `crates/core/tests/step8_segments.rs` | 红 = `FieldIndex::rebuild` 与增量维护不等价（§4.9.2） |
| **S8-T14** | ✅ **已落地（2026-09-20，PR6）**，但 **判据口径已就地更正**（见 §4.9.7）：原「与全量重建图的 top-10 重合率 **≥ 0.99**」**结构性不可达**（spike 实测：同数据重建两次只有 **0.9280**）⇒ 改为**以 `Brute`（精确）为参照的 recall@10 ≥ 0.9**（不受拓扑抖动污染，直接回答「丢没丢点」）。落点 = `S8_T14_合并后的向量侧不丢点_以精确后端为参照`。原文：**合并器向量侧等价**：合并后的图在固定 query 集上与全量重建图的 top-10 重合率 ≥ 0.99 | CI（小规模、Brute/Hnsw 各一） | 同上 | 红 = 向量合并丢点 |
| **S8-T15** | ✅ **已落地（2026-09-22，PR7b）**：**P50 = 0.694 s / P99 = 1.026 s**（n = 40、2560 篇、`batch_size = 64`）⇒ **NFR-11 去「拟」定稿**；读数落 `eval-report.md` **§8.15.8**。原文：**NFR-11 写延迟直接计时**| 脚本 | `scripts/eval_rw_concurrency.sh` | 产出 P50/P99 读数（D-S8-10 的定稿依据） |
| **S8-T16** | ✅ **已落地（2026-09-21，PR7a）**：判据① PASS（含 `intra_threads` 的**非生产路径**对照）、② 2.10×（< 2.5）、③ +148.4%（> 20%）⇒ **合取不成立 ⇒ 判「不投」**。⚠️ **落点与原文不同**：**新建** `crates/core/examples/spike_s8s2.rs` + 新建 `scripts/eval_s8s2.sh`（**不是**「`bench_embed_session.rs` 扩展」—— 两套协议的口径不同，理由见 §4.11.4 的偏差表 #1）；读数落 **`eval-report.md` §8.15**。原文：**Q3′ 数值一致性 + 会话池内存**（spike S8-S2 的产物） | 本地 | `crates/core/examples/bench_embed_session.rs` 扩展 + 脚本 | 红 = 池化改变向量数值 ⇒ **判「不投」** |
...
| **PR7** | `feat(embed)+docs: 查询侧会话池（T7-26）+ NFR-14/NFR-11 定稿（S8-08 / S8-09）` | S8-08 + S8-09 | ⚠️ **S8-08 可提前到 PR3 之前**（与 PR4~6 完全正交）。🔴 **2026-09-21 用户拍板：拆为两个 PR** —— **`PR7a` = `#66`**（`S8-08` 会话池 + spike S8-S2 + `S8-T16`，含生产代码，本节 §4.11.4；**已合并** `647e75b`）；**`PR7b` = `#67`**（`S8-09` 标定收尾：新建 `examples/eval_rw_concurrency.rs` + `scripts/eval_rw_concurrency.sh` + `S8-T11`/`T12`/`T15` + `NFR-14`/`NFR-11` 定稿 + 四处定义面回写，见 §4.13.4；⚠️ **落点比原文多一个 example** —— 「同进程内让合并与查询重叠」无法用既有 `bench` 表达） |
...
| **R51** | **向量侧的合并成本与拓扑不确定**：`hnsw_rs` 无图合并 API ⇒ 只能 dump→load→insert（R23 的 `Box::leak` + R22 的 1.6× 体积 + 临时 I/O）或全量重建（≈38.7s/12K） | 若 A 不可行且必须走 B ⇒ **小 delta 也要付 O(N) 重建**，合并频率被迫降到很低 ⇒ delta 段堆积 ⇒ 读路径随段数变慢 | spike **S8-S1** 的决策门（§4.9.3）；`MergeReport.vector_strategy` 如实记录走了哪条 | 🟡 **2026-09-22 回填（`S8-09`）**：spike S8-S1 **5 条判据全 PASS**（A/B = **0.134**）⇒ **方案 A 已落地**、策略字段如实记录。**残余收窄为**：① `HnswIo` 每次合并泄漏 **≈200B + 路径串**（10 次 ≈ 2 KB，已登记）；② **图拓扑仍会漂移**（A 保住旧点拓扑，但 `insert` 新点后的 ANN 排名**不承诺逐位一致**，§4.6.2）⇒ **保持开放** |
| **R52** | **热路径回归：`bm25_f` 从 `None` 变 `Some`**（一旦有跨段墓碑，postings 没有被物理摘除 ⇒ 必须每 posting 判谓词） | 无过滤查询的 BM25 lane 常数变大（每 posting 一次 `dyn contains()`） | ① 「无 delta 且无墓碑」时**显式短路回 `None`**；② 无墓碑但有 delta 时用逐段 `AliveOnly`（零构建成本）；③ **S8-T12** 量化三档读数 | 🟡 **2026-09-22 已量化（`S8-09`）**：lane p50 = 档1 **44 us** → 档2（有 delta）**52 us** → 档3（+ 跨段墓碑）**53 us** → 档4（合并后）**50 us** ⇒ **谓词成本仅 +1 us（+2%）**、**多段扫描 +8 us（+18%）**；档 4 回到 `segments = 1` / 零谓词（**可逆**）。**保持开放**（是读数不是门槛） |
| **R53** | **视图指针锁与「写者优先」的交互**：`std::sync::RwLock` 在 macOS / futex 实现下，写者等待会挡住新读者（**与 R34 是同一个机制**）⇒ 若写者持写锁做了重活，读端 P99 会尖刺 | 合并期的读延迟判据（NFR-14 ②）失效 | ① **I8-1**（写锁只用于指针替换）写成 rustdoc + 评审红线；② **S8-T11** 直接测「合并期 vs 静止期」的 P99 比值 | 🟡 **2026-09-22 已测（`S8-09`）**：②a 三次运行皆 ≤ 1.2（bm25 **1.107 / 1.049 / 1.063**、hybrid **0.992 / 0.927 / 0.993**）⇒ **判据口径已定稿**；⚠️ **但尾部扰动不可复现**（B 的 `max` = 8.5 / 40.8 / 23.5 ms，A1 = 15.1 / 4.1 / 3.5 ms，**一次反向**；同深度第 4 大一次达 **2.74×**）⇒ **保持开放**，复审触发条件见 §4.13.1 |
| **R54** | **会话池的内存（R36 的查询侧背面）**：N 个 `TextEmbedding` 实例各持一份 `Session` + `Tokenizer` ⇒ 可能顶穿 NFR-05 的 372MB | 用户按 NFR-05 选机器 ⇒ 开启池化后 OOM | ① **默认 `embed_sessions = 1`**（零代价，不开就不付）；② spike **S8-S2** 按**查询侧口径**实测峰值 RSS（⚠️ **不得引用 E2 的建库口径读数**）；③ 决策门含「RSS 增量 ≤ 20%」 | ✅ **2026-09-22 消解（`S8-09`）**：spike S8-S2 **判「不投」**（判据③ 实测 **+148.4%**、判据② **2.10×**）⇒ **池化不进生产默认路径**（`embed_sessions` 默认 1）⇒ **本风险的条件未被触发**。⚠️ 是「**因为判『不投』**而不发生」，**不是**「没人碰」 —— 若将来结论翻转，本条须**重新打开**（并把 §4.11.4 的「判据③ 阈值结构性不可达」一并重估） |
