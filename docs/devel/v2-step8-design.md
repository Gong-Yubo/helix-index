...
| **S8-T11** | ✅ **已落地（2026-09-22，PR7b）**：判据 ②a 实测 = bm25 **1.107 / 1.049 / 1.063**、hybrid **0.992 / 0.927 / 0.993**（三次运行，基线 = **A1**）⇒ **定稿去「拟」**（基线定义写死，见 §4.13.1）；读数落 `eval-report.md` **§8.15.6**。原文：**合并期读延迟**（同机、同**图**、同 query 集 —— ⚠️ **2026-09-17 按评审 P4-3 更正**：原写「同 fig」）：②a 比值 ≤ 1.2（拟）与 ②b 绝对护栏| 脚本 | `scripts/eval_rw_concurrency.sh` | 红 = 写锁被用在重活上（I8-1 被违反） |
| **S8-T12** | ✅ **已落地（2026-09-22，PR7b）**：四档 lane p50 = **44 / 52 / 53 / 50 us**；**R52 的谓词成本 = +1 us（档3−档2）**、多段扫描 = **+8 us（档2−档1）**；读数落 `eval-report.md` **§8.15.7**。原文：**热路径回归对照**：无 delta / 有 delta 无墓碑 / 有跨段墓碑 / **有跨段墓碑但已 `merge_all()`** **四档**的 `bm25` lane 耗时（第 4 档由 P3-1 的「合并后回到 `None`」推出 —— 2026-09-17 新增）| 脚本 | 同上 | 量化 R52（不是门槛，是读数）；第 4 档必须回到第 1 档量级 |
