...
| ② | 吞吐增益（vector 路 `QPS(4)/QPS(1)`） | **≥ 2.5×** | **2.10×**（492.9 → 1034.1）；对照**未池化 = 1.23×** | ❌ **FAIL** |
