...
| **S8-T14** | ✅ **已落地（2026-09-20，PR6）**，但 **判据口径已就地更正**（见 §4.9.7）：原「与全量重建图的 top-10 重合率 **≥ 0.99**」**结构性不可达**（spike 实测：同数据重建两次只有 **0.9280**）⇒ 改为**以 `Brute`（精确）为参照的 recall@10 ≥ 0.9**（不受拓扑抖动污染，直接回答「丢没丢点」）。落点 = `S8_T14_合并后的向量侧不丢点_以精确后端为参照`。原文：**合并器向量侧等价**
