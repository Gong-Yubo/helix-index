...
**与架构文档第 12 章的关系**：架构文档 12.1 只给了阶段纲要与验收标准（纲要级）；本文档把它展开成**可执行、可勾选的任务清单**（任务级），并补充了架构文档不写的工程细节：命令、目录、测试点、阶段门槛、并行工作流。
...
| 评测数据集     | ✅ 已装配（T2Ranking 转换：320 query / 12K 段落，4 级标注；`t2-corpus.jsonl` / `t2-queries.jsonl` 已入库） | P5 | T5-01/T5-02（`t2_prep.rs` 转换管线，固定种子）      |
...
> **详细设计见 `docs/devel/p3-design.md`**（含 RRF 手算值、Explain 语义、确定性、决策点）。本章只保留任务纲要。
...
> **详细设计见 `docs/devel/p4-design.md`**（含 hnsw_rs 调研、快照格式、A/B 方案、决策点）。本章只保留任务纲要。
...
| P6 | I-14 旧 API deprecated 评估                              | ✅（D-I6 决议：本轮不加 `#[deprecated]`，P7 后评估） |

**当前阶段**：**P0~P6 全部完成**（2026-09-04）。P6 接口重构落地：门面层 `SearchIndex`/`Searcher`、写缓冲 + 批量 embed、配置指纹校验（修 B1/B2）、CLI/示例/文档全迁移，brute 后端新旧逐位一致（回归对账通过）。下一步 P7（v2：Rerank / MMR / delta 分段）。
