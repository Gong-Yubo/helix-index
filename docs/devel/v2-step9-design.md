...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.2（2026-09-24，第 1~2 轮评审响应）** —— 首版 **v0.1（2026-09-23）**；本版补 **P3-1**（`fuse_adaptive` 闭合应用通道）+ 六条 **P4** 处置 + 锚点双钉；**逐条处置见 §4.8** |
| 日期 | 2026-09-23 |
| 状态 | **已过两轮评审**（第 1 轮「通过 + 3×P4」；第 2 轮「**有条件通过 + 1×P3 + 6×P4**」）⇒ **本版 v0.2 已逐条处置**（**§4.8**）。本文**纯设计、`.rs` 零改动**（体例同 `v2-step5-design.md` / `v2-step8-design.md` 的首版）。 |
...
| 锚点体例 | ⚠️ 跨文档引用一律 **「§ 锚点 + 文档版本 + 行号」双钉**（第 1 轮评审 **P4-3**）：行号**只作辅助**、且必须标注它是**哪个文档版本的时点**（本文自己会挪行 ⇒ 纯行号锚点合并即漂移） |
...
| **D-S9-04** | 判据口径 | 主指标**分桶 `MRR@10(1)`**（只看名次 ⇒ 对 **R46** 的 `Hit.score` 语义免疫）+ `NDCG@10` 为辅；**基线 = 固定权重 `(1.0, 1.5)`**；**同一冻结图**；**`candidate_k` 两侧相同**；**精排默认关** | `plan-v2.md` §6 验收门槛的 **Step 9** 行（**v0.25 时点 `:648`**）+ `eval-report.md` §3.3 / §3.4 |
...
| 问题 | 证据（位置） | 严重度 |
| --- | --- | --- |
| **Q-R1** paraphrase 桶 hybrid 被**弱路 BM25 稀释** | `eval-report.md` §3.3（标题行 `:48`，该文件**当前版本**）：paraphrase 桶（n=80）NDCG@10 —— bm25 **0.2306** / vector **0.4294** / hybrid **0.3799**；hybrid vs vector 的逐 query 胜负 = **25 / 11 / 44（p = 1.0）** ⇒ hybrid 在该桶被拖到 vector 之下 | 高（MRR 口径；来源 `plan-v2.md` §3 需求来源表的 **Q-R1** 行，**v0.25 时点 `:87`**） |
| **Q-R3** 假负例敏感度对照未做 | `eval-report.md` §10「残留 P6」第 2 条；`plan-v2.md` §3 **Q-R3** 行（**v0.25 时点 `:89`**） | 低 |
| **Q-R4** 评测 query 来自人类日志，缺 Agent query | `eval-report.md` §9「已知偏差」第 3 条；`plan-v2.md` §3 **Q-R4** 行（**v0.25 时点 `:90`**） | 中 |
...
| **H7**（`plan-v2.md` §4.0「开工前必须澄清的设计决策」表，**v0.25 时点 `:117`**） | **D-S9-06**：只作对照、不替换 ⇒ P5 锚点 / Step 7 的 `R = 20` 标定 / NFR-12 保持可比 |
...
5. **评测资产纪律**（`plan-v2.md` §4 Step 9 的「评测资产纪律（新增，必读）」条，**v0.25 时点 `:554`**）：新造资产**只作对照、不替换主基线**（**H7** / **D-S9-06**）。
6. **结论不外推**（`eval-report.md` §9）：仅对「T2Ranking 中文网络语料 + bge-small-zh-v1.5 + 12K 段落」成立。
7. **A/B 必须同一冻结图**（`plan-v2` V2.1 开工纪律）：`--index` 读快照 + `--runs 1`。
8. **延迟类读数不具跨机可引用性**，报数必标运行范围（`architecture-design.md` §14.6 末的「读数落点 + 报数纪律」段，**v1.24 时点 `:1687`**）。
...
- **编排层**（`searcher.rs:374` 附近）的**两条显式规则**（第 2 轮评审 **P3-1**）：
  ...
  2. **`== true`** ⇒ 构造 `FusionCtx` → 调 `fuse_adaptive(&lanes, candidate_k, &ctx)`；
     `w` 的**决策与应用都在策略内部** ⇒ 编排层**不需要、也不能**消费 `w`。
...
| **P4-6** | 附录 B 的 θ **无从传入**（布尔开关 + θ 只出现在文件名） | ✅ 改为 `--adaptive-fusion-theta <v>` 值形态 + `for θ in <预注册网格>` 循环；注明取自 §4.5 | 附录 B |
...
- 原文（`requirements-spec.md` §附-1 FR 表 **FR-35** 行，**v1.24 时点 `:309`**；`plan-v2.md` §5 需求编号草案 **FR-35** 行，**v0.25 时点 `:617`**）：「融合权重**按桶**（paraphrase / exact / 长尾）自适应」。
...
for θ in 0.00 0.05 0.10 0.15 0.20 0.25 0.30 0.35 0.40 0.45 \
         0.50 0.55 0.60 0.65 0.70 0.75 0.80 0.85 0.90 0.95 1.00; do
  cargo run --release -p helix -- bench --index <snap> --queries data/t2-queries.jsonl \
    --modes hybrid --k 10 --runs 1 --rrf-k 60 \
    --adaptive-fusion true --adaptive-fusion-theta "$θ" \
    --json "/tmp/s9_adaptive_theta-${θ}.json"   # ⚠️ 两个开关都是 PR9-1 新增
done
...
| H7（评测资产能否替换主基线） | `plan-v2.md` §4.0 **H7** 行（**v0.25 时点 `:117`**） |
| Step 9 的验收门槛 | `plan-v2.md` §6 **Step 9** 行（**v0.25 时点 `:648`**） |
| R45 / R46 的既有登记 | `architecture-design.md` **§14.5**（**v1.24 时点 `:1644`**） |
| Step 8 的风险占位到 R54（⇒ 本步从 **R55** 起） | `architecture-design.md` **§14.6**（**v1.24 时点 `:1662-1676`**） |
