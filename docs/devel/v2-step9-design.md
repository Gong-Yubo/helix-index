...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.5（2026-09-25，PR9-1 第 1 轮评审响应 ⇒ 见 §4.11）** —— v0.1（首版）→ v0.2（第 1~2 轮）→ v0.3（第 3 轮 ⇒ 可开工）→ v0.4（§4.10：PR9-1 实现期定形）→ **v0.5**（§4.11：实现轮 10 条评审逐条处置）；**逐条处置见 §4.8 ~ §4.11** |
| 日期 | 2026-09-23（v0.1~v0.3）/ 2026-09-24（v0.4）/ 2026-09-25（v0.5） |
...
| **P3-1** | 设计头部状态行/日期行未随 v0.4 更新（「可开工」「纯设计」成将来时残留） | ✅ 版本行升 v0.5、状态行改「设计已过三轮评审 + **PR9-1 已实现**（PR #72）」、日期行分版本标注；§4.9 开工前清单（历史节）逐项加「已完成，见 §4.10/§4.11」注记 | 文首三行 + §4.9 |
