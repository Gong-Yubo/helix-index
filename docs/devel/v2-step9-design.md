...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.6（2026-09-25，PR9-1 第 2 轮评审响应 ⇒ 见 §4.12）** —— v0.1（首版）→ v0.2（第 1~2 轮）→ v0.3（第 3 轮 ⇒ 可开工）→ v0.4（§4.10：实现期定形）→ v0.5（§4.11：第 1 轮 10 条处置）→ **v0.6**（§4.12：第 2 轮 ✅ 通过 + 1×P3 + 1×P4 处置）；**逐条处置见 §4.8 ~ §4.12** |
| 日期 | 2026-09-23（v0.1~v0.3）/ 2026-09-24（v0.4）/ 2026-09-25（v0.5~v0.6） |
