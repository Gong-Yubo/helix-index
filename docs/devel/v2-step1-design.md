...
| # | 测试 | 位置 | 断言要点 |
| --- | --- | --- | --- |
| T1 | 已删向量不霸占 Top-K 名额 | `tests/integration.rs` | 20 篇删 15 篇留 5 篇，Brute + Hnsw 两后端；**核心断言 `hits.len() == k`**（§2.1 澄清：幽灵进不了 `hits`，危害是占名额）。另附三模式 soundness（结果不含已删 chunk_id）。**反向验证**：摘掉向量路谓词后本测试必须失败（实测 5 → 2 条） |
