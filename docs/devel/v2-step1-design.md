...
> 面向 Agent 场景的通用检索引擎内核——V2 Step 1 的详细设计（**已实施完成，本版为交付后终版**）。
> 本文件回答：改哪些模块、为什么这么改、每个接口长什么样、怎么验证。

| 项 | 内容 |
| --- | --- |
| 版本 | **v1.1**（S1-01 ~ S1-11 全部完成，含 10 万级实测与 issue #9 修复） |
| 日期 | 2026-09-05 |
| 状态 | **已实施完成并合入 main**：PR #6（`61e3dac`）+ PR #8（`0d87b39`）+ PR #10（`e1e4b60`）。R18（低选择度延迟）为已知限制，归 V2.1 |
| 上游 | `plan-v2.md`（Step 1 / H2 / Q-C1 / Q-I1 / Q-I2）、`requirements-spec.md` v1.5（FR-26 / FR-27）、`architecture-design.md` |
| 范围 | T7-07 向量软删除 + 存活过滤、T7-08 过滤下推 + 字段索引 |
| 非范围 | 图持久化（Step 2）、原子快照（Step 3）、墓碑物理回收与体积（**Step 4**，2026-09-07 重排前为 Step 5）、精排（**Step 7**，重排前为 Step 6） |

> ⚠️ **步骤编号注记（2026-09-07）**：V2 剩余步骤已重排，本文中的 **Step 5 = 新 Step 4（资源回收）**、
> **Step 6 = 新 Step 7（精排）**；Step 1~3 编号不变。映射表见 `plan-v2.md` §附-2。
> 另：R18（低选择度延迟）原写「归 V2.1」，已按 D-J9 **提前到 V2.0 Step 5**。
...
> **最新修订见 §0.3（v1.0 → v1.1，issue #9 修复）。**
...
## 0.3 修订记录（v1.0 → v1.1，issue #9 修复）

Step 1 收尾合入后，bench 工具自身暴露一个 panic（[issue #9](https://github.com/Gong-Yubo/helix-index/issues/9)，PR #10 / `e1e4b60` 修复）：

| # | 发现 | 处理 |
| --- | --- | --- |
| **`--oracle-depth ≥ 501` 触发 clamp panic** | `oracle_depth_for` 的下界 `k × depth` 随用户参数无界增长，上界固定 `ORACLE_MAX_DEPTH=5000`，`min > max` 时 `clamp` 无条件 panic（与 debug/release 无关）。测试还发现相邻的**乘法溢出**：`usize::MAX` 级 depth 踩中 `k × depth` / `need × depth` 两处溢出 panic | 下界先被上限夹住再 clamp（与早退分支对齐）；两处乘法改 `saturating_mul`；clap 侧加 `1..=500` 范围校验；新增边界单测（depth ∈ {501, 600, 5000, usize::MAX/2, usize::MAX} 全组合不 panic） |

> 该修复只影响 bench 工具参数边界，**不影响检索路径与任何已实测结论**（§10.1 数据均以 depth=10 合法参数采集）。
