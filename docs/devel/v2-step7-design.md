...
| --- | --- | --- | --- |
| 质量轴 query | 全量 320 | **分层 60**（`--query-sample 60`：每 `type` 15 条，**交错排列**） | 成本 ÷5.3；**分层**保证子集内 type 分布与全量同构；交错 ⇒ `head` 子集也覆盖各 type |
| 档位 | `R ∈ {10,20,50,100,200}` | **`R ∈ {10,20,50}`** | `R=100/200` 单档 >12h；三点足以判「单调 vs 饱和」 |
| 交错轮数 | 2 | **1** | 质量指标**确定性** + 冻结图（S7-T11：3 次逐位一致）⇒ 跨时段漂移恒 0 ⇒ 2 轮冗余 |
| 延迟轴 | 40 query × `--reps 20 --warmup 3` | **10 query × `--reps 3 --warmup 1`** | 成本 ÷~16（**成本** = `LAT_QUERIES × modes × (1 + warmup + reps)` = 150/三路）；⚠️ **分位数样本只有 `LAT_QUERIES × reps` = 30 个/mode** —— 效果阶段与 warmup **不进分位数**，两个分母别混用 |
