...
> ## 🔧 实现期勘误（**E1~E9**）
>
> - **E1~E7（2026-09-14）**：S7-01 / S7-02 实现期（E1~E6 = PR #52 第 1 轮评审意见 3 的落地；
>   E7 = PR #52 第 2 轮「新 1 / 新 2」的落地）。
> - **E8 / E9（2026-09-15）**：**S7-04 标定实跑期** —— E8 = §4.6.1 协议**按算力实测缩减**；
>   E9 = §4.6.3 要求的「延迟分列」在 **bench 侧此前没有出口**，已补。
>
...
> | **E8** | §4.6.1 协议表（**S7-04 实测**） | 质量轴 `320 query → 分层 60`、档位 `5 档 → {10,20,50}`、交错 `2 轮 → 1 轮`、延迟轴 `40×reps 20 → 10×reps 3` —— 依据 = 单文档精排 **≈0.45s** 且与 `R` 近似线性 ⇒ 原协议复算 **≈262 小时**（详见新增 §4.6.5）。⚠️ **缩减不破坏 `Δ` 的同口径可比**（A 与各档同子集 / 同 `--runs 1` / 同一冻结图），但**与 P5 的全量数字不可直接横比** |
> | **E9** | §4.6.3 判据表（**S7-04 实测**） | 「延迟**分列**（端到端 `took` vs `rerank_elapsed`）」在 **bench 侧此前无从落地**：`Metrics.rerank_elapsed` / `rerank_window` 虽在 S7-02 加好，但 `bench` 的延迟 JSON 与表格**一个都没输出** ⇒ S7-04 的 NFR-12 判据**取不到数**。已在 `crates/cli/src/bench.rs` 补 `rerank_p50_ms` / `rerank_p99_ms` / `rerank_n` / `mean_rerank_window` 四个出口（⚠️ 收集判据 = 「本档位传了 `--rerank-window`」**且** `metrics.rerank_window > 0`；只用「耗时 > 0」会被 `NoOpReranker` 的 ~0.5µs 空调用绕过，实测踩过）；该双条件已提成纯函数 `should_collect_rerank` + 单测钉住（否则只能靠真机跑验证）⇒ `eval_latency` 参数过 clippy 阈值（7），按 `FilterCtx` 先例捆出 `LatencySpec` |
>
> | **E10** | §7 测试计划 / §4.6.3 判据（**S7-05 收尾**） | ① **落点勘误**：S7-T7 原写 `tests/step7_rerank_observability.rs`（新）—— 该文件**未创建**，T1~T7 / T12~T14 实际都落在 **`crates/core/src/query/searcher.rs` 单测**里（理由：这些用例都要**注入间谍精排器**去 `search_parts`，集成层够不到）；② **拐点判据落定**：实测**抖动带 = 0** ⇒ §4.6.3 / §4.6.4 的「增量 < 抖动带」**退化** ⇒ 实跑改用「**边际 ÷ 首段 < 10%**」（**实现期选定**，已写进 §4.6.4 正文）；③ **T8/T9/T10 已本地实跑**（此前四轮只能写「一条没跑」，读数 `eval-report.md` §8.14.11） |
>
> ✅ **v0.3（2026-09-16）已正式升版**：本节与定义面四处（需求 **v1.18** / 架构 **v1.17** / `plan-v2` **v0.18** / `docs/README.md`）**同批回写**。
>
> ### S7-05 回写清单（**7 项 —— 已于 2026-09-16 全部结清**）
>
> ✅ **结清方式**：① NFR-06 机制句已更正（`requirements-spec.md`，**以 D-S7-07 为准**）；② `plan-v2.md` 的 fastembed 锚点已改**符号指代 + 带版本行号**；③ 架构 §14.5 R48 的 `init.rs` 锚点同上；④ 跨定义面漂移一次性修完；⑤ 架构 §14.5 R48 的 `bench.rs` 锚点已改**符号指代**（并定下「本仓源码一律符号指代」的规则）；⑥ **R48 收窄后关闭** + **NFR-12 定稿**；⑦ 拐点口径已落进 **§4.6.4 正文**、NFR-12 定稿采用**双口径**。**本节以下为当时的原始记录（保留引文）**：
>
...
>    因 PR 改动 `bench.rs` 而漂移**（S7-03 实测 **→ `:760`**；**S7-04 实测再 → `:786`**）。
>    ⚠️ 属**定义面** ⇒ 本 PR **不改**，与第 2、3 项并入同一次回写。
>    **各 PR 已做的部分**：把**本文**全部 `bench.rs` 行号引用按**实测**更新 ——
>    S7-03 接线轮 **10 处**（`697→757`、`380-390→436-446`、`655-668→711-724`、`661-663→717-719`、
>    `160-162→178-180`、`1530→1597`）+ 响应轮 **5 处 +3**；
>    **S7-04（本轮）5 处**：`760→786`（×2，§3.5 与附录 C）、`720-722→746-748`、`714-727→740-753`、
>    `1600→1717`；⚠️ 同时**核过未漂移、刻意不动**的：`439-449`（`--runs > 1` 重建图块，×4 处）
>    与 `178-180`（`--threads` 前置条件）—— 两者的字节内容与行号都**未变**（PR 的改动全在其后）。
>    并**全仓扫过**其余载体 —— `v2-step5-design.md` / `v2-step2-design.md` /
>    `CHANGELOG.md`（历史条目）里的 `bench.rs` 行号**在本 PR 之前就已漂移**（Step 2/5 之后
>    `bench.rs` 又改过多次）⇒ **不属本 PR 造成**，按「历史记录不改写」的纪律**不动**。
>    ⚠️ **但要说清**：本 PR（以及 S7-03）**又把它们变得更偏**了 —— 凡引用点落在 `bench.rs` 改动
>    **之后**的行，都在这层**既有漂移之上再加 ~26（S7-04）/ ~104（S7-03）行**。这不是「不动就没事」，
>    而是**欠账在累积**：这是**第 5 项建议改成符号指代**的直接理由（在**历史文档**上也同样适用）。
>    🔑 **给 S7-05 的建议**：顺手把**定义面**里的行号引用改成**符号指代**（如「`bench.rs` 的
>    `build_analyzer`」），从根上消掉「一改 CLI 就全体漂移」这一类问题。
> 6. **（S7-04 新增）`R48` 的风险状态回填**：本 PR 已给出定量读数
>    （`eval-report.md` **§8.14.7**：固定 `R=20` / hybrid、`max_len 512→1024` 的增益 **≈0**
>    ⇒ **截断不是「精排无提升」的主因**）⇒ S7-05 应据此把 `architecture-design.md` §14.5 **R48**
>    的「残余」列从「⏳ 开放：…要等标定」改为**有依据的结论**（含两条限定：①「调大 `max_length`
>    未被数据支持」；② **1024 档未测延迟**）⇒ 派生 **NFR-12 重新定稿**（§8.14.6：拟值 `≤500ms`
>    与最小档实测 P99 **5220ms** 差 **10.4×**）。⚠️ **R44~R48 的其余条目本 PR 一字未动**。
> 7. **（PR #56 评审提出，S7-04 响应时登记）两条「别让下一个照设计原文跑的人再撞一次」**：
>    - **把拐点口径写进 §4.6.4 正文**：本 PR 实测**抖动带 = 0** ⇒ 设计原文的判据
>      「Δ 首次 < **抖动带**」**退化**（只有增量恰为 0 才成立），S7-04 只好改用在报告里标注的
>      操作口径「**边际 ÷ 首段 < 10%**」。⚠️ **现在它只活在报告 §8.14.5 里** ——
>      S7-05 应把它**落进 §4.6.4 的判据原文**（并写明这是**实现期选定**的阈值、不是设计期给的）。
>    - **NFR-12 定稿给两个口径**：评审建议同时给「**CPU / FP32 的实测上界**」（本 PR：`R=20` 档
>      P50/P99 = 8.78 / 10.86 s）与「**GPU / 量化后的目标值**」，避免像这次那样**单一数字整体失配**
>      （拟值 `≤500ms` vs 实测最小档 5220ms）。⇒ 与第 6 项的「重新定稿」是同一件事的**形态要求**。
>
...
| **H3** | **精排窗口未定**：编排在融合后**先 `take(k)` 再进精排** ⇒ 精排只能重排 `k` 条 | `crates/core/src/query/searcher.rs` 的融合后截断处（take）vs `rerank` 调用处（⚠️ **2026-09-16 更正**：本格原用**本仓行号** `:286` / `:316`，实装后已漂移（实测 `:337` / `:375`）⇒ 按 §14.5 锚点规则改**符号指代**） | 高（决定本 Step 能否达成验收） |
...
| `chunk.text.clone()` / `doc.source.clone()` / `metadata.clone()` | **整段文本拷贝**（T2Ranking 段落最长 **76,895 字符**，见 `crates/cli/src/bench.rs:786` 的注释） |
...
| 4 | **`--rerank-window` 在未编译 `local-rerank` 时** | `bail!` + 重编提示 | ⚠️ **脚枪之二**：照抄 `--analyzer charabia` 的既有做法（`bench.rs:746-748`）——**不得**静默退回 NoOp（那会让「精排档位跑通了」的结论建立在空转上） |
...
- **⚠️ 行号引用已同步（三轮）**：Step 7 的实现 PR **三次**改 `bench.rs` —— ① 接线轮（新增参数 + 守卫 + 装配注入）、
  ② **S7-03 响应轮**（把 `build_reranker` 挪到 `parse_modes` / `parse_thread_levels` 之后，理由见该处注释）、
  ③ **S7-04 本轮**（补精排延迟出口 + `LatencySpec` + 两条纯函数单测）；
  每轮都按**实测**重新定位并更新全文引用（接线轮 10 处；响应轮 5 处 **+3**；**本轮 5 处**，
  并核过 `439-449` / `178-180` **未漂移**）。**定义面**上那处
  （`architecture-design.md:1628`）的同类漂移已登记在头部勘误块 **S7-05 清单第 5 项**（含最新目标值 `:786`）。
...
| **质量轴** | ⚠️ **E8**：全量 320 → **分层 60**（`hybrid` / `vector` / `bm25` **三路仍全测**） | 质量指标不需要重复（确定性）；~~全量保证与 P5 可比~~ ⇒ 见 §4.6.5（缩减后**不可**与 P5 全量横比） |
| **延迟轴** | ⚠️ **E8**：固定子集 **10 query** × **`--reps 3 --warmup 1`** | 精排是**秒级**（实测 ≈0.45 s/文档）⇒ 原取值复算 ≈171h（见 §4.6.5） |
| **档位** | ⚠️ **E8**：**`R ∈ {k(10), 20, 50}`** + **`max_length ∈ {512, 1024}`** 的对照组 | H3 的拐点曲线 + R48 的截断影响；`R=100/200` 单档 >12h ⇒ 三点判「单调 vs 饱和」足够（§4.6.5） |
| **交错** | ⚠️ **E8**：**1 轮**（原 2 轮）；延迟轴保留控制组 A 与同 `reps`/`warmup` | 质量指标**确定性** + 冻结图（S7-T11 三次逐位一致）⇒ 跨时段漂移恒 0，2 轮冗余（§4.6.5） |
...
| **窗口拐点** | 取「MRR 相对 `R=10` 的增量 **< 抖动带**」的**最小** `R`；⚠️ **实现期落定（E10）**：实测**抖动带 = 0**（§8.14.3）⇒「< 抖动带」**退化** ⇒ 实跑采用**可操作口径「边际增量 ÷ 首段增量 < 10%」**（**实现期选定**，见 §4.6.4 末注） | 即「再加窗口没意义」的点 |
| **延迟** | 端到端 `took` P50/P99 **与** `rerank_elapsed` P50/P99 分列 | ⚠️ 只报 `took` 无法归因；只有 `rerank_elapsed` 才是 NFR-12。**NFR-12 已定稿（2026-09-16）：默认 `R = 20` 下 CPU/FP32 上界 P50 ≤ 8.8 s / P99 ≤ 10.9 s**（**双口径** + **仅精排开启时适用**）；🔴 原「≤ 500ms（拟）」**已废弃**（实测最小档差 **10.4×**）。出处与「与默认 `R` 联动」见 §4.2.2 补记 |
...
⚠️ **拐点口径的落定（E10，2026-09-16）**：本 Step 实测 **抖动带 = 0**（同一冻结图 **5 份**读数 md5 相同，`eval-report.md` §8.14.3）
⇒ 上面 ①/②/③ 里的判据**「< 抖动带」在抖动带为 0 时退化**（只有增量**恰为 0** 才成立）
⇒ 实跑采用**可操作口径**：**「边际增量 ÷ 首段增量 < 10%」判「拐点已现」**。
⚠️ **该 10% 阈值是实现期选定的、不是设计期给的** —— 落定在此，免得下一个照本文原文跑的人再撞一次同一个退化。
（S7-04 实测：hybrid 的 `20→50` 边际降到首段的 **6.6%** ⇒ 拐点 `R = 20`；读数见 `eval-report.md` §8.14.5。）
...
#### 4.6.5 ⚠️ 协议缩减（**E8**，S7-04 实跑时定；2026-09-15）

§4.6.1 的取值是**设计期**的。S7-04 实跑前实测到两件事，导致必须缩减：

| 实测 | 数字 | 后果 |
| --- | --- | --- |
| 单文档精排耗时 | **≈0.43~0.48 s/文档**（`bge-reranker-v2-m3` FP32 / CPU EP / M5 10 核 / batch 256 已计入） | 成本与 `R` **近似线性（略优于线性）** ⇒ 复算按 **`0.45 s/文档`**（延迟轴实测精排 P50 = **4.57 / 8.78 / 21.47 s/query**，`R = 10/20/50`；单条 457 / 439 / 429 ms） |
| 延迟轴的实际调用数 | 每 mode = **1（效果阶段）+ `warmup` + `reps`** —— 不是 `warmup + reps` | 设计期漏算了效果阶段 ⇒ 原取值（`40 query × 24 次`）按**脚本默认 50 query** 复算 = **171h** |

按 §4.6.1 原样复算（320 query × 3 路，统一按 **`0.45 s/文档`**）：
质量轴 = `320 × 3 路 × 2 轮 × ΣR(10+20+50+100+200) × 0.45 s` ≈ **91 小时**（5 档 × 2 轮）；
延迟轴 = `50 query × 24 次/mode × 3 路 × ΣR × 0.45 s` ≈ **171 小时**
⇒ **合计 ≈262 小时**，本机不可行（**7 天以上**）。

⚠️ **两处口径说明**（免得复算对不上）：
1. 延迟轴按 **50 query**（`scripts/eval_rerank.sh` 的 `LAT_QUERIES` 默认值）而非 §4.6.1 写的 40
   —— 40 的话是 ≈137h；**用默认值是因为「按设计原样跑」时它才是真实成本**；
2. 单条成本**取 `0.45 s` 的常数**（实测 0.43~0.48）⇒ 外推只承诺**量级**，不是精确预算。

**实际采用的协议**（对应 `scripts/eval_rerank.sh` 的同名选项）：

| 项 | §4.6.1（设计期） | **实跑（E8）** | 缩减理由 |
| --- | --- | --- | --- |
| 质量轴 query | 全量 320 | **分层 60**（`--query-sample 60`：每 `type` 15 条，**交错排列**） | **query 数 ÷5.3**（⚠️ **总成本 ÷10.7** —— 轮数 2→1 也减一次）；**分层**保证子集内 type 分布与全量同构；交错 ⇒ `head` 子集也覆盖各 type |
| 档位 | `R ∈ {10,20,50,100,200}` | **`R ∈ {10,20,50}`** | `R=100/200` 单档 >12h；三点足以判「单调 vs 饱和」 |
| 交错轮数 | 2 | **1** | 质量指标**确定性** + 冻结图（S7-T11：3 次逐位一致）⇒ 跨时段漂移恒 0 ⇒ 2 轮冗余 |
| 延迟轴 | 40 query × `--reps 20 --warmup 3` | **10 query × `--reps 3 --warmup 1`** | **调用数 `2880 → 150` ⇒ ÷19.2**（旧 = `40 × 3 × (1+3+20)`；新 = `10 × 3 × (1+1+3)`）；⚠️ **分位数样本只有 `LAT_QUERIES × reps` = 30 个/mode** —— 效果阶段与 warmup **不进分位数**，两个分母别混用 |

🔑 **缩减不破坏的**（这是它可辩护的关键）：`Δ` 仍**同口径可比** —— 控制组 A 与所有档位用
**同一子集**、同一 `--runs 1`、**同一张冻结图**；S7-T11 前置自证仍通过。
🔴 **缩减破坏的（必须随数字一起引用）**：与 **P5 的历史数字不可直接横比** ——
P5 的 `hybrid MRR@10(1) = 0.6922` 是**全量 320** 口径，本子集口径不同。
⇒ 报告里每个数字都要带「分层 60 子集」这个限定词，并把**同一子集的控制组 A 读数**并列。

⚠️ **延迟轴的两个分母（别混用）**：**分位数样本** = `LAT_QUERIES × reps`（只有计时轮进分位数）；
**总推理调用** = `LAT_QUERIES × modes × (1 + warmup + reps)`（**成本**口径，含效果阶段与 warmup）。
`--reps` 太小则 P99 无意义（`perf-ab-calibration` 规则 3：样本 <100 时 P99 只是装饰 ——
本节 `reps = 3` ⇒ 分位数样本仅 30/mode，**P99 只作量级参考**）。
...
| **S7-T7** | `Metrics.rerank_window` / `rerank_elapsed` 被填；NoOp 时 `rerank_window == k`；`metrics.took == took` 仍自洽（I7） | NFR-07 / 口径自洽 | ✅ | ⚠️ **E10 落点勘误**：原写 `tests/step7_rerank_observability.rs`（新）—— 该文件**未创建**；T1~T7 / T12~T14 实际落在 **`crates/core/src/query/searcher.rs` 单测**（都要注入间谍精排器，集成层够不到）+ 既有 `step5_query_observability.rs` 口径不破 |
| **S7-T8** | **P1/P2（§4.7）**：同进程重复 N 次 + 跨进程两次 ⇒ (`chunk_id`, `score`) 逐位一致 | R47 / NFR-06 | ⛔ `#[ignore]`（需 2.19GB 模型）；✅ **2026-09-16 已本地实跑**（P1/P2 逐位一致，`eval-report.md` §8.14.11） | `tests/step7_rerank_local.rs`（新） |
| **S7-T9** | **P3（§4.7）**：同一 `(query, doc)` 单条 vs 批内 ⇒ logit 是否逐位相同（**记录结论，不预设**） | §2.5 的 `BatchLongest` | ⛔ `#[ignore]`；✅ **2026-09-16 已本地实跑**（单条 vs 批内 logit **相同**，`eval-report.md` §8.14.11） | ⚠️ **E3**：`rerank/local.rs` **模块内部**（公开面只有 `σ(logit)`，σ **多对一** ⇒ 证明不了 logit 逐位相同） |
| **S7-T10** | 真模型 smoke：`R = k = 10` ⇒ 集合不变、仅顺序可能变；`R = 50` ⇒ 集合可增补后截断到 `k` | FR-18 的行为基线 | ⛔ `#[ignore]`；✅ **2026-09-16 已本地实跑**（`R=50` 时 **4 条**新候选进 top-10，`eval-report.md` §8.14.11） | 同上 |
...
> ⚠️ **本表的行号是 2026-09-14 设计期时点，实装后已整体漂移** —— `searcher.rs` 实测：`candidate_k` = **`:149`**、
> 融合后截断 `take(take_n)` = **`:337`**、`rerank(...)` = **`:375`**、`fuse(...)` = **`:276`**；`filter.rs` 的 `allowed_chunks` = **`:53`**。
> ⇒ **本表仅作历史记录**；按架构 §14.5 的锚点规则，**正文引用本仓源码一律「符号指代」**（定位请按符号名，不要照抄本表行号）。
...
| `--analyzer charabia` 的「缺 feature 即报错」先例 | `crates/cli/src/bench.rs:740-753` |
| T2Ranking 段落最长 76,895 字符 | `crates/cli/src/bench.rs:786` |
| `--threads` 的逐位一致前置条件（NFR-10 ①） | `crates/cli/src/bench.rs:178-180`；`check_concurrency_precondition` `:1717` |
