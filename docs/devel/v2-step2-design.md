...
| 非范围 | 原子快照的实施（Step 3，本文只定协议）、墓碑物理回收（**Step 4**，2026-09-07 重排前为 Step 5）、embed 并行与增量构建（**Step 6**，重排前为 Step 4）、精排（**Step 7**，重排前为 Step 6） |

> ⚠️ **步骤编号注记（2026-09-07）**：V2 剩余步骤已重排，本文中的 **Step 4 = 新 Step 6（构建性能）**、
> **Step 5 = 新 Step 4（资源回收）**、**Step 6 = 新 Step 7（精排）**；Step 1~3 编号不变
> （本文的 Step 3 原子快照引用**仍然有效**）。映射表见 `plan-v2.md` §附-2。
...
| **P0-7** | S2-T6 / 验收 5 的「逐位一致」在 R-P5-13 下写不死 | **已修**：改为「加载成功 + oracle Top-10 重合率 ≥ 0.95」；S2-T7（Brute）保留逐位。⚠️ 该口径**后又两次修正**（#17：改 20 query 均值；**#34**：改**两条不变式**——内容覆盖 + 自匹配），最终见 §8 S2-T6 行 | §1.3 验收 5、S2-T6 |
...
| **5** | **旧快照可加载** | `data/t2-index.snapshot`（`FORMAT_VERSION=2`、无图 sidecar）加载成功，`GraphStatus::Rebuilt`，且满足**两条拓扑无关的不变式**：① **内容覆盖**——`graph_points == 快照向量条数`；② **自匹配**——每篇文档用自己的文本查必须排第一、余弦相似度 ≈1。<br>⚠️ 判据经**三次**修正（P0-7 → #17 → **#34**）：①「与暴力 oracle 的 Top-10 重合率 ≥ 0.95」→ ②「20 query 均值 ≥ 0.90 + 最差 ≥ 0.5」→ ③ **本条（不变式）**。第 ③ 次的理由**不是**「绝对值不可用」：CI 那次的 `均值 0.545` 落在抖动分布**之外**（本地 64 次与评审 30 轮重新建图实测都是 0.99~1.00）、**至今未归因**，所以正解是**把不可归因的数值门换成可归因的不变式**——同样会红，但红得能指出是「内容缺失」「向量 / ID 灌错」还是「图不可导航」（issue #34 保持 open）。<br>⚠️ **不能用「与改造前逐位一致」做判据**——旧快照无图 ⇒ 走重建 ⇒ 重建本身跨进程不确定（§2.2），逐位一致在物理上不成立（P0-7） |
...
- **实测收益（2026-09-07，`eval-report.md` §8.6）**：12K 真实语料 11.676s → 2.182s
  （**5.35×**）；50K 合成向量 171.8s → 32.7s（5.26×）⇒ 加速比稳定 ≈5.3× 且与规模无关；
  真实语料 oracle 重合率无差异。⇒ 默认「关」的取舍**值得复议**（翻默认值需独立 PR，
  因 `S2-T22` 断言「默认必须串行」，改默认值必须同步改护栏语义）。
- **默认值的取舍**：建议默认**关**。理由：P5/P6 的全部基线（NDCG/MRR/延迟）都在串行建图下测出，Step 6 精排的对比实验需要可比基线；先出实测（S2-11）再决定是否翻默认值。
- **⚠️ 质量对账的阈值必须留抖动余量（2026-09-07 实测校准）**：并行灌入 ⇒ 图拓扑随机 ⇒
  并行库相对串行库与 oracle 的 Top-10 重合率**通常低 0~2 个百分点**（1200 条语料、20 个 query）。
  评审期定的「差 < 1pp」在 `--features charabia` 下实测出现过 `0.9850 vs 1.0000`（1.5pp）
  与 `0.9900 vs 1.0000`（正好卡边界），**CI 同树两次一过一挂** ⇒ S2-T13 的阈值放宽到 **< 5pp**。
  这不是放宽验收，而是把「固有抖动」和「真退化」分开：真退化由
  **T22（钉死 `add_batch` 是否真的走了并行分支）** 与「两条路径各自 ≥ 0.90」的底线守住。
...
| **S2-T3** | 图加载 vs 重建的质量等价性 | 持久化图与重建图对同一 query 集，与暴力 oracle 的 Top-10 重合率均 ≥ 0.95（沿用 `hnsw_rs_index.rs` 现有口径）。<br>⚠️ **本条编号已重定义**：落地时 S2-T3 改为「**并行 vs 串行** oracle 质量等价」，顺延至 S2-11 的 PR（见 `CHANGELOG.md` 的 P1 条目）。本行的**原意图**（图路径 vs 重建路径的质量等价）现由 **S2-T6 的两条不变式**（内容覆盖 + 自匹配）与 **S2-T13 的相对口径**承接 ⇒ **别再引用「S2-T3 口径 ≥0.95」**，该表述已无所指 |
...
| **S2-T6** | 旧快照兼容（验收 5） | `FORMAT_VERSION=2`、无 sidecar 的快照加载成功，`GraphStatus::Rebuilt`；**两条拓扑无关的不变式**：① **内容覆盖**——`graph_points == raw_vectors == 200`；② **自匹配，覆盖全部 200 篇**（不是 20 篇）——每篇用自己的文本查必须**排第一**、余弦相似度 ≈1，oracle 侧同一不变式（顺带钉住「第 i 篇 `chunk_id == i`」这一前提）。⚠️ 口径经**三次**修正：①「单 query 重合率 ≥ 0.95」→「20 query 均值 ≥ 0.90」（单 query 粒度只有 0.1，卡 0.95 等于要求 10/10 全中）；②「均值重合率」→「自匹配」（#17 / **issue #34**）；③ **20 篇自匹配 → 全部 N 篇自匹配 + 结构层内容覆盖**（评审实测：只查 20 篇时「重建只灌了前 30 篇」也能三查全绿，而重合率已掉到 0.22）。<br>⚠️ **撤掉数值门的理由要说准**：不是「绝对阈值不可用」，而是 CI 那次 `均值 0.545` **落在抖动分布之外、尚未归因**（本地 64 次与评审 30 轮重新建图都是 0.99~1.00）⇒ 把**不可归因的数值门**换成**可归因的不变式**（`k²/N = 0.5` 只说明「若退化为任意返回会是这个量级」，**不蕴含**拓扑抖动可达 0.5）。重合率降级为**诊断打印**（先打印、后断言），#34 保持 open。<br>⚠️ 曾考虑但**不采用**「全量检索 id 集合 == 全量 id 集合」：实测 `top_n(N)` 随拓扑在 198~200 间跳（因 `level ≥ 1` 的点不在 layer 0），**该不变式本身会 flaky**。变异测试：灌错向量（200/200 自匹配失败）与只灌前 30 篇（图点数 30 ≠ 200）都能变红 |
...
| **S2-T13** | 并行建图（若 D-S2-05 纳入） | `add_batch` 并行路径与串行路径的 oracle 重合率差 **< 5 个百分点**（⚠️ 评审期定 1pp，实测在 `--features charabia` 下并行相对串行有 0~2pp 固有抖动（C8 插入顺序不确定），1pp 会让 CI 间歇性红，见 §5.5）；阈值以下的小批量自动回落串行 |
...
1. ~~`parallel_insert` 的实测收益未知……是否值得为它引入「建图拓扑不可复现」这一永久性代价？~~
   **✅ 已出数据并收敛（2026-09-07，`eval-report.md` §8.6）**：12K 真实语料 11.676s → 2.182s
   （**5.35×**），50K 合成向量 171.8s → 32.7s（5.26×）——加速比稳定在 ≈5.3× 且与规模无关；
   真实语料上 oracle 重合率**无差异**（0.995 / 0.995）。
   ⇒ **建议把默认值翻为「开」**（5.3× 加速 + 质量无损 + NFR-06 不受影响），
   **但需独立 PR**：`S2-T22` 当前断言「默认必须串行」，翻默认值必须同步改这条护栏语义。
...
| 5 | 旧快照可加载 | ✅ S2-T6 覆盖。**oracle 断言是评审后补的**（#14 发现 1）：原实现只断言「topk 非空」，而 §8 对 T6 的口径本就是「与 oracle 的重合率」。⚠️ 落地后口径**三次**修正：①「单 query ≥ 0.95」→「20 query 均值 ≥ 0.90 + 最差 ≥ 0.5」（charabia 下 flaky）；②「均值重合率」→「自匹配不变式」（**issue #34**）；③「20 篇自匹配」→「**全部 N 篇自匹配 + 结构层内容覆盖**」（评审实测的覆盖盲区：只查 20 篇时「重建只灌了前 30 篇」也全绿）。**#34 的根因仍未定位**（CI 那次 0.545 在抖动分布之外），本 PR 换的是**可归因**的门，不是「已修 flaky」。见 §8 S2-T6 行 |
