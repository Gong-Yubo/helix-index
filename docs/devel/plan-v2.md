...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.7（2026-09-08：Step 3 已合并 `ed25d5c` 状态校正 + Step 4 详细设计已出（v0.2 评审回应版），待拍板）** |
| 日期 | 2026-09-08 |
| 状态 | **决策已定案（D-J1~J7 + D-J8~J11）；Step 1 / Step 2 / Step 3 已完成并合并进 `main`（Step 3 = `ed25d5c`）；Step 4 详细设计已出（`v2-step4-design.md` **v0.2，PR #28 评审回应版**，D-S4-01~10 待拍板），实现待开工；任务编号按 2026-09-07 复审重排** |
...
- **详细设计**：🟨 **已出，待评审**（`docs/devel/v2-step4-design.md` **v0.1**，2026-09-08）。
  ...
  9 个待拍板决策 D-S4-01~09；任务拆分 S4-01~10（PR 切分：S4-01 先合 → 核心单 PR → CLI/workload → 文档回写）。
...
| **Step 4** | T7-12 墓碑物理回收（compaction，含 manifest 重发） | 🟨 **设计 v0.2（PR #28 评审回应版——新增 D-S4-10「`compact()` 与写缓冲 `pending` 的交互」，采纳「先 `commit()`」修法 + I8/T13；验收 3 `nb_point` 口径改正）**；D-S4-01~10 待拍板，实现未开工 |
