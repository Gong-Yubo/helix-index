...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.7（2026-09-08：Step 3 已合并 `ed25d5c` 状态校正 + Step 4 详细设计已出，待评审）** |
| 日期 | 2026-09-08 |
| 状态 | **决策已定案（D-J1~J7 + D-J8~J11）；Step 1 / Step 2 / Step 3 已完成并合并进 `main`（Step 3 = `ed25d5c`）；Step 4 详细设计已出（`v2-step4-design.md` v0.1，D-S4-01~09 待拍板），实现待开工；任务编号按 2026-09-07 复审重排** |
...
| **Step 4** | T7-12 墓碑物理回收（compaction，含 manifest 重发） | 🟨 详细设计已出（`v2-step4-design.md` v0.1，待评审；D-S4-01~09 待拍板），实现未开工 |
