...
| 「`TextRerank` API 已核实可用」 | ✅ 对（`fastembed` 的 `TextRerank::try_new` —— `src/reranking/impl.rs`；v6.0.2 下 `:126-132`）；⚠️ 但 `rerank()` **返回全量排序结果**（`src/reranking/impl.rs` 的 `TextRerank::rerank`，v6.0.2 下 `:215-224`。⚠️ **2026-09-16 更正**：原文写「stable sort ⇒ 同分保持输入序」——**本项目的实装刻意不沿用**该行为，改用 **`chunk_id` 升序**（D-S7-07，`crates/core/src/rerank/scoring.rs` 的 `apply_scores`）；「返回全量排序结果」这一条不变）⇒ 需自行 `take(top_n)` 并按 `RerankResult.index` **回填** |
