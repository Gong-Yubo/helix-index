...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.13（2026-09-11：Step 6 详细设计**已出**——新增 `v2-step6-design.md` v0.1（D-S6-01~09 待评审）；同时**清掉上一批遗留的陈旧状态行**：Step 5 收尾 PR #37 已并入（main **`590315d`**）、Step 3「PR #27 待合并」改为已合并 `ed25d5c`、Step 5 进度表两行由 ⬜ 改 ✅）** |
| 日期 | 2026-09-11 |
| 状态 | **决策已定案（D-J1~J7 + D-J8~J11）；Step 1 / Step 2 / Step 3 / Step 4 / Step 5 均已完成并合并进 `main`（main 现为 **`590315d`**；Step 3 = `ed25d5c`；Step 4 = S4-01 `1efc128` + 核心 `6f34ef2` + CLI/workload `ac9fcbe` + 文档回写 `9e2211e`；Step 5 = 设计 `ee42b34` + 实现 `48c0ac7` + 收尾 `590315d`，issue #22 已结案）；**Step 6 详细设计已出、实现未开工**（`v2-step6-design.md` v0.1，**D-S6-01~09 待评审**；其中 D-S6-01「T7-09 是否投 M 级」由 E1/E2/E3 spike 数据回答，D-S6-08「NFR-10 数值目标」需评审拍板）；Step 7 待做。任务编号按 2026-09-07 复审重排** |
...
> **状态（2026-09-11 更新）：🟩 详细设计已出、实现未开工**（`docs/devel/v2-step6-design.md` **v0.1**，
> issue **#2**，D-S6-01~09 **待评审**）。
>
...
> **未决需评审拍板**：**D-S6-08 / Q4 —— NFR-10 的数值目标缺失**（全文无数字 ⇒ 今天无法判定达标）；**Q5 —— 「追加 10%」以什么为单位**。
...
- [ ] **Step 6**：**NFR-03 按双口径**重测达标（首次全量 < 240s ✅ 已达标 / **增量追加待实现**）；NFR-05 内存重测记录；`Searcher` 跨线程并发读吞吐 + 正确性有数据。⚠️ **开工前置**：详细设计已出（`v2-step6-design.md` v0.1），**D-S6-01~09 待评审**——尤其 **D-S6-08 / Q4（NFR-10 数值目标缺失）** 拍板前，NFR-10 无法判定达标
...
| **Step 6** | T7-09 embed 并行 / T7-11 增量构建 / T7-17 并发检索压测 | 🟩 **详细设计已出、实现未开工**（`v2-step6-design.md` v0.1，2026-09-11，**D-S6-01~09 待评审**） |
...
1. `requirements-spec.md`：
   ...
   - 🆕 **Step 6 设计回写（本轮 → v1.13）**：NFR-03 ② / NFR-05 / NFR-10 / NFR-11 补「设计已出」与设计锚点；⚠️ **NFR-10 数值目标缺失**（设计 §9.2 Q4）与 **NFR-03 ② 的 10% 单位**（Q5）登记待拍板；**§1.1 版本表补齐 v1.11 / v1.12 两行**（清「定义面漂移」）。**未修订任何指标**。
