...
| 项 | 内容 |
| --- | --- |
| 版本 | **v0.15（2026-09-13：**Step 6 收尾（S6-10）——`v2-step6-design.md` 升 **v0.4****：四处定义面回写（需求 **v1.15** / 架构 **v1.14** / 本文件 / `docs/README.md`）+ **NFR-10 定稿并收窄适用范围**（② 只覆盖「不含查询侧编码的读路径」，编码侧并发登记为架构 **R43** 并**保持「打开」**）+ **NFR-05 补口径限定词**（「检索路径，batch 1」）+ **架构 §14.4 扩为 R36~R43**（新增 R41/R42/R43）+ 集成测试补 **S6-T10**；main 现为 **`8f4d08c`**（PR #44 已并入）。此前 v0.14：Step 6 设计评审收口——评审 F1~F7 全部采纳、**D-S6-05 / D-S6-08 拍板**、Q4~Q7 收口；需求随之 v1.14、架构 v1.13、main 曾为 `bb23553`）** |
| 日期 | 2026-09-13 |
| 状态 | **决策已定案（D-J1~J7 + D-J8~J11）；Step 1 ~ Step 6 均已完成并合并进 `main`**（main 现为 **`8f4d08c`**；Step 3 = `ed25d5c`；Step 4 = S4-01 `1efc128` + 核心 `6f34ef2` + CLI/workload `ac9fcbe` + 文档回写 `9e2211e`；Step 5 = 设计 `ee42b34` + 实现 `48c0ac7` + 收尾 `590315d`，issue #22 已结案；**Step 6 = 设计 PR #38 → `46ab3a4` → spike S6-01~03（PR #40）→ 增量构建 S6-06/07/09（PR #43）→ 并发采集点 S6-08（PR #44 → `8f4d08c`）→ 收尾 S6-10（本 PR）**，issue **#2**）；**Step 6 收尾结论（2026-09-13）**：**S6-04 / S6-05 判「不投」**（T7-09：E2 增益 +11.7%/+16.0% 而峰值 RSS +96.4%/+289.2%，两侧门槛都不过）；**NFR-10 已按实测定稿并收窄适用范围**（需求升 **v1.15**：② 只覆盖「不含查询侧编码的读路径」，编码侧并发登记为架构 **R43** 并**保持「打开」**）；架构升 **v1.14**（**R38 / R39 关闭**、**R40 上锁（残余仍在）** + 新增 R41/R42/R43）；Step 7 待做。任务编号按 2026-09-07 复审重排** |
