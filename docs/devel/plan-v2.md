...
- [ ] **Step 7**：Reranker 接入后 MRR@10(1) 相对**已复现基线**可测提升；精排延迟满足 NFR-12。**2026-09-14 设计已出**：基线口径**收紧**为「**同一张冻结图**上的 `--rerank off` 臂」（P5 锚点自带 ~0.6% 图漂移）；NFR-12 补为**三部分**（增量 P99 ≤ **500ms 拟** / 质量 / 降级可观测）；交付形态与 S7-T1~T12 见 `v2-step7-design.md` v0.1。⬜ **待实现**（设计 PR = 本 PR，待评审）
...
2. `architecture-design.md`：
   ...
   - 🆕 **Step 7 设计回写（本轮 → v1.15）**：**§14.5 新增 R44~R48**（R44 精排模型内存 / R45 候选窗口与 `candidate_k` 的静默夹取 / R46 组装的线性放大成本 / R47 `BatchLongest` 的数值不可复现 / R48 冻结图依赖与 `--runs > 1` 的重建陷阱）+ §14 导读补一行；**§5.7 / §5.8 补 `candidate_window` 与 `Explain.rerank_score` / `Metrics.rerank_window` / `rerank_elapsed`**。**无 trait 破坏**（`Reranker` 签名不变；`Config` **新增**字段属向后兼容的加法）。
