...
| 文档版本 | v1.13                |
| 创建日期 | 2026-09-02           |
| 状态   | **V2.0 Step 1~5 已并入 `main`**（main 现为 **`bb23553`**——Step 5 收尾 PR #37 → `590315d`；PR #39「T13 质量门 → 同图可归因不变式」→ `bb23553`，2026-09-11）；**Step 6 详细设计已出（评审已收口）、实现未开工**（`v2-step6-design.md` **v0.2**：评审 7 条意见 F1~F7 全部采纳、**D-S6-05 / D-S6-08 拍板**、Q4~Q7 收口；§14.4 R36~R40 为本 Step 引入）；Step 7 未开工。步骤编号映射见 `plan-v2.md` §附-2 |
...
> V2 Step 6（构建性能与多线程）的 **R36~R40** 见 §14.4 —— **设计期登记、其中 R36 / R37 为条件性**（取决于 embed 并行 spike 的「投 / 不投」判定），
...
#### 14.4 V2 Step 6 引入的风险（R36 ~ R40，**设计期登记，条件性**）

> **状态：设计已出、实现未开工**（`v2-step6-design.md` v0.1，2026-09-11，D-S6-01~09 待评审）。
> ⚠️ **R36 / R37 是条件性风险**——只在 E2（多 session）/ E3（CoreML EP）经 spike 判定「投」之后才实际生效。
> R38 / R39 / R40 则**今天就已存在**（只是无人上锁），无论 spike 结论如何都应处理。
> 完整触发条件与未决问题见 `v2-step6-design.md` §9。

| # | 风险 | 影响 | 应对 | 残余 |
| --- | --- | --- | --- | --- |
| **R36** | **多 session 抬高内存峰值**（E2）：N 份 ort session 的 arena / 工作区，`intra_threads` 按 session 数切分后总占用可能高于单 session | NFR-05（峰值 RSS 372MB）被顶穿；小内存机器 OOM | spike 必录峰值 RSS（S6-01）；决策门含「峰值 RSS 增量 ≤ 20%」（D-S6-01），不达标即不投 | ⏳ 若判定「不投」则风险消失；若「投」须重测 NFR-05 |
| **R37** | **E3（CoreML EP）改变向量数值**：EP 改变算子实现 ⇒ 浮点归约顺序变 | ① 所有既有效果指标（MRR / NDCG）与 P5 基线**不可横比**；② 快照的"语义"变了（同模型不同 EP ⇒ 向量不同） | ① D-S6-04：把 EP 编码进 `embedder_id` ⇒ CPU 建库 / CoreML 加载报 `ConfigMismatch` 而非静默错配；② 设计 §3.5 要求**显式声明不可横比**；③ E3 落地单独立决策（D-S6-01） | ⚠️ **声明是缓解不是消除**：若采纳，P5 的效果基线需重跑（属 Step 7 的前置"复现基线"的一部分） |
| **R38** | **`default_embedder()` 静默降级**（`search/config.rs:312-318` 的 `.ok()`）：`LocalEmbedder::new()` 失败 ⇒ `None` ⇒ 索引变成**纯 BM25**，**无错误、无警告** | 静默的功能缺失——最坏的一类（"要了向量却拿到纯文本"，且不知道坏了） | D-S6-05：`--vectors` 显式请求向量时**硬失败**（方案 A）；测试 S6-T7 断言"要么 `Err`、要么可观测标志为真，绝不静默" | ⏳ D-S6-05 待评审；**本 Step 内可关闭** |
| **R39** | **追加路径漏发图 manifest**：`build --index` 追加产生新点 ⇒ 图 sidecar 的 `nb_point` / `graph_crc` 全变；若 save 链路未重发，则新图永远匹配不上（**与 Step 4 的同一坑**） | 每次冷启动都走降级重建（≈10s），NFR-04 **静默失效** | S6-T13 显式断言：追加后冷启动 `GraphStatus == Loaded`（非 `Rebuilt`）且 manifest `nb_point` 与新增 chunk 数吻合 | ⏳ 待实现；**本 Step 内可关闭** |
| **R40** | **ID 复用 ⇒ 幽灵向量**：`raw_vectors` / 图 sidecar 按 `chunk_id` 索引；若追加时新 ID 与历史 ID 重叠，旧向量会挂到新 chunk 上（正确性缺陷，与 Q-C1 同族但**成因相反**） | 静默的错误召回 | 设计 §4.5.2 给出"今天不冲突"的证明（`insert_*` 用槽位长度分配 + `export/import` 保留尾部空洞）；S6-T8 写成**不变式测试**锁住 | ⚠️ 证明依赖三条隐含前提（尾部空洞保留 / 导入不压缩 / compaction 同步重写 `raw_vectors`）⇒ **靠测试锁，不靠文档** |
