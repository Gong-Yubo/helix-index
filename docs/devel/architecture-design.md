...
| **R51** | **向量侧的合并成本与拓扑不确定**：`hnsw_rs` **无图合并 API** ⇒ 只能「dump 旧主图 → load → 增量 `insert` delta 点」（⚠️ 带出 **R23** 的 `Box::leak` + **R22** 的 1.6× 体积 + 临时 I/O）或「从 `raw_vectors` **全量重建**」（ADR-A 允许，≈ **38.7s/12K**） | 若「dump→load→insert」不可行而必须走全量重建 ⇒ **小 delta 也要付 O(N·logN)** ⇒ 合并频率被迫降到很低 ⇒ **delta 段堆积 ⇒ 读路径随段数变慢**（FR-17 的收益被吃掉） | ① spike **S8-S1** 的决策门（设计 §4.9.3）；② `MergeReport.vector_strategy` **如实记录走了哪条**（**不得静默降级**） | ⏳ **开放（等 spike）**；⚠️ **2026-09-17（#60 第 1 轮评审 P3-5）补两条**：① **累积性** —— 方案 A 每次合并都 `load_graph` ⇒ **每次都多一个 leaked `HnswIo`**，而**后台异步合并是 FR-17 的常态路径**（不是边角场景）⇒ 长驻宿主（`helix serve` / `build --watch`）会累积；② **量级（评审的量级前提经独立复算后更正）** —— leaked 的**只有 `HnswIo`**（`vector/persist.rs:210-216` 的 P1-2 注释逐字：**每次加载约 200B + 路径串**；向量数据由 `Hnsw<'static>` **自持**、随 `Segment` 释放**可回收**）⇒ **不是图规模**；10 次合并 ≈ 2 KB（RSS 噪声级）。S8-S1 的决策门**第 ⑤ 条据此把阈值定为「连续 10 次合并 RSS 增量 < 1 MiB」**（其鉴别力 = 区分「只泄漏 `HnswIo`」与「误泄漏整张图」；若按「相对增量 ≤20%」写会**必然假通过**）。⚠️ **非对称**：方案 B 是内存重建、不 `load` ⇒ **不新增泄漏**；但 B 的「更省」只在「图本身」这一项上成立 |
