...
> 📌 **锚点写法（本步定的规则，后续步骤沿用）**：**引用本仓源码一律用「符号指代」**（`文件` + 符号名，如「`bench.rs` 的 `--single-chunk` 处注释」），**不写行号** —— Step 7 的三次实现 PR **每次都让定义面的行号锚点漂移**（S7-03 一次 10 处、S7-04 一次 5 处），改成符号后从根上消掉这一类。**依赖源码**（`fastembed` / `hnsw_rs`）可**两者都写**：符号名 + 「v 版本下 `:行号`」的**带版本锚点**（依赖版本锁死 ⇒ 不会悄悄漂）。
