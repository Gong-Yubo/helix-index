...
| [`devel/v2-step9-design.md`](./devel/v2-step9-design.md)（V2 Step 9 设计） | **相关性深化（自适应融合 + 评测资产纪律）（T7-14 / T7-15 / T7-16 / issue #4 / FR-35 / NFR-15）**：决策 **D-S9-01 ~ D-S9-10**、验收 **S9-1a~d / S9-2 / S9-3**、测试 **S9-T1 ~ S9-T12**、风险 **R55 ~ R58**（→ 架构 **§14.7**）。🔴 **设计期 5 条新发现**：**桶标签含 qrels ⇒ 标签泄漏**（`t2_prep.rs:206-230`）/ **`fuse` 无查询上下文 ⇒ FR-35 结构性做不到**（取 **provided 方法**）/ **架构 §5.6 伪码与实现漂移** / **`bench --modes` 表达不了同轮 A/B** / **80 条桶的样本量约束**（⇒ 只允许「零参数 / 单阈值」+ **区间达标**）。⚠️ **PR 切分 4 段 + 收尾**，`PR9-1` **默认关 ⇒ 零行为变化**，`PR9-2` 含 **spike S9-S1**（**不预先承诺收益**）。 🆕 **v0.3（2026-09-24，已过三轮评审 ⇒ 可开工）**：第 3 轮补三处（架构行号重取 / 附录 B 的 bash 变量改 ASCII / **`Metrics` 填充路径闭合**）；v0.2 已补 **P3-1** —— `weights_override` 只定义「决策」 ⇒ 再加一个 provided 方法 **`fuse_adaptive`** 闭合「应用」通道（默认 = `self.fuse` ⇒ 零改动）； + 六条 P4（θ 网格预注册 / 锚点双钉 / 容差口径等）；**跟踪 issue #71**。 | 实现者（**纯设计、`.rs` 零改动**） |
