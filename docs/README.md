...
| [`devel/v2-step3-design.md`](./devel/v2-step3-design.md)（V2 Step 3 设计） | **原子快照（崩溃一致性）**：`atomic_write` 通用原语（快照与 manifest 共用）、save 全序列崩溃窗口矩阵、tmp 孤儿回收、故障注入测试钩子、R19 写路径残余收敛（`catch_unwind`）、D-S3-01~06 决策点、S3-T1~T10 测试计划 | 实现者、评审者（**v0.1 待评审**） |
