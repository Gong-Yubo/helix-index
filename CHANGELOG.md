...
- **交付物 ①（S9-04）**：`scripts/eval_s9.sh` —— 预注册常量（θ 网格 21 点 / G1~G4 阈值）+ 64 臂
  （三路锚点 1 + 基线 1 + 3 信号 × 21 θ）+ **脚本侧按 qid 配对** + G5 逐臂自证（`k` / `rrf_k` /
  精排关 / 同一冻结图指纹）+ **内置自检**（`θ=0.00` 臂须与基线逐 query 零差）+ `readings.tsv`（头含
  冻结图指纹 ⇒ 产物自证）。**两轮全量复跑曲线逐位相同**（确定性）。
