...
### 新增 · V2 Step 8 PR 7a —— **查询侧会话池**（`S8-08` / T7-26）+ spike S8-S2 判「不投」（2026-09-21）

> ⚠️ **`Config` 新增字段 `embed_sessions`** —— `Config` **不是外部可达类型**（未从
> `helix_core::search` 再导出；全仓唯一字面构造点是 `build_config`）⇒ **对下游不构成破坏性变更**
> （同 `Q8` 的核实结论）。本轮 **零行为变化**：默认 **1**，且 `dim` / `id()` / `is_normalized()` /
> `ConfigFingerprint` **一字不变**。
> 门测试 = **`S8-T16`**（spike S8-S2 的三条**合取**判据）。读数落 `eval-report.md` **§8.15**。

#### 新增

- **`LocalEmbedder` 改为会话池**：`pool: Vec<Mutex<TextEmbedding>>` + **轮转取槽**（轮转游标 =
  一个 `AtomicUsize`，无锁、常数代价）。`embed_query` / `embed_documents` **只锁各自那一个槽**
  ⇒ 解开 `R43` 的「查询侧编码被单 session 串行化」（`String` 层的动机见 §4.11）。
- **`LocalEmbedder::new_with_sessions(n)`**（新公开构造；`new()` = `new_with_sessions(1)` ⇒
  **既有签名不变**）+ **`LocalEmbedder::sessions()`**（新公开读数）。
- **`Config::embed_sessions: usize`（默认 1）** + **`SearchIndexBuilder::embed_sessions(n)`**：
  只对**默认装配路径**生效（显式 `embedder(Some(..))` 时忽略）。
  ⚠️ **不进配置指纹** ⇒ 同一快照在 `sessions = 1` 与 `N` 下**都合法**（会话数不改变索引内容）。
  `0` 归一为 **1**（空池不可用；归一发生在 `build_config`，使 `Config.embed_sessions >= 1` 成为易守的不变式）。

#### 🔴 spike S8-S2 的结论：**判「不投」**（三条合取，见设计 §4.11.3 / §4.11.4）
...
| ② | 吞吐增益（vector 路 `QPS(4)/QPS(1)`） | **≥ 2.5×** | **2.10×**；对照**未池化 = 1.23×** | ❌ **FAIL** |
| ③ | 峰值 RSS 增量（查询侧口径） | **≤ 20%** | **+148.4%**（201 104 → 499 600 KiB） | ❌ **FAIL** |

⇒ `NFR-10` ③ 与 `R43` **保持「打开」**（如实记未达标，**不是**「只记录不判定」）；
**四处定义面回写归 PR7b（`S8-09`）** —— 本轮只把结论与读数落到设计 §4.11.4 + `eval-report.md` §8.15。

🔑 **两条给评审复核的加法**（都可独立复算）：

1. ⚠️ **判据③ 的阈值在查询侧口径下对任何 `sessions > 1` 结构性不可达**：4 会话比 1 会话多
   `298 496 KiB / 3 槽 ≈ 97 MiB / 槽`（每槽是**独立 ONNX 会话**、各持一份权重）⇒ **只加到 2 个会话**
   也约 **+50%，已越过 20% 上限**。⇒ 阈值取自**建库口径**（`D-S6-01`）与**查询侧口径**不匹配：
   查询侧基线只有 201 MiB（其中约一半就是那一个会话的权重）⇒ **相对阈值 + 小基数 = 结构性不可达**
   （同族：`#60` 评审 P3-5 的「相对阈值 + 小量级 = 空转门」）。**本轮不擅自改阈值**（裁定归 `S8-09` / 评审）。
2. ⚠️ **判据②「未过」的机理尚未单独证伪**（**推断，非实测**）：最可疑处 = 每槽 `intra_threads`
   沿用 fastembed 默认（`None` ⇒ **吃满所有核**）⇒ 4 槽 × 满核线程在 10 核上**过订阅**
   （Step 6 的 E2 正是按 `ceil(核数 / sessions)` 切分）。定罪需让每槽可设 `intra_threads`
   —— 生产 `LocalEmbedder` **当前不暴露该参数**（§4.11.1 只规定「槽同构」）⇒ **本轮不做**，
   登记进**复审触发条件**。⚠️ 因此 ② 的 FAIL **不能**读成「池化无效」：`1.23× → 2.10×` 说明**确有增益**。

🔑 **判据① 比设计预期更强**：①c 的**非生产路径**对照显示 **`intra_threads` 也**不**改变数值**
（`10` vs `2` 的 `embed` 逐位相同）⇒ `Q3′` 的**两个子问题都有答案**（实例数不变、`intra_threads` 不变）。
⚠️ **样本边界**：①c 是**单观测**（一对取值 × 12 条短文本 × 1 次）⇒ 不可外推成「任意组合都逐位一致」。

#### 测试

- **无模型单测（进 CI）**：`S8_08_轮转取槽把并发摊到不同槽且单槽时恒为0`（**期望序列刻意不是全 0**
  —— 恒 0 = 退化成单槽时本用例必红，这是「池化还在但不轮转」的唯一绊线；另含「40 次取槽均匀落在 4 槽」）、
  `S8_08_会话数零归一为一`、`S8_08_builder的会话数默认一且零归一为一`、
  `S8_08_会话数不进配置指纹`、`S8_08_会话数必须真的到达构造器且默认为一`（**记录型 ctor**：
  只看「结果对不对」抓不到漏传 ⇒ 用 `AtomicUsize` 记录 ctor 实收值，把**接线本身**变成断言）。
- **真模型判据（`#[ignore]`，本地按需跑）**：`S8_T16_判据一_会话数不改变向量数值`、
  `S8_T16_判据一_同实例内四个槽互相一致`。

#### 变异自证（4 组，逐组已还原；每组**只**打红目标判据）

| 变异 | 手法 | 命中判据 |
| --- | --- | --- |
| `m-s8s08-a` | `slot_index` **恒返回 0**（池化名存实亡） | `S8_08_轮转取槽…` ✅（其余 11 条绿） |
| `m-s8s08-b` | `normalize_sessions` 退化为恒等 | `S8_08_builder的会话数默认一且零归一为一` + 接线判据 ③ ✅（**2 条**） |
| `m-s8s08-c` | 装配链路吞掉会话数（`default_embedder(…, ctor)` → `(1, ctor)`） | `S8_08_会话数必须真的到达构造器且默认为一` ✅ |
| `m-s8s08-d` | 指纹的 `dim` 混入 `embed_sessions` | `S8_08_会话数不进配置指纹` ✅ |

🔴 **`m-s8s08-c` 首轮「零命中」，本轮据此**改了实现取形**（值得留痕）**：第一版接线判据
**直接调** `resolve_embedder(...)` ⇒ 只能证明「形参往下转交」，**绕过了**
`build_config → default_embedder` 那一段 ⇒ 会话数写死成 `1` 也**全部单测照样绿**。
⇒ 新增 `build_config_with(ctor)` 接缝（生产入口 `build_config` 注入真实构造器），判据改从
**装配那一层**进 ⇒ 同一变异当场变红。
⚠️ 为何特别值得记：漏传的后果是**静默退回单槽**，而「单槽」**就是默认的合法配置**
⇒ **行为判据永远看不见它**；只有「断言构造器**实收了什么**」才看得见。

#### 🔴 CI 抓到的一处**我自己的用例缺陷**（已修，值得留痕）

- `S8_08_会话数不进配置指纹` 首轮 CI（run `35603322668` 的 `feature isolation` / **只有 `charabia` 这步**）红：
  它用 `build_config()`（**生产入口**）装配 ⇒ 调**真实**构造器 ⇒ **在 CI 里会联网取模型**；
  「一次构造失败、一次成功」⇒ 两个 `Config` 的**唯一变量变成「有没有模型」而不是「会话数」**
  ⇒ 断言比的是 `None` vs `Some`（本地因模型已缓存而侥幸绿）。
- **修法**：改用**注入接缝** `build_config_with(ok_ctor)`（不碰 ONNX / 网络）+ 一条**前提断言**
  （两侧都必须真的拿到 embedder ⇒ 自证「差异只可能来自会话数」）；同类**两条**用例一并改。
- **对照验证**：在**无模型缓存的 `HOME`** 下跑同一批单测 ⇒ **10 passed / 0 failed**（证明不再接触网络）。
- ⚠️ **顺带观察（不属本 PR、未改）**：既有的 `默认配置零配置可用` 同样走 `build_config()`
  ⇒ 无缓存环境下花 **~85 s** 尝试取模型并失败（这也解释了 CI 的 `charabia` 步骤为何 **69 s**）；
  它的断言是**恒真**的 ⇒ 实际内容只有「不 panic」。

#### 覆盖边界（本轮**明确不声称**守住的）

- **判据②的量级依赖核数**（10 核上的过订阅程度与其它核数不同）⇒ 换机需重测；**未测 Linux / CI**。
- **未测建库侧口径**（§8.15 的 RSS **刻意**是**查询侧**；建库侧见 §8.10 / `R41`）。
- **①c 是单观测**；**`LocalEmbedder` 不给 `intra_threads`** ⇒ 判据②的机理方向本轮无法单独证伪。
- 🔵 **CLI 不加 `--embed-sessions`**：`Q4` 只说「库侧默认不动」，CLI 侧的**口径与推荐值**待 spike 结论
  ⇒ 本轮结论是「不投」，不加开关更自洽；**库侧两条公开入口已可用**
  （`SearchIndexBuilder::embed_sessions(n)` / `LocalEmbedder::new_with_sessions(n)`）。
- 🔵 **`S8-T16` 的落点与设计原文不同**：改为**新建** `examples/spike_s8s2.rs` + **新建**
  `scripts/eval_s8s2.sh`（**不是**「`bench_embed_session.rs` 扩展」）—— 两套协议的口径不同，理由见设计 §4.11.4 的偏差表 #1。
- 🔵 **PR7 拆为 PR7a / PR7b**（用户 2026-09-21 拍板）；`PR7b` = `S8-09`（新建
  `scripts/eval_rw_concurrency.sh` + `S8-T11`/`T12`/`T15` + `NFR-14`/`NFR-11` 定稿 + 四处定义面回写）。

### 修复 · V2 Step 8 PR 6 —— `S8-06` **第 1 轮评审响应**（`1×P1` 阻塞 + `1×P2` + `1×P3` + `4×P4`）（2026-09-21）

> ⚠️ **无 `Breaking`**（公开面签名一字未改；`MergeReport` 的字段集也不变 —— 变的只是
> `tombstones_applied` 的**口径**，而**新口径才是它文档里承诺的那个**）。
> 评审 = `pulls/65/reviews` 1 条（`5262508481`，SHA `66f87fb`）+ 行内 4 条；**七条全部采纳**；
> 评审列出的「本轮无异议」清单（spike 方法论 / 「三线程 → 两线程」的论证 /
> `VectorMergeStrategy::None` / `S8-T13` battery / 门测试 21+5 全绿）**逐条签收 ✅**。
> 设计文档升 **v0.12**（新增 **§4.9.8**）。

#### 修复

- 🔴 **`merge_pending()` 的部分合并会丢弃指向「未合并段」的跨段墓碑 ⇒ 已删文档复活**
  （评审 **P1-1**，**阻塞**）。根因是两处叠加：① `SearchIndex::remove` 的**分支②**对
  **任何既往段**（含**还没合并**的 delta）都记墓碑；② 原 `merge_n` 把**全量** `View.tombstones`
  交给 `absorb_window` 摘除，而 `Index::remove` 对**越界 `doc_id` 是静默 no-op**
  ⇒ 出现「**被摘掉、却没被物理化**」⇒ 检索期再无东西挡它，继续合并还会把它**固化进主段**
  并随 `save` 落盘（**删除永久丢失**）。**修法**：新增 `Tombstones::below(upper)`，
  判据 = **本次合并覆盖到的 doc 槽位区间**（`upper` = 物理化前主段的 doc 槽位上界；FIFO 下
  等于 `deltas[take].base_doc`）；区间外的墓碑**原样保留**。同时 `MergeReport.tombstones_applied`
  改计**真物理化**数（修前实测**谎报 1**、真值 0 ⇒ 违反 `NFR-07`）。
- 🟠 **零向量增量段触发向量侧全量重建**（评审 **P2-1**）：`merge_vectors_incremental` 原对
  `delta_raw.is_empty()` 返回 `Ok(None)` ⇒ 调用方**全量重建主图**（`O(N·logN)` + 一次
  `OsRng` 拓扑重随机）；而零增量**根本没有向量侧工作**（主图原样正确 —— 死点由检索期的存活位图
  挡掉，`Q-C1` 既有机制）⇒ 改走方案 A 的 `dump→load` **零插入**，`vector_strategy` 如实记
  `Incremental`。⚠️ 评审指出的类型边界如实记录：`Segment.vector_index` 是 `Box`（非 `Clone`）
  ⇒「直接复用主图、零 I/O」拿不到所有权。
- 🟠 **文本侧同类一并修**（同 **P2-1**；原登记于 `S8-03` 评审 **P4-1**）：`merge_n` 对
  **两个槽位都为 0** 的增量段**跳过 `merge_from`** —— 它的末步是全量 `field_index.rebuild`，
  对零内容段是白付的一 pass。**等价性**：`append_from` 无内容、`stats` / `chunk_lens` 各加 0、
  `content_hashes` 无条目、`base_*` 不变 ⇒ **逐位等价**。
- 🟡 **`S8_T10` 读端的记忆序窗口**（评审 **P3-1**）：写端 `commit()` **之后**才
  `committed.store(i + 1)` ⇒ 它若在这两条之间被抢占，读端会以 `i < i` **误报失败**
  （而本条是**验收标准 5 的唯一判据** ⇒ 红了会把人引向错误方向）⇒ 判失败前**宽限重读 50ms**。
  「检测能力不缩水」由**三臂探针**证明（见下）。

#### 变更（公开面语义细化，非破坏）

- **`merge_pending()` 的 `Err(Busy)` 语义写进 rustdoc**（评审 **P4-3**）：两种成因（窗口内另一
  写端 `compact()` 过 / `fold` 过**同一个 `Arc<Shared>`**）、恢复动作 = **重试本次合并**
  （同 `PublishCaller::Fold` 口径）、以及宿主循环的建议形状（⚠️ `Busy` 重试要**有界**）。
- `merge_n` 的**错误不变式注释删除**（评审 **P4-4**）：原文「FIFO 保证墓碑的目标段必已先被合入」
  是**错的**，换成真判据（doc 区间）与它的失效条件。

#### 测试

- **新增** `S8_06_部分合并不得丢弃指向未合并段的墓碑`（P1-1 回归锁；**含 `save`→`load` 臂** ——
  前三步只证明「视图里还挡得住」，**落盘臂**才钉住「**固化**」那一层）。
- **新增** `S8_06_墓碑only段的合并不触发向量全量重建`（P2-1；**两条判据缺一不可**：
  ① 策略 = `Incremental`；② **保拓扑是真的** —— 合并后向量检索仍能命中内容、且被删 doc 仍不可见）。
- **新增单测** `S8_06_墓碑可按合并区间切分且边界是严格小于`（把边界 `doc < upper` **逐值钉死**；
  写成 `<=` 会在「墓碑目标 = 下一个未合并段的**首个** doc」这一格上放行 = 复活）。
- `step8_rw_concurrency.rs`：import 收敛为 `Error` + 删除 `let _: Result<()> = Ok(());` 占位（评审 **P4-2**）。

#### 变异验证（**5 组，逐组已还原**；每组**只**打红目标那一条用例）

| 注入 | 期望命中 | 实测 |
| --- | --- | --- |
| `physicalized` 退回**全量**墓碑（= 修前形态） | P1-1 用例 | ✅ 红（`step8_segments.rs:1619`，`left: 1 / right: 0`）；**其余 22 条全绿** |
| 行为不动、**计数**改报全量墓碑数 | P1-1 用例的第 2 条断言 | ✅ 红（`:1624`，`left: 1 / right: 0`） |
| 零增量**退回** `Ok(None)`（= 修前形态） | P2-1 用例 | ✅ 红（`:1720`，`left: Rebuild / right: Incremental`） |
| 走 A 但**交回一张空图**（策略仍是 `Incremental`） | P2-1 的**「保拓扑」**断言 | ✅ 红（`:1735`）⇒ **策略断言抓不到它，这条有独立牙齿** |
| `Tombstones::below` 的 `<` 改成 `<=` | 新单测 | ✅ 红（`view.rs:1278`）；其余 9 条单测全绿 |

#### 🔴 顺带：**我自己的 markdown 检查器漏报了 P4-1**（已修 + 补对照样本）

P4-1 的形态是「表格行**行尾的竖线与后三格一起丢**」。我的结构检查器要求**同时**以竖线开头
**和**结尾才算 table 行 ⇒ 这种行被**整个当非表格行丢掉** ⇒ 对设计文档报 **0 处**（**漏报**）。
⇒ 判据改为「以竖线开头即算 table 行；缺尾按补齐归一后再数单元格」，并补**两组对照样本**：
① 「行尾竖线与后几格一起丢」必须**红**；② 「**整表**都省略尾竖线」是**合法**写法、必须**静默**。
⚠️ 该判据同时在全仓扫出 `docs/devel/thirdparty.md` **3 处**同类形态；经「**比内容不比行号**」
核对 = `HEAD` 与 `origin/main` **都是 3 处** ⇒ **非本 PR 引入**，且本 PR **一行未动**该文件
⇒ **如实登记、不擅自改**（同 #59 / #60 对「`main` 上既有问题」的处置先例）。

#### 覆盖边界（本轮**明确不覆盖**的）

- **`Metrics.segments` 被零内容段膨胀**（`S8-03` 评审 P4-1 的次生效应①）**有意保留**：
  入列的**理由**（跨段墓碑只能搭一次发布的便车进 `View.tombstones`）没有替代方案；
  膨胀**有界且自愈**（宿主按 `deltas.len() > K` 合并 ⇒ 合并后段数回落），且经上面两条修复后
  **消化这类段的代价已降到最低**。
- **`Box::leak` 的 `HnswIo` 泄漏**（≈200B + 路径串 / 每次合并）不变（`R41` 保持开放）。
- 本轮**不改任何决策（`D-S8-01` ~ 12）、不改需求 / 架构口径、不动风险编号** ⇒
  四处定义面只需**设计文档** + **本文件**。
