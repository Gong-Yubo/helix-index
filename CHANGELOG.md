...
- **R58 的显式开关**：真跑需 `HELIX_LLM_ALLOW_EGRESS=1`，且**先打印**「将外发多少条 / 端点 host / 字符量级」。
- **S9-T12 的载体 = `--self-test`**（**离线桩、零网络**）：**28 项断言**覆盖 解析 / 落盘 / 配对 + 7 个负例
  （候选外文档、空 query、`grade=4`、非整数 grade、缺数组、无 JSON、漏标候选）。⚠️ 设计 §7 把 S9-T12
  列在「测试计划」里（读起来像 Rust 用例）⇒ **偏差已登记**（设计 §4.16 ③ 偏差 1）。
...
- **资产面**：T7-16 的 `type` 固定 `"agent"`，**不冒充四桶**（`bench` 的分桶表是固定四桶 ⇒ 本资产四桶行恒 `n=0`；
  ...
  （`.gitignore` 新增规则；**可重建**），入库的是**读数 + 来源指纹**（`asset-meta.json` 里记两个输入的 sha256 前 16）。
