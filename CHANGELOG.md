...
- **`helix bench --threads N[,N...]`：T7-17 / NFR-10 的采集点。**
  ...
  - 口径（设计 §4.6.2 / D-S6-09）：**先跑一趟预热扫描并丢弃**再测第二趟；QPS 用**进程内**墙钟
    （spawn→join），不含进程启动与快照加载。⚠️ 并发下 per-query 延迟**含排队**，不得与
    NFR-02（单线程口径）横比。
  - **正确性是前置条件**：所有档位、所有 worker 的 `hits`（`chunk_id` + `score`）签名必须**完全相同**，
    否则直接 `Err` 且**不打印表格**（不给「看起来正常」的数字）。
...
- **前置条件 ✅ 全过**：三模式 × 四档位的 `hits` 签名**逐位一致**（bench 在不一致时已直接 `Err`）。
...
- `crates/cli/src/bench.rs` 单测 **+3**（9 → 12）：`queryexecutor可跨线程共享`（编译期 `Sync` 断言）、
  `并发档位解析`（含「单档 1 ⇒ 不跑阶段」= S6-T12 的**结构性**保证）、
  `并发检索结果逐位一致`（S6-T11：同档内一致 + 跨档一致 + **与手写朴素单线程循环一致**）。
- CI `CLI smoke` **+1 步** `bench --threads 1 与默认一致`：`--no-latency` + 屏蔽计时行后**逐字比对**，
  并**反向自证**含 >1 档时必出现并发阶段。**走 BM25 ⇒ 不需要模型**，可进 CI。
- **变异验证 2 次**（均已还原并复跑确认）：① 签名丢掉 `chunk_id` ⇒ 打中**独立性**断言；
  ② 并发档少跑一条 query ⇒ 打中**跨档一致**断言。
...
- **拟登记 R43**：向量/融合路径的并发吞吐被查询侧编码的**单 session 互斥**封顶（实测 t=4→8 零增益）。
