...
### 文档 · V2 Step 9 **PR9-3 第 1 轮评审响应**（设计 v0.10 → **v0.11**，PR #74）（Refs #71，2026-09-26）

第 1 轮评审（`pulls/74/reviews` `5326169683`，基线 `9eeb1dc`，**独立验证型复审** + **7 条行内**）=
**✅ 通过 —— 无阻塞项、达到可合并形态（0×P1/P2 + 1×P3 + 5×P4）**。评审的主验证 = **自己把 T7-15 整链
重跑**（`REBUILD=1`：抽取 → 建对照快照 → 两臂 → 配对）+ 独立运行 S9-T12 桩自检 ⇒ `bm25` 两臂
**10 位小数逐位复现**、主基线三模逐位复现。**逐条全部采纳**（P4-2 / P4-3 采纳了比「加注明」更强的
「修实现 + 补判据」）。机制面零改动；**零 `.rs` 改动**。

- 🔴 **P3-1（本轮最有价值）**：对照臂 `vector`/`hybrid` 读数**带建图抖动**（两次独立建图：`mixed` 桶 Δ
  **符号翻转**、`natural` 差 0.005、逐 query 计数差 1），全局 Δ 低于本项目自测的跨图抖动带宽
  ⇒ **方向不可归因，可归因的只有 `bm25`**。⚠️ **一处事实更正**：我方第三次建图复核后，「`mixed` 桶 Δ
  符号翻转」**不成立** —— 其自列两数（`−0.0063` / `−0.0131`）**都是负的**，实为**幅度差 2 倍**；
  精确表述 = 「**符号 3/3 一致、幅度不稳（可差 2~3.4 倍）**」（全表见设计 §4.17.1）。处置 = 解读边界补到三处（设计 §4.16.5「读数能读到多细」/
  §4.16.6「引用前必读」+ 分布行限定 / §4.16.7）+ **`readings.tsv` 头加「ctrl 图为无种子新建」**
  （脚本同步 ⇒ 未来每次产出自带该提示）。⚠️ **出处更正**：评审引「`eval-report` §3.4 = −0.586%」，
  核对后 §3.4 是**跨图 NDCG 极差 ≤0.0024**，而 `−0.586%` 出自 `v2-step7-design.md:138` ⇒ 两处分别写清。
- **P4-1**：负例计数三处口径不一（docstring `8` / 设计·正文 `7` / **实际 `10`**）⇒ **复核属实**；
  处置不止改数 —— `--self-test` **自报**总数与负例分组计数（现 `33/33`，负例 **12** 项），
  5 处载体一律照抄该行 ⇒ **结构上不会再漂**。
- **P4-2**：外发量估算口径不符（前 n 篇全文长 vs 哈希秩最小 n 篇 + 截断 1200 字）⇒ 抽常量
  `EGRESS_DOC_CHARS`（单一来源）+ `egress_guard` 改吃**已抽样的 `picked`**；**新增判据**：守卫必须报
  **截断后**的字节量（1700 字文档证明它报 `1200`）。
- **P4-3**：`sample_docs` 的 `exclude` 未接线 ⇒ **接线**：`--exclude-qrels`（默认 `data/t2-queries.jsonl`）
  + 显式 `--no-exclude-qrels`，结果进 `gen-meta.json` 的 `sampling.exclude_qrels`；**新增端到端判据**
  （走 `main()` 的 `--stub` 完整离线链路）—— 顺带**闭合了评审登记的那条边界**「未验证 `--stub` 端到端」。
- **P4-4**：中位取上中位 ⇒ 改**偶数 n 两中值均值**（本数据值不变，但 TSV 里持久化的口径写对）。
- **P4-5**：`REBUILD=1` 重写已入库的 `asset-meta.json`（只差 `built_at`）⇒ **分离运行产物与入库记录**；
  **实跑验证**：全量重装后该文件 `git status` **零改动**。
- 设计 **v0.11**（§4.17 逐条处置 + §4.16 三处更新）；CHANGELOG 本条目。

### 评测 · V2 Step 9 **PR9-3**：T7-16 生成通道 + T7-15 假负例对照（S9-06，**只作对照**）（Refs #71，2026-09-26）

按 `v2-step9-design.md` **§4.7** 实现两个**只作对照**的评测资产（实现期定形见设计 **§4.16**，v0.9 → **v0.10**）。
**零 `.rs` 改动**（评测资产工具 = 脚本，同 PR9-2 的口径）。前置：**R58（语料外发）已由用户 2026-09-26 拍板批准**。

- **T7-16 生成器（新增）`scripts/gen_t7_16.py`**（Python 标准库，**零新增依赖**）：
  `D-S9-07` 的 provider-agnostic 通道（`HELIX_LLM_BASE_URL` / `HELIX_LLM_API_KEY` / `HELIX_LLM_MODEL` +
  OpenAI 兼容 `POST /chat/completions`；**密钥不入库**，元信息只记端点 **host**）；
  **多轮 + 独立自查**（**Q9-4 定形**）：轮 1 对每篇候选段落生成 query，轮 2 **独立**判定各段 `grade ∈ 0..3`，
  **生成源段 grade ≥ 1 才保留**（否则丢弃 ⇒ 通过率进元信息）——把「源段必相关」从**公理**变成**可被拒绝的判定**。
- **R58 的显式开关**：真跑需 `HELIX_LLM_ALLOW_EGRESS=1`，且**先打印**将外发什么（**已抽样那批**的条数 /
  每篇截断长度 / 合计字符数 / 端点 host —— v0.11 起**抽样前移到守卫之前** ⇒ 打印的数字与真正送出的
  内容**逐项一致**；第 1 轮评审 **P4-2** 修正：原稿按「前 n 篇全文长」估算，既取错子集、又漏了截断）。
- **S9-T12 的载体 = `--self-test`**（**离线桩、零网络**）：**33 项断言**，其中**负例 12 项**
  （解析 7：候选外文档 / 空 query / `grade=4` / 非整数 grade / 缺数组 / 无 JSON / 漏标候选；外发守卫 1；
  抽样与校验 4：池不足 / 空资产 / `--exclude-qrels` 文件缺失 / 该文件行缺 `relevance`）。
  ⚠️ **数字由该命令自报**（`S9-T12 离线桩自检：33/33 通过（其中负例 12 项：…）`）—— 第 1 轮评审 **P4-1**
  实测三处口径不一（docstring `8` / 设计·正文 `7` / **实际 10**）⇒ 改为**自报**，手写常量不再漂。
  ⚠️ 设计 §7 把 S9-T12 列在「测试计划」里（读起来像 Rust 用例）⇒ **偏差已登记**（设计 §4.16 ③ 偏差 1）。
- **CI 新增 job `eval-assets`（job 数 10 → 11）**，并写明它与 `ci.yml` 头部「**评测脚本故意不进 CI**」的关系
  —— 那条针对**读数类**（需 12K 段落 embed / 毫秒级 P99 不可引用），本条是**零网络、零模型、毫秒级**的纯逻辑自检。
- **T7-15 对照（新增）`scripts/eval_s9_t7_15.sh`**（一键跑：装配 + 两臂 + 按 qid 配对）：对照语料 =
  `data/t2-corpus.jsonl` 中 `source ∈ 全部 qrels 段落`的行 ⇒ **12000 → 5042 篇**（自证缺失 **0**）；
  建库与主图**逐项同口径**（`--vectors --single-chunk`），两臂**只换 `--index`**。
  读数（`data/eval/t7-15/readings.tsv`）：`bm25` **+0.0162** / `vector` **+0.0033** / `hybrid` **+0.0031**；
  hybrid 逐桶 `exact −0.0146` / `natural +0.0194` / `mixed −0.0062` / `paraphrase +0.0139`；
  **逐 query 配对 280/320（87.5%）完全不变**、中位 Δ = 0。
  🔴 **解读边界**：Δ 是「假负例多少」与「**语料规模/统计量**」的**合成效应**（`idf` / HNSW 图都变），
  **不能**读成纯假负例效应；本对照**只作对照**，对 `NFR-15` 主判据无影响。
  ⚠️ **引用前必读（第 1 轮评审 P3-1）**：对照图**每次重建**（`hnsw_rs` 无种子）⇒ `vector`/`hybrid` 的
  `mrr_ctrl` 与 Δ（**含逐桶**）的**幅度含建图抖动、不可按字面引用** —— **三次独立建图**实测
  （`mixed` `0.0062~0.0131` ≈×2、`paraphrase` `0.0139~0.0278` ≈×2、全局 `0.0031~0.0068`；
  **符号 3/3 一致**）、逐 query 计数 `280 → 279`；全局 Δ 幅度（`0.31%~0.68%`）落在本项目自测的
  跨图抖动带宽内（`eval-report.md` §3.4 极差 ≤`0.0024`；`v2-step7-design.md` §3.1+§① 的 `−0.586%`
  ⇒「<~1% 无法与噪声区分」）⇒ **方向不归因（样本仅 3 且幅度更小）**；**可归因的只有 `bm25 +0.0162`**
  （无图、**逐位**可复现）。⇒ 逐桶 Δ 是**单次建图的实现值**（`readings.tsv` 头已写明）。
- **资产面**：T7-16 的 `type` 固定 `"agent"`，**不冒充四桶**（`bench` 的分桶表是固定四桶 ⇒ 本资产四桶行恒 `n=0`；
  对照取**全局行** + `per_query` 配对）。派生资产（对照语料 10.5MB + 对照快照 ~38MB）**不入库**
  （`.gitignore` 新增规则；**可重建**），入库的是**读数 + 来源指纹**（`asset-meta.json` 里记两个输入的
  sha256 前 16；v0.11 起该文件**只放确定性字段** —— `built_at` 移入**不入库**的
  `qrels-only-corpus.build.json`，第 1 轮评审 **P4-5**：原稿使每次 `REBUILD=1` 都弄脏工作树）。
- **⚠️ 未完成项（如实登记）**：**T7-16 的真实资产未在本 PR 产出** —— 本机无 `HELIX_LLM_*`
  ⇒ 交付的是「通道 + 生成器 + 离线桩 + schema」，**真实资产待端点就绪后由同一脚本产出**；
  因此 T7-16 的对照读数在本 PR **为空**（§8.16 由 PR9-4 分列「T7-15 有读数 / T7-16 待产出」）。

### 文档 · V2 Step 9 **PR9-2 第 2 轮评审响应**（设计 v0.8 → **v0.9**，PR #73）（Refs #71，2026-09-25）

第 2 轮评审（`pulls/73/reviews` `5319498739`，基线 `dc6bfa5`，**响应验证型复审** + 3 条行内）=
**✅ 复审通过 —— 无阻塞项、达到可合并形态；本轮 0×新意见**。第 1 轮 7 条 + 正文两条非编号意见
**逐条被独立验证为落地**（评审明确「不采信响应自述」）；3 条行内 = **2 条验证记录 + 1 条认错回帖**
⇒ **无新意见需处置**。本响应轮**零 `.rs` / 零脚本改动**，设计 **v0.9**（新增 §4.15）：

- **评审自查**：它第 1 轮 P4-3 ③ 的前提**确实不成立**（注记在 `30b61b9` 上已由 `06e8432` 引入，
  它是在 `d3ebf52` 上核的 ⇒ 当时属实、现已过时；根因 = 跨轮基线前进后未在新基线重核遗留清单）
  ⇒ §4.14 的更正成立、该条不再悬置。
- **两条验证记录 + 我方复现**：① 入库 TSV ↔ 臂 JSON **全精度对账 10/10**；② 新配对段**四向测试**
  （正向 2 exit 0 / `signal` 错标与 `theta` 错标各 exit 3）。本响应轮用**我方自己的 65 份臂 JSON**
  独立复现了这两条（方法 = 抽取内嵌 python 段 + 真产物；**判据级 + 产物级**，非端到端重跑）。
- **臂级 JSON 是否入库**（评审征询作者拍板）：**决策 = 不扩面** —— ① TSV + 冻结图指纹已覆盖复算所需
  （已被评审对账**实证**）；② 65 份 JSON 体积大且可用 `OUT_DIR=` 重跑重建。⚠️ 若将来要「按触发率分桶」，
  需要的是 `bench` **增记录**（新面、单独立项），与「JSON 入库」是两件不同的事。

### 文档 / 工具 · V2 Step 9 **PR9-2 第 1 轮评审响应**（设计 v0.7 → **v0.8**，PR #73）（Refs #71，2026-09-25）

第 1 轮评审（`pulls/73/reviews` `5316771251`，基线 `30b61b9`，独立验证型复审 + 7 条行内）=
**✅ 通过 —— 无阻塞项、达到可合并形态（0×P1 + 0×P2 + 3×P3 + 4×P4）**；**「不投」的结论被独立复算成立**
（评审自己重跑 4 条关键臂 + 按 ASCII 归属**逐 query** 复核 `shape` 的 240/80 零例外 ⇒ 与 §4.13 逐位吻合），
且明示**不要求复跑守门链**（本轮零 `.rs` 改动）。**逐条全部采纳**，其中一条按评审倾向做到了更强的形态：

- **P3-1**（`eval_s9.sh` 的 Q-R1 复核行**标签与公式口径不一致**）：改成**两个口径都打印** ——
  paraphrase 桶 **+0.0592**（公式也换成桶口径）+ 全量参考值 **−0.0113** 并注明「不可互换」；
  两个值一并进 TSV 头（原稿只活在 stdout）。
- **P3-2**（G5 **漏核 `signal` / `theta`**）：`assert_g5` 增两参数 + 两条断言（**θ 容差 `1e-6`** ——
  评审实测 JSON 里 θ 是 f32 噪声 `0.550000011920929`，`1e-9` 会全臂误报）；调用点传入本臂身份。
- **P3-3**（读数**不可追溯到持久产物**）：脚本加 `OUT_DIR`（缺省仍临时目录）+ **本轮 TSV 入库**
  `data/eval/s9-2026-09-25/readings.tsv`（含 README）；§4.13 三处同步，并**如实登记「臂级 JSON 未持久化」**。
- **P4-1**（「64 臂」vs 括号 1+1+63=**65**）：按**实测调用数**统一为 **65 次 `bench` 调用**，
  并在 §4.13.1 补出总数（三个载体同口径）。
- **P4-2**（S9-1b 硬编码 `/ 80`；四桶条数只打印不断言；空桶 Δ=0 让 G2 恒真）：三处都改 ——
  分母改**实际桶大小**；桶结构升级为**判据**（和 == 总数 / 无空桶 / 四桶等分，独立退出码 6/7/8）；空桶直接退出。
- **P4-3**（状态行未随 v0.7；§7 的 S9-T11 行未注记）：两处属实、已改；③ §7 的 S9-T6 行「无注记」的
  **前提与实物不符**（该注记在 `30b61b9` 上已由 `06e8432` 引入）⇒ 见响应评论「与评审不同之处」，但仍重排该格防扫读误判。
- **P4-4**：PR 正文「守门链」标题重复 ⇒ 去重。
- 另采纳评审正文的一条**非编号**意见：「内置自检」原用全局标志 ⇒ 只查第一个信号 ⇒ **直接修实现**
  （三个信号各查一次），并写明它与 G5 的覆盖面**互补**。
- **变异 / 验证 5 组**：M1/M2（G5 的 `signal` / `θ` 断言 —— **真接线注入**，改 bash 后真跑）红；
  M3（自检覆盖面：新版红 / 旧版绿两臂）；M4（桶结构判据三臂 ⇒ 分别 exit 6/8/7）；M5（Q-R1 两口径正向）。
- 设计 **v0.8**（§4.14 逐条处置 + §4.13 三处更新）。

### 评测 · V2 Step 9 **PR9-2**：spike S9-S1 —— 自适应融合判「**不投**」（S9-04 / S9-05，T7-14 / FR-35）（Refs #71，2026-09-25）

按 `v2-step9-design.md` **§4.5 的预注册五门**（G1~G5 合取）跑完 θ 灵敏度曲线 ⇒ **三个候选信号无一满足 G4**
（区间达标），其中 `df` / `shape` 连单点达标都没有。**判「不投」是设计认可的合法结论**（D-S9-10）：
交付「工具 + 如实记录未达标」，`NFR-15` **保持「打开」**并写明复审触发条件（设计 **§4.13**，v0.6 → **v0.7**）。

- **交付物 ①（S9-04）**：`scripts/eval_s9.sh` —— 预注册常量（θ 网格 21 点 / G1~G4 阈值）+
  **65 次 `bench` 调用**（三路锚点 1 + 基线 1 + 3 信号 × 21 θ = 63 自适应臂；⚠️ **v0.8 更正**：
  原稿写「64 臂」而括号自己加起来是 **65** —— 第 1 轮评审 **P4-1**）+ **脚本侧按 qid 配对** +
  G5 逐臂自证（`k` / `rrf_k` / 精排关 / **本臂的 `signal` 与 `θ`**（v0.8 补，P3-2）/ 同一冻结图指纹）+
  **内置自检**（**三个信号的** `θ=0.00` 臂**各自**须与基线逐 query 零差；v0.8 起每信号各查，原稿只查第一个）+
  `readings.tsv`（头含冻结图指纹 + 桶结构 + **两个 Q-R1 口径** ⇒ 产物自证；**本轮产物已入库**
  `data/eval/s9-2026-09-25/`，第 1 轮评审 **P3-3**）。**三轮全量复跑曲线逐位相同**（确定性）。
- **交付物 ②（S9-05）**：设计 **§4.13** 记录协议 / 三路锚点 / 三条等价类压缩曲线 / 五门判定 / 机制层解释 / 复审触发条件。
- **读数（冻结图 `data/t2-frozen.snapshot`，320 query，`MRR@10(1)`，两臂同一图）**：
  - **Q-R1 复现**：paraphrase 桶 hybrid **0.4803** ＜ vector **0.5395**（配对均值差 **+0.0592**）；全局 hybrid **0.6941** 仍为三路最高。
  - **`overlap`**：唯一有达标点的信号（θ=0.55/0.60：paraphrase **+0.0189** / exact +0.0025 / natural +0.0208 /
    mixed +0.0128 / 全局 **+0.0137**，三门皆过）——但**最长连续 2 点**（宽度 0.05）＜ G4 的 ≥3 ⇒ **不投**。
  - **`df`**：覆盖率在本语料上几乎恒 1.0 ⇒ 惰性（paraphrase Δ 最大 **+0.0003** ≪ G1 的 +0.01）⇒ **不投**。
  - **`shape`**：paraphrase **+0.0592**（= 完整 vector 上界，机制有效）但 exact **−0.0523** / natural **−0.0207** /
    全局 **−0.0034** ⇒ G2/G3 双破 ⇒ **不投**。
- **三条机制层解释（可核对）**：① `shape` 在 `θ=0.50` 下的 320 条 query **全部落且只落在两个规范态**
  （**240 条逐 query `mrr` ≡ vector 单路**（桶均值差 `+0.000e+00`）/ **80 条 ≡ baseline**（`+0.000000`）/
  **「两者都不等」= 0**）⇒ 触发集恰为 240 条非 ASCII query（75%）、零例外 ⇒ **规则的 query 条件性成立**，
  「一刀降 0」的**取形**才是瓶颈（设计 §4.2 的**档位 `{1.0, 0.5, 0.0}` 形态未实现 ⇒ 未扫**）；
  ② `overlap` 的「2 点」= 信号量子 `1/k = 0.1` 恰为 θ 步长 0.05 的两倍 ⇒ 每个等价类占 2 点、
  G4 需两个相邻类同时通过；③ `df` 惰性（非方向错、是量太小）。
- **覆盖边界（如实登记）**：只扫「降到 0」一种降权取形；`bench` JSON 不记 `FusionCtx` ⇒ 触发集合
  **只在 `shape` 上可零例外指认**（二值信号 + 参照臂正交），`overlap` / `df` 的逐 query 触发集合无产物可指认；
  仅 MRR@10(1) 逐点判定；语料固定 ⇒ 不外推。
...
前两轮的 **1×P3 + 6×P4 经评审逐条验证为闭合**。⇒ 三条新 P4 **一并处置**，设计进入「**PR9-1 可直接开工**」状态。

#### 三条 P4

| # | 处置 |
| --- | --- |
| **P4-7** | 架构侧 **3 处双钉行号又漂了（+26）** ⇒ 全部重取 **`a6d87e4` 实测值**：§14.5 `:1644 → :1670`、§14.6 `:1662-1676 → :1688-1702`（`R54` 在 `:1702`）、读数纪律 `:1687 → :1713`。**根因** = 行号在 `f3d7b2c` 时点量的，而随后的 `a6d87e4` **自己**给架构 §5.6 加了 25 行 + 1 个版本表行 ⇒ 挪了下方所有锚点 ⇒ 上升为文首「锚点体例」的**两条硬纪律**（见下） |
| **P4-8** | 附录 B 的 `for θ in …` **不是合法 bash**（`` `θ': not a valid identifier ``）⇒ 循环变量改 ASCII **`t`**（θ 只留注释 / 文件名语义）。**独立复算属实**：bash 3.2 与 5.x 均报同一条错 |
| **P4-9** | `Metrics.fusion_weights` / `fusion_signal` 的**填充路径未闭合**（P3-1 修复的连锁遗漏）⇒ §4.1 补**规则 3**：开关开时编排层**另调一次** `weights_override(&ctx)` 专用于填 `Metrics`（`&self` 纯函数 ⇒ 两次调用必然同值）；并写明**为什么不能**改 `fuse_adaptive` 的返回签名（会破坏「默认 = `self.fuse`」的转发与 `S9-T6`）、**不能**让策略共享可变状态（破坏 `Send + Sync` 与并发逐位一致）；§6.2 同步补「填充路径」 |

#### 🔴 两条硬纪律（写进文首「锚点体例」）

1. **版本标签与行号必须同一时点** —— 本轮曾标 `v1.24` 却给了 `v1.23` 时点的行号 ⇒ **标签本身是假的**；
2. **行号必须在「本 PR 全部编辑完成后的最终 head」上量** —— 只要本 PR 自身会在锚点**之上**插入内容，中途量到的行号必然漂（本轮实测 **+26**）。

#### ⚠️ 一条自我登记：修 P4-6 时造出了 P4-8

附录 B 的 `for θ in …` 是**第 2 轮响应里为修 P4-6（θ 参数化）而写的**，结果它是非法 bash。
⇒ **「修复」本身也是新命令**，同样要过「能不能真跑」这一关；与「改了做法要回头扫描述该做法的句子」
（第 2 轮）是同一族的另一半：**改了命令就要回头问「它真能跑吗」**。
⚠️ 顺带（评审发现）：`scripts/check_shell_expansion.py` 只扫 `scripts/` 与仓库根 `*.sh`，
**扫不到文档附录里的 bash 块** ⇒ 已登记进 **#69**（文档形态守门）。

#### 版本面

设计 **v0.3** / `plan-v2` **v0.26**（Step 9 设计行改引 v0.3 ⇒ 升；其 7 处双钉的行号经复核**未受影响**、
仅版本标签同步为 v0.26）/ README 同步。⚠️ **架构 v1.24、需求 v1.24 均不变**（本轮没改这两份文档的内容）。

#### 验证

- **锚点自检（新增）**：15 个双钉**逐个**验证「目标文件该行确实含预期内容」⇒ **0 失败**；
  版本标签唯一性 3/3 ✅（≥1 个文件多版本标签即红 —— 正是 P4-7 的形态）。
- **13 段守门链全绿**；测试四配置 `369 / 351 / 348 / 21` ⇒ 与前三轮逐项相同 ⇒ 自证零代码改动。
- ⚠️ **既有 `**` 孤儿复核**（`plan-v2` 头部）：Δruns = **74**（偶）且未配对位置与 `origin/main` 逐字节相同 ⇒ 非本轮引入（#69）。
- ⚠️ 本轮无断言可变异（改动全在文档）⇒ 替代验证 = 多锚点预检 + 正/反向断言 + 锚点自检 + 文档附录 bash 实际执行。


### 文档 · V2 Step 9 设计 **第 1~2 轮评审响应**（`v2-step9-design.md` v0.1 → **v0.2**，PR #70）（Refs #4，2026-09-24）

两轮评审（`pulls/70/reviews` `5300723031` / `5300824624`，基线 `116b1d8`）= **0×P1/P2 + 1×P3 + 6×P4**；
结论：第 1 轮「**通过**」、第 2 轮「**有条件通过**」。⇒ **逐条全部采纳**，设计升 **v0.2** + 新增 **§4.8（评审响应）**。

#### 🔴 P3-1（唯一 P3，建议随本 PR 补 —— 已补）：`weights_override` **只定义了「决策」、没定义「应用」**

- 实装里 `SearchParts.fusion` 是 `&'a dyn FusionStrategy`（`searcher.rs:80`），而 `fuse` 读的是策略**私有**字段
  `self.weights`（`rrf.rs:62`）⇒ 编排层**没有任何合法通道**把 `w` 送进融合计算 —— 「`Some(w)` 时用 `w`」这句**没有落点**。
- **独立复算属实**（未只读评审结论）：`grep -nE "fusion\.fuse|weights_override|fuse_adaptive"` 在
  `searcher.rs` / `fusion/*.rs` 上**无**任何把 `w` 传出去的通道。
- **修法**：再加一个 **provided 方法 `fuse_adaptive(&self, lanes, k, ctx)`**，默认 = `self.fuse(lanes, k)`
  ⇒ **既有实现一行不改、默认逐位一致**；`RrfFusion` 覆写之（内部 `weights_override` 决策 → 以 `w` 跑 RRF，
  抽带权重的私有 helper 与 `fuse` 共用一段数学）。`weights_override` **保留**为决策钩子（`S9-T6` 与
  `Metrics.fusion_weights` 挂它）。
- 并写死显式规则：**`Config::adaptive_fusion == false` ⇒ 编排层不调用 `fuse_adaptive`、不构造 `FusionCtx`**
  ⇒ `S9-2` 的逐位一致是**结构性**的，不是靠约定。
- 同步：架构 **§5.6**（补该方法 + 段末说明）、设计 §4.1 / §6.4 / §7（`S9-T6`）/ 附录 A。
- ⚠️ **与 `Reranker::candidate_window` 先例的差别**（写进 rustdoc）：后者返回值（`usize`）编排层**自己能消费**
  （进 `candidate_k`）⇒ 一个方法即闭环；`weights_override` 的返回值编排层**消费不了** ⇒ 必须再补一个「消费方」。

#### 六条 P4（全部采纳）

| # | 处置 |
| --- | --- |
| **P4-1** | §4.3 补「**对应的 ctx 字段 / 编排层预算量**」列（s1 需 `LaneStats.chunk_ids`、s3 需 `FusionCtx.query_df`、s4 需 `query_chars` + `has_ascii`）+ 显式规则「**ctx 新增字段须同步 `S9-T3` 白名单**」；`LaneStats` 补**空态定义**（`Option`，**不用 `0.0` 哨兵** —— BM25 无上界、`0.0` 是合法分值） |
| **P4-2** | θ 网格**预注册**：`0.00 → 1.00`、步长 `0.05`（**21 点**）；「一个可解释的档」= **≥ 3 个连续点**（宽度 ≥ 0.10）；并写进 `eval_s9.sh` 常量 |
| **P4-3** | 约 **15 处裸行号锚点** ⇒ 全改 **「§ 锚点 + 文档版本 + 行号」双钉**（行号取 HEAD 实测值），并新增文首「**锚点体例**」行 |
| **P4-4** | §4.2 点破 **tier 1 是行为空转**（RRF 只用名次 ⇒ 空 lane 本就零贡献）；`S9-T7` 断言改落**可观测面**（`fusion_weights == Some([0.0, w])`） |
| **P4-5** | S9-1b / S9-1c 容差**统一为 `0.0024`**，口径改为「**政策容差（交易余量）**」—— ⚠️ 同冻结图 ⇒ Δ 是**精确值**、不含漂移成分 ⇒ 该数是**取舍声明**不是噪声界 |
| **P4-6** | 附录 B 的 θ **参数化**为 `--adaptive-fusion-theta <v>` + `for θ in <预注册网格>` 循环（原布尔开关拿不到 θ ⇒ sweep 无从传入） |

#### 其它

- 🆕 **建本步跟踪 issue #71**（Steps 2~8 各有其一；两轮评审均建议随 PR9-0 建）。
- 版本面：设计 **v0.2** / 架构 **v1.24**（§5.6 内容改 ⇒ 升；两表各加一行）/ `plan-v2` **v0.25**（Step 9 块引用改 v0.2 ⇒ 升）/
  `docs/README.md` 索引行同步。⚠️ **需求侧 v1.24 不变** —— 本轮**没改**需求内容（对偶检查：「这份文档的内容真的改了吗？」）。
- ⚠️ **本轮无断言可变异**（改动全在文档）⇒ 替代验证：多锚点预检（34 + 11 条，`assert count == 1`）、
  `mdcheck` 0 处、表内裸竖线 0、围栏配平、**正/反向断言**（旧措辞须 0、新措辞须 ≥1）。
- ⚠️ **两条既有 `**` 孤儿复核**（架构 / `plan-v2` 头部状态行）：基线各 1 处、HEAD 各 1 处、
  Δruns = **26 / 58**（均为偶）且未配对位置与 `origin/main` **逐字节相同** ⇒ **非本轮引入**（已登记 **#69**）。
- 🔑 **评审侧如实登记的教训**（值得跨轮记住）：第 1 轮以**事实核验**为主（源码主张 / 读数 / 锚点逐条复算、全部属实）
  却**放过**了 P3-1 ⇒ **「事实全对」不代表「机制闭合」** —— 核验清单里要单列「机制是否闭合」这一类。


### 文档 · V2 Step 9 详细设计（`v2-step9-design.md` v0.1，PR9-0）（Refs #4，2026-09-23）

> **纯设计 PR、`.rs` 零改动**（体例同 `v2-step5-design.md` / `v2-step8-design.md` 的首版）。
> 范围 = **T7-14** 自适应融合（**FR-35** / 判据 **NFR-15**）+ **T7-15** 假负例对照 + **T7-16** Agent query 评测集（后两者**只作对照**）。

#### Added

- **新增** `docs/devel/v2-step9-design.md` **v0.1**：决策 **D-S9-01 ~ D-S9-10** + 验收 **S9-1a~d / S9-2 / S9-3** + 测试计划 **S9-T1 ~ S9-T12** + **风险 R55 ~ R58**（→ 架构 **§14.7**）+ 附录 A ~ D。
- **三条口径已由用户拍板（2026-09-23）**：**H7 = 只作对照、不替换主基线**；**统计口径 = 按幅度读、不声称显著**（Step 7 教训：配对符号检验最小 p = **0.1553**）；**T7-16 的 LLM 通道 = 外部 LLM API**（形态 provider-agnostic：三个环境变量 + OpenAI 兼容端点；⚠️ **语料外发待确认**）。

#### Fixed

- 🔴 **架构 §5.6 的 `FusionStrategy` 伪码与实现漂移 ⇒ 按实现更正**（`architecture-design.md` §5.6）：原伪码 ① 把 `weights` 写成 `fuse` 的**形参**（实际在**构造期注入**）、② 用了**不存在的**类型名 `ScoredHit`、③ **漏了 `k` 形参** —— 三处都与 `crates/core/src/fusion/mod.rs:21-28` 不符。**原伪码引文保留**（设计与 §14.7 的注记里），不静默删除。

#### Docs

- `plan-v2.md` → **v0.24**：§4 Step 9 补「设计已出」块 + **就地更正两处**（**`T7-16 → T7-14` 的依赖不成立** / 验收已定值）+ §6 门槛行补开工前置 + §7 进度表新增设计行 + §5 的 `FR-35` / `NFR-15` 行补设计锚点 + **V2.1 横切表**补 **`#68`** 行、标注 **`#69`**、刷新 **`#2`** 行。
- `requirements-spec.md` → **v1.24**（§1.1 + 附录 C **两处**都记）：`FR-35` 备注补设计锚点 + **措辞更正提案**（『按桶』→『按 query 侧可观测特征、**不得依赖相关性标注**』，**待评审拍板**，**原措辞保留**）；`NFR-15` 补「**三条口径已定、数值待标定**」（⚠️ 明确区分，避免「指标列写草案 / 备注列写已定」的自相矛盾）。
- `architecture-design.md` → **v1.23**（§1.1 + §15.2 **两处**都记）：§5.6 更正 + **新增 §14.7（R55 ~ R58）** + §14 导读补承接行。
- `docs/README.md`：新增 `v2-step9-design.md` 索引行 + `plan-v2` 行补状态。

#### 设计期新发现（用源码推翻计划里的想当然）

- 🔴 **「桶」含标注**：`exact` / `paraphrase` 的划分用到 `qrels` 里 `grade ≥ 1` 的段落（`examples/t2_prep.rs:206-230`）⇒ **生产不可用 + 标签泄漏**，而 `NFR-15` 的字面口径正是「在**分桶**上优于固定权重」⇒ 设计把「**桶 = 评测分组**」与「**自适应信号 = 检索自身可观测**」**分开**（D-S9-02 / 判据 S9-3）。
- 🔴 **`FusionStrategy::fuse` 没有查询上下文**（`fusion/mod.rs:21-28`）⇒ `FR-35` 在「不动接口」下**结构性做不到** ⇒ 取 **trait provided 方法**（先例 = `Reranker::candidate_window`，同仓已用过的非破坏手法）。
- ⚠️ **「`T7-16` 是 `T7-14` 的前置」不成立**：`data/t2-queries.jsonl` 的 `type` **就是桶标签**（四类各 80、已物化）、`bench` 已按它出分桶表 ⇒ **判据今天就能算**；T7-16 降为**外部效度增量**。
- ⚠️ **`bench --modes` 表达不了「hybrid@权重A vs hybrid@权重B」** ⇒ 判据走**脚本侧跨运行配对**（D-S9-09），且**必须同一冻结图**。
- ⚠️ **80 条/桶的样本量约束** ⇒ 自适应规则只允许「**零参数 / 单阈值**」形态，判据要求**区间达标**（决策门 **G4**），**单点达标不算**。

#### 覆盖边界（本轮**明确不声称**）

- **零实现**：本轮只有设计文档 + 四处定义面回写，`git diff main..HEAD --name-only | grep -c '\.rs$'` = **0**；判据 / 实现 / 评测载体 / 两个评测资产**全部在后续 PR**（`PR9-1` ~ `PR9-3`）。
- **`NFR-15` 未定稿**（**数值**待 spike `S9-S1`）；**`FR-35` 的措辞更正只是提案**（待评审拍板）。
- ⚠️ **两条既有 `**` 孤儿**（架构与 `plan-v2` 的**头部状态行**）**仍未修**（已立项 = **`#69`**）：本轮**编辑了这两行** ⇒ `mdcheck --vs <base>` 按**内容**比对会报「新引入 1」= **内容级假报**；判据 = **块内 runs Δ 为偶数**（架构 `151 → 159`，Δ = 8；`plan-v2` `427 → 467`，Δ = 40）+ **未配对位置逐字节相同** ⇒ **非本轮引入**。

### 文档 · V2 Step 8 收尾（`S8-09`）：**第 2 轮评审 —— 复审通过**（Refs #58，2026-09-23）

> `#67` 第 2 轮评审（`pulls/67/reviews` `5287966572`，基线 **`04a668d`**，**纯 review body、无新增行内意见**）
> = **0×新意见 + 1×P4（文档小疵）**，结论「**复审通过、无阻塞项、达到可合并形态**」；
> 第 1 轮的 **1×P3 + 3×P4 逐条签收 ✅**。

#### Fixed

- 🔵 **P4：设计 §4.13.5 的「版本面」段与实际改动矛盾** —— 该段写「本轮**不升**需求 / 架构 /
  `plan-v2` 的版本号 ⇒ 只**就地扩写**那三行版本描述」，而实际升了 **v1.23 / v1.22 / v0.23**。
  根因：这段是**决策改变之前**写下的 —— 先按「同一件收尾工作不重复开版」写下「不升号」，
  随后按仓内先例（`v1.21` 就是为**两处笔误**升的）改为**升版本行**，结果**只改了版本行、
  漏改了「描述这件事」的那一段**。⇒ **已改准**，旧句**保留**并标注「旧措辞更正」。
- 🔑 **一般化（已写进设计 §4.13.6）**：**「改了做法，就要回头扫『描述该做法的句子』」** ——
  它与「改了行为要回头改正文里的旧断言」（`#66` 的功课）是**同一类**，但**载体位置更隐蔽**：
  不在「结果」里，而在「**方法 / 说明**」里。⇒ **提交前判据**：对本轮写下的每个
  **否定式 / 选择式说明**（「本轮**不** X，只 Y」「改为 Z」）**再确认一次它仍然成立**
  （本轮出错的句子恰好就是「本轮**不**升…」这个形态）。

#### Changed

- 设计文档升 **v0.17** + 新增 **§4.13.6 第 2 轮评审响应**（逐条复核表 + 根因与一般化 +
  **版本面对偶检查** + 未闭环）。
- ⚠️ **本轮除 §4.13.5 那一句与新增节外零改动**：判据 / 读数 / 门槛结论**均不受影响**；
  需求 / 架构 / `plan-v2` **无需再动**（第 2 轮没有改它们的**内容** ⇒ **不再升版本行**，
  可复核 `git diff --stat`）。

#### 复核（评审的独立复跑）

评审用**更新后的脚本**又跑了一次（默认参数、`SKIP_WRITE=1`、exit 0）：②a **1.1325 / 1.0114**、
**早期切片对照列正常输出**（1.1224 / 1.0352）、②b **3.47 / 4.25 ms**、四档 **19 / 22 / 23 / 21 us**
（档4/档1 = 1.1053，**无门槛标记** ✓）、R52 分列 谓词 **+1 us** / 多段 **+3 us**；
CI head `04a668d` **10/10**。⇒ 与本机**同参数不同轮次**的读数（②a 1.09 / 1.07、四档 20/24/25/22）
**同量级、同结论**；⚠️ 仍**只报「同向 + 同结论」，不做逐位比对**（跨轮次数字不可相减）。

### 文档 · V2 Step 8 收尾（`S8-09`）：**第 1 轮评审响应** —— NFR-10 ③ 判后记录 + 早期切片对照（Refs #58，2026-09-22）

> **范围**：`#67` 的第 1 轮评审（**0×P1/P2 + 1×P3 + 3×P4**，结论「**无阻塞项**」）**四条全部采纳**。
> ⚠️ **本轮仍无生产代码改动**（`crates/*/src/**` 零改动）；唯一被改的 `.rs` 是
> **`examples/eval_rw_concurrency.rs`**（标定载体 —— 非库代码、不进 public API）。

#### Changed

- 🔴 **`NFR-10` ③ 的判后记录（评审 P3-1）** —— 这是 `v1.20` ④ **预注册**的义务（「若判『不投』⇒ ③ 如实记为
  未达标、`R43` 保持打开」）；PR7a 把它的回写归 PR7b，但本轮之前**一直没有落地**。三个载体各补一句：
  ① 需求 **`NFR-10` 行**（升 **v1.23**）；② 架构 **§14.4 的 `R43` 行**（升 **v1.22**）；
  ③ **`plan-v2` §6 的门槛行 ④**（升 **v0.23**）。
  🔑 口径：③ **维持未达标**（**不是**「待测」）、`R43` **保持打开**；⚠️ **「触发条件已达成」≠「③ 已解决」**
  （会话池**已落地但默认 1**）；⚠️ 池化**确有增益**（未池化对照 1.23× → 池化 **2.10×**）⇒ FAIL **不得**读成「池化无效」。
- 🔵 **脚本去门槛（评审 P4-2）**：`scripts/eval_rw_concurrency.sh` 不再对 `档4/档1 ≤ 1.2` 打 ✅/❌，
  改为「**读数（非门槛）**；档 4 语料 ≠ 档 1」；**顺带**更正脚本头部同族的「第 4 档**必须**回到第 1 档量级
  （取 ≤ 1.2× 作可判定的读数口径）」（旧措辞保留在注释里供对照），并把 R52 的两项成本**分列**
  （谓词 = 档3−档2、多段扫描 = 档2−档1 —— 与 `eval-report.md` §8.15.7 的两条读法对齐）。

#### Added

- 🔑 **「窗口早期切片」对照（评审 P4-1）**：`B1` 窗口内布局随 `merge_pending` **逐段改善**（17 段 → 1 段）
  ⇒ 整窗聚合把「合并干扰」与「布局改善」混在一起。载体现新增 `B1_early_quarter` 臂（窗口前 25%）+
  `p99_b1early_over_a1` 字段；`eval-report.md` §8.15.6 加对照表。**r4 实测**：
  ...
  ⇒ **均值口径的混合效应为真且大**（hybrid 的 −12.2% 几乎全是布局改善贡献；与评审复跑的 −11.6%
  **同向同量级**），但 **p99 主判据几乎不受影响**（整窗 vs 切片：bm25 1.0604 / 1.0478、
  hybrid 0.9597 / 0.9698）⇒ **②a 的结论不变**。
- `eval-report.md` §8.15.6 / §8.15.7 各补**精确复现命令**（评审 P4-3）：
  `MAIN=4000 DELTA=400 DELTAS=16 Q=400 TOMB=40`，并写明「脚本默认 = 2000/200/8/20/300
  ⇒ 按默认复跑得到**另一组绝对数**（比值与门槛结论仍复现）」。

#### Fixed（数据 / 文档诚信）

- 🔴 **更正 `eval-report.md` §8.15.6 与设计 §4.13.1「保留 1」的「均值同向」措辞**：该「同向」在 **hybrid** 上
  **只是数值同向、机理不同向** —— 那 −10.8% 主要是**布局改善**的贡献；换成早期切片后 mean 变成 **−0.4%**。
  原句**保留**（供对照）+ 加更正块，**不静默改**。
- ⚠️ **本文上一条目里的「`git status --porcelain | grep -c '\.rs$'` = 0」在本轮不再成立**（载体 `.rs` 被改）
  ⇒ 已在该条目里**就地更正**为「`crates/*/src/**` 零改动 + 唯一被改的 `.rs` 在 `examples/` 下」。

#### 覆盖边界

- 早期切片**不是判据**（切片 `n` 更小 ⇒ 分位数更不稳）；「同深度尾部」仍是**未决项**；
  本轮**未**在 Linux / CI / 其它核数上复跑（评审的复跑与本轮读数**同机**）。
- 两条既有 `**` 孤儿（架构与 `plan-v2` 的头部表格块）**仍是各 1 处**：本轮对该块的 runs 增量为**偶数**
  （架构 119 → 151、`plan-v2` 335 → 433），且未配对位置的上下文与 `origin/main` **逐字节相同**
  ⇒ **非本轮引入**、**不擅自改**；⚠️ `mdcheck --vs` 按**内容**比对会误报「新引入 1」。

### 文档 · V2 Step 8 收尾（`S8-09`）：NFR-14 / NFR-11 定稿 + 四处定义面回写（Refs #58，2026-09-22）

> **范围**：把 Step 8 实现期产生的欠账一次性落进**定义面** —— 标定 `S8-T11` / `S8-T12` / `S8-T15`、
> **`NFR-14` ② 与 `NFR-11` 的判据定稿**、**四处定义面回写**、**`R51`~`R54` 残余回填**、**修 `FR-17` 的状态漂移**。
> ⚠️ **零生产代码改动**：改动面 = 文档四处 + 设计 + **新增 demo/脚本** + 本文件
> （`crates/*/src/**` 的改动数 = **0**；`.rs` 只动在 `examples/` 下（两个新增 + 1 个被改 —— ⚠️ **后者的表述已由第 1 轮评审响应条目更正**：原写「`git status --porcelain | grep -c '.rs$'` = 0」，在本轮改过载体后**不再成立**，保留原句供对照））
> **不进 public API**）。裁定的方向 = **「② 口径定稿去『拟』、但 `NFR-14` 整体保持『打开』」**
> —— 不是「修并发」，也不是「只记录不判定」。

#### 📊 Changed（定义面：NFR 口径）

- **`NFR-14` ②（读延迟）口径定稿（去「拟」）**：②a 相对判据 ≤ **1.2**，🔴 **基线定义写死 = 「合并**起始**布局」**
  （⚠️ **不得**改用「合并**完成后**的静止期」—— 那个布局已被合并改变，会得到 1.07 / 1.26 / **1.41** 的**假 FAIL**）；
  ②b **定稿为「数量级护栏」**（合并期 P99 仍在 NFR-02 该模式预算内；⚠️ 本档语料规模与原协议不同）。
- 🔴 **`NFR-14` 整体保持「打开」（三条并记）**：① 判据口径已定稿；② **另立一条如实记未达标/未决** ——
  **样本数不可比**（A 臂 400 条 ⇒ 其「P99」≈ 第 4 大；B 臂 3.7k~12.7k）+ **尾部扰动「可复现同向、
  不可复现幅度」**（B 的 `max` = 8.5 / 40.8 / 23.5 ms vs A1 = 15.1 / 4.1 / 3.5 ms，**一次反向**；
  同深度第 4 大一次达 **2.74×**）；③ **复审触发条件**（更稳环境/交错执行、宿主把合并放后台线程、
  向量侧合并路径变动）。
- **`NFR-11`（写延迟）拟值定稿（去「拟」）**：`P50 ≤ 1.0 s / P99 ≤ 2.0 s` **保持原值**（实测 0.694 s / 1.026 s）
  + **写明口径边界**（`batch_size = 64`、**n = 40** ⇒ 「P99」实际是**最大值**、语料 **2560 篇**而拟值原文写「12K」、
  不含加载）+ 复审触发条件（12K 全量口径复跑）。
- ⚠️ **不改任何其它预算数值**（NFR-02 ~ 15 的其它阈值 / 预算 / 时限一字未动）、**不增删 FR / NFR**。

#### 📌 Added（风险登记：`R51` ~ `R54` 残余回填）

- **`R51`** 收窄：spike S8-S1 **5 条判据全 PASS**（A/B = **0.134**）⇒ **方案 A 已落地**；残余 = `HnswIo`
  累积（≈200B + 路径串 / 次）**+ 图拓扑仍漂移**（`insert` 新点后的 ANN 排名不承诺逐位一致）⇒ **保持开放**。
- **`R52`** 已量化：`bm25_elapsed` 四档 p50 = **44 / 52 / 53 / 50 us** ⇒ **逐 posting 谓词仅 +1 us（+2%）**、
  **多段扫描 +8 us（+18%）**；档 4 回到 `segments = 1` / 墓碑 0（**可逆**）⇒ **保持开放**（是读数不是门槛）。
- **`R53`** 已测：②a 三次运行皆 ≤ 1.2 ⇒ **判据口径定稿**；**尾部扰动不可复现** ⇒ **保持开放**。
- **`R54`** ✅ **消解**：spike S8-S2 **判「不投」** ⇒ 池化不进生产默认路径（`embed_sessions` 默认 1）
  ⇒ **条件未被触发**。⚠️ 是「**因为判『不投』**而不发生」，**不是**「没人碰」—— 将来翻转须**重新打开**。
- ⚠️ **不新增 / 不删除任何风险编号**（R1 ~ R50 一字未改）。

#### 🧪 Fixed（定义面漂移）

- **`FR-17` 的状态漂移**（两处）：FR 表行 + `§5.3.5` 的「**读不阻塞写的完整版归 V2.1**」→
  **已由 V2.0 Step 8 交付**（PR #60 ~ #66 + PR7b）。⚠️ **保留原措辞引文**并写明它是 2026-09-07 步骤重排的残留。

#### 📈 Measured（新增实验与载体）

- **`S8-T11` / `S8-T12` / `S8-T15` 已落地** ⇒ 新增 `crates/core/examples/eval_rw_concurrency.rs`
  （三 mode：`latency` 的 A1→B→A2 三臂 / `hotpath` 四档 / `write-latency`）+ 新增
  `scripts/eval_rw_concurrency.sh`（逐档位独立进程 + TSV + 合取表）。读数落 `eval-report.md`
  **§8.15.6 ~ §8.15.8**。
- **`S8-T11` 实测**（4000 篇主段 + 16 增量段 × 400 篇 + 40 跨段墓碑；合成嵌入器；三次运行）：
  ②a = bm25 **1.107 / 1.049 / 1.063**、hybrid **0.992 / 0.927 / 0.993**（基线 = A1）；
  ②b = bm25 **3.69 ms ≤ 5 ms**、hybrid **7.54 ms ≤ 20 ms**。
- **`S8-T15` 实测**（真实 `bge-small-zh-v1.5`，`batch_size = 64`，40 批 × 64 篇）：
  **P50 = 0.694 s / P99 = 1.026 s** ⇒ 两项达标（余量 31% / 49%）。
- 🔑 **工具新增的一条纪律**：`remove` **只写 `builder.tombstones`**，墓碑要**再 `commit()` 一次**才发布进
  `View`（`SearchMode` 无关）—— 少了那一次 `commit()`，「合并期有墓碑要物理化」这臂会**静默测错对象**
  （本轮实测踩到并写进 example 注释）。

#### 🗂 Docs（四处 + 版本行 + 设计版本 + 顺手修的历史欠账）

- **需求 `v1.22`**：NFR-14 ② / NFR-11 定稿 + FR-17 状态修正（头部状态行 + 文档版本行 + §1.1 + 附录 C 四处同步）。
- **架构 `v1.21`**：§14.6 的 `R51` ~ `R54` 残余回填 + 收尾句（头部状态行 + 文档版本行 + §1.1 + §15.2 四处同步）。
- **`plan-v2` `v0.22`**：§6 的 V2.1 门槛 Step 8 行（三条各自结清、**① 记 🟡** —— `NFR-14` 整体待复审）、
  §7 进度表 Step 8 五行（补齐 PR 号与状态：设计 `v0.1 → v0.15`、`T7-25` #61、`T7-26` #66、`T7-06` #62~#65、
  `T7-18` 并入 #62）、§8 回写计划补本轮。
- **`docs/README.md`**：`plan-v2` 与 `v2-step8-design` 两行的索引状态同步；风险区间 **R1~R54 不变**。
- **设计 `v2-step8-design.md` `v0.15`**：§4.13.1 / §4.13.2 定稿回填 + **新增 §4.13.4（标定结果与裁定）** +
  §7 的 `S8-T11`/`T12`/`T15` 标已落地 + §9.1 的 `R51`~`R54` 残余同步 + 头部版本链与状态行。
- **顺手修的历史欠账（收尾自查）**：① **`§7` 的 `S8-T2` / `T3` / `T7` 补「实际用例名」映射**
  （台账审计发现三条无同名用例，逐一核实为**语义已覆盖、只是命名不同** ⇒ 写进落点列，免得后人重做审计）；
  ② **`eval-report.md §8.15` 的导读**从「由 `S8-09` 续填」改为「已续填」。
- ⚠️ **两条既有孤儿登记（不擅自改）**：架构与 `plan-v2` 的**头部状态行**各有 **1 处 `**` 孤儿**，
  经「块内 `**` run 数奇偶」核对：**`origin/main` 与 HEAD 皆 1 处、Δ runs 为偶数 ⇒ 非本 PR 引入**
  （`mdcheck --vs` 按**内容**比对会报「新引入 1」——那是**内容级比对的假报**）。定位手段无鉴别力 ⇒ 只登记。

#### 覆盖边界（本轮**明确不声称**守住的）

- **`S8-T11` 不覆盖**「真实模型下查询编码与合并的交互」（用合成嵌入器是**更严**的比值口径，见设计 §4.13.1）；
  **不覆盖** Linux / CI / 其它核数（过订阅程度不同 ⇒ 读数量级可能变）。
- **`S8-T15` 不覆盖** 12K 全量口径、Linux / CI；**`n = 40` ⇒ 「P99」实为最大值**（保守但样本少）。
- **`S8-T12` 的「档 4」≠「档 1」**：档 4 的语料比档 1 多 delta 那 400 篇 ⇒ 「回到第 1 档量级」只能读作
  「回到 **1 段的形态**」（零谓词），不是同一个数字。
- **尾部扰动**：见上（不可复现 ⇒ 不入判据，但已登记为未决项）。

### 测试 · V2 Step 8 PR 7a 第 1 轮评审响应（`S8-08`）（2026-09-22）

评审结论 = **0×P1/P2 + 1×P3 + 3×P4 ⇒ PR7a 范围内无阻塞项、达到可合并形态**
（`pulls/66/reviews` `5273480265`，`COMMENTED`，基线 `45d2566`；另有行内 **3 条**）。
**逐条全部采纳**；另有一项「不立项」按评审归属留 `PR7b`。本轮**无断言可变异**（改动全是文档 / 注释）。

#### 修复 / 变更

- 🟡 **P3-1（公开面文档未回填标定结论）**：把「**由 spike S8-S2 决定**」（将来时）全部改成
  **已判「不投」+ 当前没有 >1 的推荐值** + 指向设计 §4.11.4 / `eval-report.md` §8.15。
  🔑 **载体实为 7 处**（评审点到 **2** 处，另 5 处同族扫到）：`search/config.rs` 的 builder rustdoc /
  `Default` 注释 / `Config::embed_sessions` 字段文档 / `required_local_embedder` 的 CLI 注释，
  以及 `embed/local.rs` 的 `LocalEmbedder` 类型文档（**2 处**：内存门槛、`intra_threads` 的数值影响）
  与 `new_with_sessions`。⚠️ 后 4 处**写在结论之前是将来时、结论一出就成了错话**。
- 🔵 **P4-1（`slot_index` 注释与代码不符）**：原注释写「取模前先把计数器收敛（`% len` 后再加）」
  与「回绕到**负**」—— 实现是 `fetch_add(1) % len`（**先加后取模**、计数器**无界递增**、`usize` 无负值）。
  ⇒ 注释改写为「收敛的是**返回值**、不是计数器本身；回绕点最多一次调度『不轮转』，语义无害」。
- 🔵 **P4-2（判据② 的映射钉死）**：`eval-report.md` §8.15.3 新增**映射定义**
  （= `NFR-10` ② 的字面口径：**同 `sessions` 下 threads 4 vs 1**）+ **并列三个读法**：
  `2.10× / 2.04× / 1.66×` ⇒ 🔑 **三者皆 < 2.5× ⇒ 「不投」的结论对读法稳健**
   （**不是**「换一个分母就达标」）。
- 🔵 **P4-3（`new_with_sessions` 无上界守卫）**：**不加 cap**（与 `batch_size` 同类：信任调用方）
  ⇒ 改为**写清失败模式**（并更正评审的一处前提，见下「更正」第 1 条）。
- ⚪ **（不立项）既有 `默认配置零配置可用` 仍走 `build_config()` 联网**：**同意归 `PR7b`**，
  本轮把**载体范围**查清并登记（见下）。

#### 🔴 更正两处「说法不准」（数据诚信）

1. **更正评审 P4-3 的一处前提**：评审写「`embed_sessions(usize::MAX)` 会 `Vec::with_capacity`
   **直接 abort**」。**实测（`rustc` 直编复刻、同类型宽度）**：**是 panic（`capacity overflow`）
   而不是 abort** —— `catch_unwind` **能**捕获；真正的阈值 = `N × size_of::<Mutex<TextEmbedding>>()`
   `> isize::MAX`（**本机构建**实测该尺寸 **1232 字节** ⇒ `N > 7.49e15`），**远在实用范围之外**；
   **先撑爆的是内存**（≈ **97 MiB / 槽**）。⇒ 建议本身采纳，但**失败模式必须写准**。
2. **更正我自己上一轮的登记**（本文件与 PR 正文都写过）：「无缓存环境下花 **~85 s**
   **尝试取模型并失败**」—— **两处都不准**：**实测（全新 `HOME`、串行）= 取模型成功**
   （下载 **96 MB / 54 s**、**零失败告警**）；**失败只出现在「冷缓存 + 并发」**
   （`Failed to retrieve model file 'onnx/model.onnx'`）⇒ 这**正好解释** CI 首轮那次红灯
   （同一用例两次构造「一失败一成功」—— CI 每次都从**冷缓存**开始、且 runner 默认并行跑测试）。
   ⇒ 正确措辞是「冷缓存下取模型**很慢且非确定**」，**不是**「取不到模型」；`~85 s` 是**多用例合计**
   的模糊数（单用例冷缓存实测 **54 s**）。
   ⚠️ 附带一条差点被采信的**假读数**：`/tmp/nohome` 会被上一次跑**写满** ⇒ 二次测量得到的是
   「有缓存」读数（**0.87 s**），差点据此写下「不慢」——**每次必须 `rm -rf` 新建 `HOME`**。

#### 「联网用例」的载体范围（`PR7b` 的清单）

- 全仓 `build_config()` 调用点 **12 处**（生产 **3**：`SearchIndexBuilder::build()` / `::load()` /
  `SearchIndex::load()`；测试 **9**）；
- 其中**会触默认构造器（因而读模型 / 联网）的只有 3 处**，全在 `search/config.rs` 的 **2 个用例**里
  （`默认配置零配置可用`、`并行建图默认开且可显式关闭`）；
...
- **本轮无断言可变异**（P3-1 / P4-1 / P4-2 / P4-3 全是**文档与注释**）⇒ **明说「不适用」**。
  替代验证：① **事实复核** —— 逐处回读改后文本（**正向断言 8 条**）+ 反向扫旧措辞（**4 条须 0 次**）；
  ② **P4-2 的三个读数是复算出来的** —— 从 `eval-report.md` §8.15.2 的八臂原表按三种分母各算一次
  （`2.098 / 2.038 / 1.662` ⇒ 2.10 / 2.04 / 1.66）。

#### 覆盖边界（本轮**明确不声称**守住的）

- 「三种读法皆 FAIL」只在**本条协议**（10 核 / 2000 篇冻结图 / 12 条短 query × `queries = 300`）下成立；
  换核数 ⇒ 过订阅程度变 ⇒ 量级可能变（`§8.15.5` 的未覆盖项不变）。
- P4-3 的失败模式是**实测 + 源码级**证据（`catch_unwind` + `size_of`），**不是**把 `usize::MAX`
  真的喂给生产 `LocalEmbedder` 跑一遍。
