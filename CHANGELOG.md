...
- **`BRUTE_FALLBACK_MAX_ALLOWED`：1024 → 8192**（`crates/core/src/vector/hnsw_rs_index.rs`）。
  10 万级 A/B 标定（`--brute-fallback off` vs 强制精确，8 档位 × 2 轮）实测交叉点在
  `allowed ≈ 9700`（区间 `(5043, 9946)`，且**两个端点无需归一化**就成立），取 2¹³ 留 ~15% 余量
  （路径 C 的 `O(N)` 项随规模增长 ⇒ 交叉点下移，宁低勿高）。
...
- `docs/devel/eval-report.md` **§8.9**：标定完整 A/B 表（8 档位 × 2 轮）、方法与噪声说明、定稿阈值下的
  路由验证、以及**三条局限**。⚠️ 方法学要点：**两轮非交错执行 ⇒ 控制组归一 ×1.382 / ×1.541；
